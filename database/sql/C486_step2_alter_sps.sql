...
-- STEP 2: ALCase GET (based on original, add Invoice after MaterialCode aliases, Note after Total)
...
END;
GO
PRINT 'STEP 2 OK: ALCase GET'
GO

-- STEP 3: ALCase IUD (based on original, add Invoice + Note)
...
BEGIN
    ...
    BEGIN TRY
        ...
        SELECT 
            ...
            COALESCE(T.atP,T.Person), COALESCE(T.atV,T.Vendor), COALESCE(T.atF,T.Factory),
        ...
        FROM OPENXML(@iDoc, @InsertTableName, 3) WITH (
            ...
            Total VARCHAR(50), Note NVARCHAR(500)
        ) AS T
