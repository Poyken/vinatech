...
-- exec [usp_VVT_checkFIFO_FinishGood] 'VVPP162R710604',0



CREATE PROCEDURE [dbo].[usp_VVT_checkFIFO_FinishGood] 
...
BEGIN
	...
	if(isnull ( @partno,'')='') begin
		...
		from STB_VN_FINISHGOODS  with(nolock) where lotno=@pLotNo
	end
	...
	;with data1 as (
	select lotno ,'' oldbarcode, MaterialCode,'' oldmaterial,locations from 	STB_VN_FINISHGOODS fg with(nolock)
	...
	,data2 as (
		...
		join STB_VN_FINISHGOODS fg with(nolock) on NewBarcode = fg.LotNo and fg.flag=1 and isnull(fg.dateexport,'')='' and  isnull(fg.typeexport,'')='' and isnull(Statusout,'')<>N'Xuất'
	...
	--where barcode = (select lotno from 	STB_VN_FINISHGOODS fg with(nolock) where lotno=@pLotNo)

	--select * from 	STB_VN_FINISHGOODS fg with(nolock)
...
--select top 1000 * from STB_VN_FINISHGOODS
