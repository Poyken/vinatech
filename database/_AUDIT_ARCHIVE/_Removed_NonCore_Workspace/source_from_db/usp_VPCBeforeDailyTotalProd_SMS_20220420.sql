...
Create PROCEDURE [dbo].[usp_VPCBeforeDailyTotalProd_SMS_20220420]
   ...
   FROM  (  
                ...
                --  [1] 권취누계  									
					SELECT 'VPC권취합계 ' As LineCode
							, Format(Convert(BIGINT, 178000),  'N0')AS PlanQty                                         -- 고정값 세팅
							, Convert(Varchar(10), DateAdd(Day, -1, GetDate()), 121)  As ProdDateTime
							, '권취' As RouteName
							, '[권취합계]' As LineName
							, Format(CONVERT(BIGINT, Sum(A2.ProdQty)),  'N0') AS ProdQty 
							, Round(Convert(NUMERIC(20,2), (Sum(A2.ProdQty) / 178000.00 * 100)), 1) AS ProdRate                  -- 고정값 세팅													
					 FROM (
										SELECT  B.LineCode
   	             ...
   	             --  [2] 커링누계 Start  									
				        UNION ALL
							               ...
							               SELECT 'VPC조립합계 ' As LineCode
													, Format(Convert(BIGINT, 155000),  'N0')AS PlanQty                                         -- 고정값 세팅!!!
													, Convert(Varchar(10), DateAdd(Day, -1, GetDate()), 121)  As ProdDateTime
													, '조립(커링)' As RouteName
													, '[조립합계]' As LineName
													, Format(CONVERT(BIGINT, Sum(A2.ProdQty)),  'N0') AS ProdQty 
													, Round(Convert(NUMERIC(20,2), (Sum(A2.ProdQty) / 155000.00 * 100)), 1) AS ProdRate          -- 고정값 세팅!!!													
													FROM (
																SELECT  B.LineCode
																		, IsNull(B.PlanQty, 0) As PlanQty
																		, A.RouteName						
...
,010-4214-5342,010-7612-7123,010-5613-0679,010-8848-8231,010-7687-1601,010-6439-6622,010-2378-5681,010-2875-1676,010-2901-2070,010-9263-9714
