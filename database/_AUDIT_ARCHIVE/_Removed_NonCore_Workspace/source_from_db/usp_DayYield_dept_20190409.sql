...
-- =============================================
Create PROCEDURE [dbo].[usp_DayYield_dept_20190409]
...
SELECT '합계'                      AS RouteName
        ...
        , SUM(C.누계생산)        AS  누계생산
		, CASE WHEN   SUM(C.일일생산수량)  = 0 THEN 0 ELSE  SUM(C.누계생산) /  SUM(ISNULL(C.일일생산수량, 0))    END       AS  누계달성율   
		, SUM(C.누계생산) / 99                            AS 누계진도율    
 FROM (
