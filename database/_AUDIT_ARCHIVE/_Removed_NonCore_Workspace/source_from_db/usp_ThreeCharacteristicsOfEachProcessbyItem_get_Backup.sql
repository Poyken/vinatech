...
-- Description:	
-- Modified:
-- 실행문 :  usp_ThreeCharacteristicsOfEachProcessbyItem_get '','','0820','','2020-12-01','2020-12-31','IQC_GPD_19'
-- =============================================
Create PROCEDURE [dbo].[usp_ThreeCharacteristicsOfEachProcessbyItem_get_Backup]
...
SELECT A.Size
	...
	--	, A.CompanyCode AS CompanyCode
		...
		--, CASE WHEN A.CompanyCode= 'VNT' THEN SUM(STDEV_Value)
		--         WHEN A.CompanyCode = 'VVT' THEN 0	                                             END AS VNT_STD


		, CASE WHEN A.CompanyCode  = 'VNT' THEN  AVG_Value
		          WHEN A.CompanyCode  = 'VVT' THEN 0	                                             END AS VNT_AVG

		, CASE WHEN A.CompanyCode  = 'VNT' THEN  CPK
		      WHEN A.CompanyCode  = 'VVT' THEN 0	                                             END AS VNT_CPK


		--, CASE WHEN A.CompanyCode = 'VVT' THEN STDEV_Value
		--         WHEN A.CompanyCode = 'VNT' THEN 0	                                             END AS VVT_STD

		, CASE WHEN A.CompanyCode = 'VVT' THEN  AVG_Value 
		          WHEN A.CompanyCode = 'VNT' THEN 0	                                             END AS VVT_AVG

	    , CASE WHEN A.CompanyCode = 'VVT' THEN  CPK 
		          WHEN A.CompanyCode = 'VNT' THEN 0	                                             END AS VVT_CPK
FROM (		
				...
				SELECT SUBSTRING(MBI.ModelName, CHARINDEX('(', MBI.ModelName, 0) + 1, 4) AS Size
						...
						, SUM(MQSR.TestValue) as SUM_Value
						, AVG(MQSR.TestValue) as AVG_Value
						, STDEV(MQSR.TestValue) as STDEV_Value
