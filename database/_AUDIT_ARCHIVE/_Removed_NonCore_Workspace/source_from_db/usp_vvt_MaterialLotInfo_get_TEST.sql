...
-- [usp_vvt_MaterialLotInfo_get_TEST] '','','VVT', 'VVT_F1', 'ROH_VN_WH', '',  '',  '',  '',  '',  '',  '',  '',  '',  '',  '',  ''

-- [usp_vvt_MaterialLotInfo_get_TEST] '','','VNT', 'VNT_F1', 'ROH_VN_WH', '',  '',  '',  '',  '',  '',  '',  '',  '',  '',  '',  ''
-- [usp_vvt_MaterialLotInfo_get_TEST] '','','VNT', 'VNT_F1', 'ROH_BG_WH', '',  '',  '',  '',  '',  '',  '',  '',  '',  '',  '',  ''

-- [usp_vvt_MaterialLotInfo_get_TEST] '','','VVT', '', 'ROH_BG_WH', '',  '',  '',  '',  '',  '',  '',  '',  '',  '',  ''
CREATE PROCEDURE [dbo].[usp_vvt_MaterialLotInfo_get_TEST]   -- exec usp_vvt_MaterialLotInfo_get_TEST '','','VVT', 'VVT_F1', 'ROH_VN_WH_01', '',  'GBAKAC-054',  '',  '',  '',  '',  '',  '',  '',  '',  ''
...
;WITH LABELINFO AS
	(
	 ...
	 basedat as (
		...
		select top (1)* from STB_MaterialWarehouseInOutHist  order by CreateDateTime DESC
) SELECT
			...
			case when isnull(substring(MDD.MRMDExtText04,1,10),'')<>''  then substring(MDD.MRMDExtText04,1,10) else convert(varchar(10),mdd.createdatetime,120) end as DateImport,
	...
	FROM
		...
		left join holddate inout with(nolock) on mdli.LotID = inout.LotID
		left outer join STB_MaterialDocInfo mdi with(nolock)  on mdd.MaterialDocNo = mdi.MaterialDocNo
	...
	where  
			...
			--and 
			--(@pMaterialWarehouseCode='TOTAL_MATERIALS'    and isnull(MLI.MaterialWarehouseCode,mdli.MaterialLocationCode) not like 'PROD%'    and  isnull(MLI.MaterialWarehouseCode,mdli.MaterialLocationCode)  not like 'ROUTE%'
			--	or isnull(MLI.MaterialWarehouseCode,mdli.MaterialLocationCode) LIKE @MaterialWarehouseCode+'%') 			
				
			--and

			 (@MaterialCode = '*' OR MLI.MaterialCode LIKE @MaterialCode) 			
			--and (@LotID = '*' OR MLI.LotID = @LotID)					   
			and (MaterialDocType<>'GI' or MaterialDocType is null)
			--or mdli.LotID in (select LotID from basedat)
			and MLI.LotID ='ML20240914000750'
