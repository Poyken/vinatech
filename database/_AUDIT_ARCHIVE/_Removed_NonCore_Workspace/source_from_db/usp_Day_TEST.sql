...
CREATE PROC [dbo].[usp_Day_TEST] 		
AS

---[실행문]    EXEC  usp_Day_TEST
...
BEGIN
 ...
 --[기존값]
	...
	--DECLARE @ToDt              VARCHAR(19) = CONVERT(VARCHAR(10), DATEADD(DAY, 1, CONVERT(smalldatetime, GetDate())), 121) + ' 08:30:00'                     -- 금일 오전 8시반        SELECT CONVERT(VARCHAR(10), DATEADD(DAY, 0, CONVERT(smalldatetime, GetDate())), 121) + ' 08:30:00'
...
-- [수동으로 실행시 -> 지우지말것]
	DECLARE @FromDt             VARCHAR(19) =  '2019-04-11 08:30:00'            -- 전일                    
	DECLARE @ToDt                VARCHAR(19) =  '2019-04-12 08:30:00'            -- 금일
	DECLARE @ToDay              VARCHAR(02) =   '11'                                  -- 전일자로 넣음
...
--  SELECT * FROM CURLING_LINE WHERE 라인명 = '베트남'
									...
									(
									 -- Z부분
												...
												WHERE 1=1															   
												   ...
												   AND 작업일자 BETWEEN  '2016-01-01 08:30:00'  AND  '2019-05-01 23:00:00'	
					...
					GROUP BY  지시번호 			
					  ...
					  -- [베트남부분 추가예정]
						...
						-- 사이즈 1325, 설비 권취 3호기, 13일기준으로 이전은 베트남수량, 이후는 본사수량  
							...
							WHERE 1=1																																						
								AND D.작업일자 BETWEEN '2018-01-01 00:00:00' and '2019-04-30 08:30:00'                                  --  이설정대로 하면 주야간 모두
 ...
 SELECT GG.라인명                         AS 정렬라인명
		...
		, GG.특이사항                                                                                       AS 특이사항
		, SS.일일총수량                                                                                     AS 일일총수량	 
		, CONVERT(VARCHAR(10),  GetDate(), 121)                                                    AS 기준일자               -- [주석]풀을것
		, GG.라인코드                                                                                        AS 라인코드		
				...
				-- GG START
							...
							(
									SELECT  PL.라인명
										...
										FROM CURLING_PLAN   PL                                                                        -- 월별게획 
												LEFT OUTER JOIN CURLING_PROD PO ON   PO.라인명 = PL.라인명           -- 일별실적
								WHERE 1=1
								--AND PL.라인명 = '01호기'
...
SET @sqlStr = 'UPDATE CURLING_PROD   SET DAY' + @ToDay + ' = ISNULL(B.[일일총수량], 0)    FROM CURLING_PROD A     INNER JOIN #TEMP_TABLE100 B  ON A.[라인코드] = B.[라인코드]'
...
-- TEMP TABLE삭제
  ...
  DROP TABLE    #TEMP_TABLE101     --> 베트남관련 TEP_TABLE
