...
-- 프로시저실행 (2021-12-21)  : EXEC [usp_LotTrackingInfo_get_20211221] 'kilee','Korean', '', 'VNT', '2021-12-20', '2021-12-20', 'E-33', '', '', '', ''
-- ===========================================================================================================
CREATE PROCEDURE [dbo].[usp_LotTrackingInfo_get_20211221]
...
AS
	...
	DECLARE @FromDate         VARCHAR(19) = CONVERT(VARCHAR(10), @pFromDate, 121) + ' 08:30:00'                                                            -- SELECT  CONVERT(VARCHAR(10), '2021-12-20', 121) + ' 08:30:00' 
...
BEGIN
	...
	(
		...
		FROM
				...
				Left Outer join STB_ProdRouteSummary PRS WITH(NOLOCK)	  ON PRS.PONo = SI.PONo   
	...
	SELECT CASE WHEN PRH.ComPanyCode = 'VNT' THEN '전주본사'
		                          ...
		                          SELECT PoNo 
								       ,   Max(JobDate) as JobDate
										FROM STB_ProdRouteSummary WITH(NOLOCK) 
										 WHERE 1=1										    
										 GROUP BY PoNo
						       )   PRS  ON PRS.Pono = SI.Pono            -- 2021.12.21 추가
	 ...
	 --  AND PRH.ProdDateTime BETWEEN @FromDate AND @ToDate
	   ...
	   AND (PRH.RouteCode IN ('E-27', 'E-28') 
				OR PRH.RouteCode IN (SELECT RouteCode 
