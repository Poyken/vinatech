...
Create PROCEDURE [dbo].[usp_ProductStockInfoLookup_get_20210423]
  				...
  				SELECT A.ProductStockNo
					  ...
					  , Case When  LEN(Case When Len(A.Day) = 1 then     CONCAT(A.Year, A.Month, '0',  A.Day)  Else   CONCAT(A.Year, A.Month, A.Day) End)  =  8 Then    DATEDIFF(dd,  Convert(Datetime, Case When Len(A.Day) = 1 then     CONCAT(A.Year, A.Month, 0,  A.Day)  else   CONCAT(A.Year, A.Month, A.Day) End) , Getdate() )   
						...
						, A.MBISizeW   -- 파이추가
							 ...
							 , (ISNULL(A.ManufacturingUnitPrice, 0)  * ISNULL(A.StockQty,0)) AS StockPrice
			FROM (

						  -- 기존
								SELECT MAX(PSI.ProductStockNo) as ProductStockNo
									  ,PSI.CompanyCode
									  ,CI.CompanyName
									  ,PSI.WorkCenterCode
									  ,WCI.WorkCenterName
								   ...
								   , CASE  WHEN LEN(PSI.Barcode) < 15 THEN  ISNULL(Convert(Varchar(20), SubString(PSI.Barcode, 5, 2)), 0)
									  ...
									  , MAX(PSI.CreateDateTime)  as CreateDateTime
									  , MAX(PSI.CreateUserID)      as CreateUserID
									  , MAX(PSI.ChangeDateTime) as ChangeDateTime
									  , MAX(PSI.ChangeUserID)     as ChangeUserID
									 , MBI.MBISizeW
                                ...
                                Group by   PSI.CompanyCode
											 ...
											 -- ,PSI.StockQty
											 , MBI.MBISizeW
											 , ISNULL(MCR.CostPrice, (MCRM.CostPrice * MBI.MBIExtInt01)) 
											 , MCR.CostTypeCode 
							  )   A
