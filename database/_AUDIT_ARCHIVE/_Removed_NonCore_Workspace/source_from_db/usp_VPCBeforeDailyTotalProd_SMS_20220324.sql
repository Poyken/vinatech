...
-- Browsable : True
-- Group : SMS문자메세지 > 매일 오전 8시반에 [전일실적] - 3월3일에 수정보완
-- Description: 
...
-- 프로시저 실행 :                    usp_VPCBeforeDailyTotalProd_SMS_20220324  '',''
-- ========================================================================
CREATE PROCEDURE [dbo].[usp_VPCBeforeDailyTotalProd_SMS_20220324]
...
-- 1. 권취공정
    -- 1-1. SMS를 위한 메세지 조립
  SELECT @msg = @msg + Z.LineName + ' (' + Convert(Varchar(20), PlanQty) + '개 ' + ' | ' + Convert(Varchar(20), ProdQty) + '개 ' + ' | ' + Convert(Varchar(20), ProdRate) + '% ' + ')<br>'
   FROM  (  
              ...
              -- [1] 라인별 집계
				SELECT A2.LineCode
						...
						--, SUM(A2.DefectQty) AS DefectQty  --불량수량
				...
				FROM (
								...
								FROM  (
														...
														--RIGHT OUTER JOIN STB_ProdLinePlan B  WITH(NOLOCK) ON A.LineCode = B.LineCode  														
														   RIGHT OUTER JOIN STB_VPCLinePlan B  WITH(NOLOCK) ON A.LineCode = B.LineCode       -- 2022.03.13
														...
														--		WHERE IPQ.CreateDateTime BETWEEN CONVERT(VARCHAR(10), GetDate()-1, 121) + ' 08:30:00'  AND CONVERT(VARCHAR(10), GetDate(), 121) + ' 08:29:59'
														--		 AND DPP.LineCode  <> 'ASSYLINE-05'
							...
							---[2] 누계추가 (2022.03.10)
										...
										SELECT 'VPCLINE-합계 ' As LineCode
													...
													, Round(Convert(NUMERIC(20,2), (Sum(A2.ProdQty) / 139000 * 100)), 1) AS ProdRate                  -- 고정값 세팅													
													FROM (
												SELECT  B.LineCode
															, IsNull(B.PlanQty, 0) As PlanQty
															, A.RouteName						
												...
												FROM  (
																		...
																		--				LEFT OUTER JOIN STB_RouteInfo RI    	    ON RI.RouteCode = IPQ.RouteCode
																		--		WHERE IPQ.CreateDateTime BETWEEN CONVERT(VARCHAR(10), GetDate()-1, 121) + ' 08:30:00'  AND CONVERT(VARCHAR(10), GetDate(), 121) + ' 08:29:59'
																		--			AND NOT DPP.LineCode LIKE 'ASSYLINE%'
																		--		GROUP BY DPP.LineCode, Convert(Char(10), IPQ.CreateDateTime, 121), RI.RouteName, LI.LineName
...
--			      + '[슬리브실적]' + '<br>' + @msg2 + '<br>' 
			...
			SET @LineMessage = 'VPC 전일 권취실적 재공유드립니다.' + '<br>'
			            + ' [전달사항] : ' + 'VPC 4라인 13,500개가 추가로 입력되어 전일 누계수량에 착오가 있었습니다. 금일부터 중간저장기능을 제외시키도록 하겠습니다. 혼란을 드려 대단히 죄송합니다.' + '<br>' 			
	                    + ' [기준일] : ' + ISNULL(@YesterDate,'') + '<br>'
						+ ' [기준시간] : ' + ISNULL(@StandardTime,'') + '<br>'								
						+ '<br>'
						+  '[권취 실적]' + ' ( ' + '목표수량' + ' | ' + '생산수량' + ' | ' + '완료율(%) )' + '<br>'
			            +	@msg + '<br>' 
...
-- 1. 테스트용 (나만)	
 --      Exec usp_DoSendSMS '', '', '010-2320-7233,', @LineMessage , ''    -- 나만
       ...
       Exec usp_DoSendSMS '', '', '010-2320-7233,010-4654-3727,010-4652-7649,010-4214-5342,010-3222-6697,010-2060-1714,010-2912-1830,010-9433-1324', @LineMessage , ''    -- 나만
