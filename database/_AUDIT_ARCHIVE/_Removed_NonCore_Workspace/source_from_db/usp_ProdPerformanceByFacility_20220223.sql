...
-- Browsable : True
-- Group : 생산관리 > 생산현황관리 > 생산현황 > [B657] VPC일별공정별 불량현황 
-- Description: [B657] 일별공정별설비별_생산실적
-- Modified:  상위 Grid 조회용 프로시저

-- 프로시저 실행:   usp_ProdPerformanceByFacility  'kilee2','Korean', 'E-22', '2022-02-01' ,'2022-02-15'
-- ===============================================================================================
CREATE PROCEDURE [dbo].[usp_ProdPerformanceByFacility_20220223]
...
BEGIN
   ...
   SELECT CONVERT(CHAR(10), FIN.ProdDateTime, 120) AS ProdDateTime
     ...
     -- , CONVERT(NUMERIC(20,2), SUM(FIN.ProdQty)) AS 생산수량
        , CONVERT(NUMERIC(20,1), SUM(FIN.DefectQty)) AS 불량수량
        , CONVERT(NUMERIC(20,1), SUM(FIN.DefectQty) / SUM(FIN.InputProdQty) * 100) AS 불량률
   ...
   FROM (
      SELECT   TOP 100000
			   ...
			   , CASE WHEN RI.RouteCode = 'E-22' AND (MM.MachineName IS NULL OR MM.MachineName = '' OR MM.MachineName = '권취1호기' OR MM.MachineName = '셀7 권취기' OR MM.MachineName = '' OR MM.MachineName = '셀#5 권취기' OR MM.MachineName = '셀#1 커링&슬리브기') THEN '셀#1 권취기'
         ...
         FROM STB_SetInfo SI WITH(NOLOCK) 
               ...
               LEFT OUTER JOIN ( SELECT DRI2.ControlNo
                              ...
                              WHERE RepairType = 'NONE'
                                AND DefectCode NOT IN ('E-22_X03','E-22_X12','E-24_X03','E-24_X12','E-24_ZZ','E-33_ZZ','E-22_QQ','E-23_QQ','E-24_QQ')
...
;WITH Items AS
	...
	FROM #Third_Table  New
     ...
     Drop Table #DefectReportInfo_Test     -- Temp테이블 삭제
