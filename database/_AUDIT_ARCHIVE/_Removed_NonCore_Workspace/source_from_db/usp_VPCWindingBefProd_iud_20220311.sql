...
-- 프로시저 실행 :  usp_VPCWindingBefProd_iud_20220311 '',''


-- 테이블 조회 :     Select * from VPC_Performance Order by RouteCode
-- ===============================================================================================
Create PROCEDURE [dbo].[usp_VPCWindingBefProd_iud_20220311]
...
BEGIN
	...
	DECLARE @FromDate DateTime = Convert(Varchar(10), GetDate()-1, 121) + ' 08:30:00'            -- 전일8시반        Select Convert(Varchar(10), GetDate()-1, 121) + ' 08:30:00'  
	DECLARE @ToDate    DateTime = Convert(Varchar(10), GetDate(),    121) + ' 08:29:59'            -- 금일8시29분     Select Convert(Varchar(10), GetDate(), 121) + ' 08:29:59'    
	...
	--DECLARE @FromDt     VARCHAR(19) = CONVERT(VARCHAR(10), GetDate(), 121) + ' 08:30:00'                         -- 전일 오전 8시반       SELECT CONVERT(VARCHAR(10), DATEADD(day, -1, @Date), 121) + ' 08:30:00'                       
	--DECLARE @ToDt        VARCHAR(19) = CONVERT(VARCHAR(10), DATEADD(Day, 1, GetDate()), 121) + ' 08:29:59'     
	DECLARE @ToDay      Varchar(02) = Substring(Convert(Varchar(10), GetDate()-1, 121), 9, 2)             --테이블에 데이터 입력되는날짜이므로 전일이 맞다!!  / Select Substring(Convert(VARCHAR(10), GetDate()-11, 121), 9, 2) 
...
SELECT A2.LineCode
		...
		, CONVERT(BIGINT, Sum(A2.ProdQty)) AS ProdQty 
										...
										FROM STB_SetInfo SI WITH(NOLOCK) 
											...
											LEFT OUTER JOIN ( SELECT DRI2.ControlNo
																...
																WHERE RepairType = 'NONE'
																	...
																	--AND PRH2.ProdDateTime BETWEEN CONVERT(VARCHAR(10), GetDate()-1, 121) + ' 08:30:00'  AND CONVERT(VARCHAR(10), GetDate()+1, 121) + ' 08:29:59'																
