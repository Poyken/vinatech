...
Begin
 ...
 if @WorkCenterCode ='VVT_F2'
    ...
    LineCode, LineName, LineType, routecode, routename, 
    _id, 
    _errorcode, _errorname, 
    beginOccur,  
    beginFix1 AS beginFix, 
    delf,  
    id, 
    errorcode AS endcode,
    status, 
    finishTime1 AS finishTime,
    finishFix1 AS finishFix,    
    del,

    -- Tính thời gian hoạt động (DuringOP)
    CASE 
        WHEN ISNULL(DATEDIFF(MINUTE, beginOccur, ISNULL(finishTime2, getdate1)), 0) = 0 
            THEN 1 
        ELSE DATEDIFF(MINUTE, beginOccur, ISNULL(finishTime2, getdate1)) 
    END AS DuringOP,

    -- Tính thời gian sửa lỗi (DuringFix)
    CASE 
        WHEN ISNULL(DATEDIFF(MINUTE, beginFix2, ISNULL(finishFix2, getdate1)), 0) = 0 
            THEN (
                CASE 
                    WHEN ISNULL(DATEDIFF(MINUTE, beginOccur, ISNULL(finishTime2, getdate1)), 0) = 0 
                        THEN 1 
        ...
        ELSE DATEDIFF(MINUTE, beginFix2, ISNULL(finishFix2, getdate1)) 
    END AS DuringFix,
...
WHERE 
    ...
    -- Chỉ lấy các dòng có DuringOP <= 1000
    AND (
        CASE 
            WHEN ISNULL(DATEDIFF(MINUTE, beginOccur, ISNULL(finishTime2, getdate1)), 0) = 0 
                THEN 1 
    ...
    -- Chỉ lấy các dòng có DuringFix <= 1000
    AND (
        CASE 
            WHEN ISNULL(DATEDIFF(MINUTE, beginFix2, ISNULL(finishFix2, getdate1)), 0) = 0 
                THEN (
