...
CREATE PROCEDURE [dbo].[usp_DoCreateVVTLabel]
	@pProcessLanguage VARCHAR(20),
	@pProcessUserID VARCHAR(20),	
	@pMaterialDocDetailNo VARCHAR(30),
	@pLabelQty INT,
...
BEGIN
	...
	BEGIN		
		RAISERROR( 'Khong the tim thay don Nguyen lieu.',16,1)
		RETURN
	END
	...
	BEGIN
		RAISERROR('Khong phai don Nhap nguyen vat lieu.',16,1)
		RETURN
	END

	IF @DocStatus <> 'ARRIVAL' BEGIN
		RAISERROR('Trang thai don chua la ARRIVAL.',16,1)
		RETURN
	END
	...
	if(@CompanyCode='VNT' or @CompanyCode='*') begin
		...
		IF @AllowQty < @CurrentQty + @MakeQty	
		 BEGIN
			RAISERROR('Khong the in tem nhieu hon so luong ReceiveQty.',16,1)
			RETURN
		  END
	end

	--for Vietnam only use decimal value
	if(@CompanyCode='VVT') begin
		IF @AllowQty + @PackingQty < @CurrentQty + @MakeQty	
		 BEGIN
			RAISERROR('Khong the in tem nhieu hon so luong ReceiveQty.',16,1)
			RETURN
		  END
	...
	IF ISNULL(@MaterialLocationCode,'') = '' BEGIN
		RAISERROR('Thieu thong tin Kho nhap.',16,1)
		RETURN
	END
	...
	WHILE @Index <= @LabelQty BEGIN
		...
		SELECT
				...
				MDD.StockAttrib2,
				MDD.StockAttrib3,				
				case when @Index=@LabelQty then (@AllowQty - (@LabelQty-1) * @PackingQty) else @PackingQty end,
				CASE @IsAutoCheck
					WHEN 'Y' THEN 1
					ELSE 0
				END,		-- IsCheck
