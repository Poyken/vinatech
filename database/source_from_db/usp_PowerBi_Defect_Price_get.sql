...
-- [프로시저 실행문]     usp_PowerBi_Defect_Price_get 'kilee','Korean','2020-03-01 00:00:00', '', 'VVT'
-- ==================================================================

CREATE PROC [dbo].[usp_PowerBi_Defect_Price_get]
...
BEGIN
	...
	--        , IsNull(SUM(Taget_Defect_Price), 0)													   AS Taget_Defect_Price
	-- FROM MEDIUM_PLAN MP
	-- WHERE  1 = 1 
	--    AND 기준년월 =  (                                                                                                  
	...
	--								 AND  ToDate     >= @OneDay
	--							)      
	--	AND 공정코드 IN ('V-28', 'E-28')
	--    AND ((@CompanyCode = '*') OR (CompanyCode = @CompanyCode))    
 --   Group By LineCode, 공정코드
 --   ORDER BY LineCode


	--- 이전것 백업
	Select LineCode
	     , (Select LineName From STB_LineInfo SL WHERE SL.LineCode = SDP.LineCode) As LineName
	     , TargetDefectPrice 		 
		 , CompanyCode
	From STB_Target_Defect_Price	 SDP
	where 1=1
	 AND ((@CompanyCode = '*') OR (CompanyCode = @CompanyCode))    
