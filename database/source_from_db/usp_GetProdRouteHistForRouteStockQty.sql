...
--              2020-06-02 포장공정 완료된 LotNo는 제외시킴 ->  주영진 부장
--              2020-06-17 생산안되는 라인데이터 제외요청 -> 주영진 부장    

--  EXEC usp_GetProdRouteHistForRouteStockQty 'kilee', 'Korean','VNT', 'VNT_F1', '', '', 'VJKO093R015602'
-- =============================================

CREATE PROCEDURE [dbo].[usp_GetProdRouteHistForRouteStockQty]
...
BEGIN
  ...
  SELECT	POR.PONo,
			...
			CASE WHEN  SUM(PRH.ProdQty) - CASE WHEN ISNULL(NPOR.RouteCode,'') = '' OR SUM(NPRH.ProdQty) > SUM(PRH.ProdQty)  THEN SUM(PRH.ProdQty)
										...
										END - CASE WHEN ISNULL(SUM(DQI.DefectQty), 0) > 0 
												...
												END < 0
            THEN 0 ELSE 
				SUM(PRH.ProdQty) - CASE WHEN ISNULL(NPOR.RouteCode,'') = '' OR SUM(NPRH.ProdQty) > SUM(PRH.ProdQty) THEN SUM(PRH.ProdQty)
												ELSE CASE WHEN SUM(NPRH.ProdQty) <= SUM(PRH.ProdQty) 
														  THEN ISNULL(SUM(NPRH.ProdQty),0)  ELSE 0 END 
												END - CASE WHEN ISNULL(SUM(DQI.DefectQty), 0) > 0 
														   THEN ISNULL(SUM(DQI.DefectQty), 0) 
														   ELSE 0 
													   END
...
---- [B630] 공정재고정보 화면에서 LotNo를 제외시키는 경우  ------------------------------------------------------ 주석부분

--select * from STB_SetInfo 
--where 1=1
-- and InputLineCode in ( 'ASSYLINE-06', 'ASSYLINE-12')


--select * from STB_SetInfo 
--where 1=1
-- and InputLineCode in ( 'ASSYLINE-01', 'ASSYLINE-02', 'ASSYLINE-03', 'ASSYLINE-04', 'ASSYLINE-06', 'ASSYLINE-08', 'ASSYLINE-1102', 'ASSYLINE-12', 'ASSYLINE-14')
-- and IsProdFinish <> 1
