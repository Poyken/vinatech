...
-- Browsable : true
-- =============================================      usp_Vietnam_MTTR_Andon_get '',''          usp_DoProcessDefectRepairInfoByBarcode_SmartApp
CREATE PROCEDURE [dbo].[usp_Vietnam_MTTR_Andon_get]
...
combinedat as (
 ...
 ---- DuringFix,
  ...
  isnull(DATEDIFF(MINUTE, beginOccur, isnull(finishTime,getdate1)  ),0) DuringOP
  ,isnull(DATEDIFF(MINUTE, beginFix, isnull(finishFix,getdate1) ),0)    DuringFix
 ...
 --,case when isnull(DATEDIFF(MINUTE, beginFix, isnull(finishFix,getdate1) ),0)=0 then 1 else DATEDIFF(MINUTE, beginFix, isnull(finishFix,getdate1)  ) end DuringFix
  ...
  , mttrtotal as (
  select linecode,DAILY,NULL WEEKLY,NULL MONTHLY,DuringOP as DuringOP_minutes,DuringFix as DuringFix_minutes, counte,round(DuringOP/counte/60.0,2) as MTTR_hours
  from mttrday
  union 
