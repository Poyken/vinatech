...
-- =============================================
CREATE PROCEDURE usp_MaterialQcSampleResult_get_BendingCutting
...
BEGIN
	...
	-- position move
	select @companycode=CompanyCode from STB_MaterialQcInfo_BendingCutting WITH(NOLOCK) where materialqcno=@pMaterialQcNo
	...
	begin
		...
		-- check value ok or not to update attribute='OK'
		 ...
		 select @SampleQty=sampleQty,@PatternID=QcInspectionItemCode,@LSL=LSL,@USL=USL  from STB_MaterialQcDetail_BendingCutting where MaterialQcNo=@MaterialQcNo and MaterialQcDetailNo=@pMaterialQcDetailNo
		...
		begin
			...
			if(@value1 > 0)
			begin
				  delete top(@value1) from STB_MaterialQcSampleResult_BendingCutting  where MaterialQcNo=@pMaterialQcNo and MaterialQcDetailNo = @pMaterialQcDetailNo and TestValue is null
		...
		begin
			...
			begin
				  ...
				  delete top(@value7) from STB_MaterialQcSampleResult_BendingCutting  where MaterialQcNo=@pMaterialQcNo and MaterialQcDetailNo = @pMaterialQcDetailNo and TestValue is null
		...
		begin
			...
			if(@valueocv > 0 )
			begin
				  delete top(@valueocv) from STB_MaterialQcSampleResult_BendingCutting  where MaterialQcNo=@pMaterialQcNo and MaterialQcDetailNo = @pMaterialQcDetailNo and TestValue is null
		...
		begin
			...
			if(@value2 > 0)
			begin
				  delete top(@value2) from STB_MaterialQcSampleResult_BendingCutting  where MaterialQcNo=@pMaterialQcNo and MaterialQcDetailNo = @pMaterialQcDetailNo and TestValue is null
		...
		set @SampleQty=10
		 ...
		 select @cnt= count(*) from STB_MaterialQcSampleResult_BendingCutting  where MaterialQcNo=@pMaterialQcNo and MaterialQcDetailNo = @pMaterialQcDetailNo
		...
		begin
			...
			begin
				...
				select @SampleNo = COALESCE(max(materialqcsampleno),0) from STB_MaterialQcSampleResult_BendingCutting where MaterialQcNo=@MaterialQcNo and MaterialQcDetailNo=@pMaterialQcDetailNo
						...
						begin
							...
							begin
								...
								insert into STB_MaterialQcSampleResult_BendingCutting(MaterialQCNo, MaterialQCDetailNo,materialqcsampleno, testvalue,createdatetime, CreateUserID ) values(@MaterialQcNo,@pMaterialQcDetailNo,@SampleNo+1,@value8, DATEADD(HH, -2, GETDATE()),'system')
					...
					begin
						...
						begin	
							...
							insert into STB_MaterialQcSampleResult_BendingCutting(MaterialQCNo, MaterialQCDetailNo,materialqcsampleno, testvalue,createdatetime, CreateUserID ) values(@MaterialQcNo,@pMaterialQcDetailNo,@SampleNo+1,@value, DATEADD(HH, -2, GETDATE()),'system')
	...
	FROM STB_MaterialQcSampleResult_BendingCutting MQSR WITH(NOLOCK)
	LEFT OUTER JOIN STB_MaterialQcDetail_BendingCutting MQD WITH(NOLOCK)	  
