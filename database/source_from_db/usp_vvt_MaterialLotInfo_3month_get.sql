...
-- [usp_MaterialLotInfo_get] '','','VNT', 'VNT_F1', 'ROH_WH', '',  'GAKA00-002',  '',  '',  '',  '',  '',  '',  '',  '',  '',  ''

-- [usp_vvt_MaterialLotInfo_get] '','','VVT', 'VVT_F1', 'ROH_VN_WH', '',  'ECVT27-382',  '',  '',  '',  '',  '',  '',  '',  '',  ''

-- =========================================================================
CREATE PROCEDURE [dbo].[usp_vvt_MaterialLotInfo_3month_get]
...
SELECT
			...
			dateadd(  month,  convert(INT,case when MMExtInt01 is null or MMExtInt01='' then 3 else MMExtInt01 end) , mdi.BasicDate ) as  EffectDate,

			case when dateadd(  month,  convert(INT,case when MMExtInt01 is null or MMExtInt01='' then 3 else MMExtInt01 end) , mdi.BasicDate ) <= getdate() then 'Expired' 
			when  dateadd(  month,  convert(INT,case when MMExtInt01 is null or MMExtInt01='' then 3 else MMExtInt01 end) , mdi.BasicDate ) <= dateadd(day, 
	...
	FROM
		(
		select '' MaterialDocDetailNo,MaterialLotNo,LotID,MaterialLocationCode,MaterialCode,StockQty,LotAttr10 
		from STB_MaterialdocLotInfo  WITH(NOLOCK) where MaterialLocationCode LIKE @MaterialWarehouseCode+'%'
		union all 
		select '',MaterialLotNo,LotID,MaterialLocationCode,MaterialCode,currentqty,LotAttr10 
		from STB_MaterialLotInfo  WITH(NOLOCK) where lotid like 'SP%' and MaterialWarehouseCode LIKE @MaterialWarehouseCode
		) mdli --with(nolock) 
	...
	where  
		    ...
		    MLI.CreateDateTime < dateadd(month,-3,getdate()) and
			( isnull(MLI.MaterialWarehouseCode,mdli.MaterialLocationCode) LIKE @MaterialWarehouseCode+'%') AND			
			
			(@MaterialCode = '*' OR MLI.MaterialCode LIKE @MaterialCode) AND (@LotID = '*' OR MLI.LotID = @LotID)
