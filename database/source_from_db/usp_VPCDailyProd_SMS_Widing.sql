...
-- Browsable : True
-- Group : VPC생산실적 자동문자메세지 발송 > 하루에 2번 문자발송!! (오전오후8시반)
-- Description: 
...
-- 프로시저 실행 :              usp_VPCDailyProd_SMS_Widing  '', ''
-- ================================================================================
CREATE PROCEDURE [dbo].[usp_VPCDailyProd_SMS_Widing]
 ...
 -- SELECT @msg = @msg + Z.LineName + ' (' + Convert(Varchar(20), PlanQty) + ' / ' + Convert(Varchar(20), ProdQty) + ' / ' + Convert(Varchar(20), ProdRate) + '% ' + ')<br>'
  ...
  --FROM  (  
			...
			-- [Main 조회부분]
				...
				FROM (
								...
								FROM  (
														...
														--RIGHT OUTER JOIN STB_ProdLinePlan B  WITH(NOLOCK) ON A.LineCode = B.LineCode  And A.LineName = B.LineName
														RIGHT OUTER JOIN STB_VPCLinePlan B  WITH(NOLOCK) ON A.LineCode = B.LineCode  And A.LineName = B.LineName
														...
														UNION ALL
																...
																WHERE 1=1
																    AND IPQ.CreateDateTime BETWEEN CONVERT(VARCHAR(10), GetDate(), 121) + ' 08:30:00'  AND CONVERT(VARCHAR(10), GetDate()+1, 121) + ' 08:29:59'
																    AND NOT DPP.LineCode LIKE 'ASSYLINE%'
																	AND IPQ.RouteCode = 'E-22'
         ...
         --   Select  '[권취 실적]' + '<br>' + @msg + '<br>' 
			...
			--SET @LineMessage = 'VPC 금일 권취실적 공유드립니다.' + '<br>'
	  --                  + ' [기준일] : ' + ISNULL(@ProdDateTime,'') + '<br>'
			--			+ ' [기준시간] : ' + ISNULL(@StandardTime,'') + '<br>'
			--			--+ ' [특이사항] : ' + 'VPC 7라인 권취기 시험 가동중입니다.' + '<br>'                           -- 특이사항 부분
			--			+ '<br>'
			--			+  '[권취 실적]' + ' ( ' + '목표수량' + ' | ' + '생산수량' + ' | ' + '진행률(%) )' + '<br>'
			--            +	@msg + '<br>' 						
