...
CREATE PROCEDURE [dbo].[usp_RCV_ASN_Total_iud]
...
BEGIN
	...
	IF @MaterialCode IS NULL BEGIN
		RAISERROR('품목코드가 존재하지 않습니다. 패킹ID를 확인해주세요. [패킹 ID : %s]', 16, 1, @PackingID)
	END

	-- 품목명
	SELECT @MaterialName = REPLACE(MaterialName, ',', '_' )
	...
	BEGIN
		IF @MaterialWarehouseCode IN ('PROD_STBY_VN_WH', 'PROD_STBY_WH') 
		
		--BEGIN  -- 법인 구분없이 완제품 대기창고에 있으면 완제품 창고로 이동한다.
		--	IF @CompanyCode = 'VNT' 
