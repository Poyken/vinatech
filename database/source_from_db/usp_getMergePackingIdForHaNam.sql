...
-- Description:	Lấy ra packing gốc
-- exec usp_getMergePackingIdForHaNam '','','pkql2000229'
-- =============================================
CREATE PROCEDURE [dbo].[usp_getMergePackingIdForHaNam]
...
BEGIN
	...
	select  @pMergeParentId = MergePackingId from  STB_MaterialLotInfo where LotID=@pMergeParentId 

	--select @pMergeParentId=MergeNilonToSmallBox from STB_DividePackaging where PackingID=@pMergeParentId;
    -- Insert statements for procedure here
		;WITH LabelInfo AS
	(
		SELECT
	...
	SELECT 
		...
		from STB_PackingHN710_HN POP WITH(NOLOCK) 
		...
		where POP.PackingId=@pMergeParentId
END
