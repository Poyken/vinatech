...
---[실행문]     EXEC [usp_Medium_Daily_Input_Sudong2]                                --  EXEC usp_Medium_Daily_Input_20200326  '2020-03-26', 'V-22'

CREATE PROC [dbo].[usp_Medium_Daily_Input_Sudong2] 		
				--@pDate DATE,
												  ...
												  SELECT   사이즈             AS 규격   
													...
													(																																	
																	...
																	FROM							   STB_ProdRouteSummary  A                                                                                                
																		...
																		WHERE 1=1							
																		   AND A.TIMECODE <> 'E'																																																			   																			
																			AND A.JobDate = SUBSTRING(CONVERT(VARCHAR(12), '2020-03-26 08:30:00', 121), 0, 11)               -- SELECT SUBSTRING(CONVERT(VARCHAR(10), GETDATE(), 121), 0, 11)     
...
--  	  --  SET @sqlStr = ' UPDATE MEDIUM_PROD  SET DAY' + @ToDay + ' = ISNULL(B.[일일총수량], 0)  FROM MEDIUM_PROD A INNER JOIN  #TEMP_TABLE51 B ON A.[사이즈] = B.[사이즈] AND A.[RouteCode] = B.[공정코드] AND 기준년월 = ''' + @BaseYm + ''' AND A.RouteCode = ''' + @RouteCode + '''  AND A.CompanyCode = ''VNT''  AND A.LineCode = B.라인코드 '                               -- 2020.02.04 수정
	      SET @sqlStr = ' UPDATE MEDIUM_PROD  SET DAY' + '26' + ' = ISNULL(B.[일일총수량], 0)  FROM MEDIUM_PROD A INNER JOIN  #TEMP_TABLE51 B ON A.[사이즈] = B.[사이즈] AND A.[RouteCode] = B.[공정코드] AND 기준년월 = ''202004'' AND A.RouteCode = ''V-25''  AND A.CompanyCode = B.[CompanyCode]  AND A.LineCode = B.라인코드 '       -- 2020.03.26 수정
