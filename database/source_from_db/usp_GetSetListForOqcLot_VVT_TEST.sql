...
-- EXEC [usp_GetSetListForOqcLot_VVT_TEST] @pProcessUserID='kilee',@pProcessLanguage='Korean',@pCompanyCode='VVT',@pWorkCenterCode='VVT_F2',@pLineCode='',@pBarCode = 'VVOO173R850602'       
-- =============================================

CREATE PROCEDURE [dbo].[usp_GetSetListForOqcLot_VVT_TEST]
...
BEGIN
	...
	(
		...
		FROM
				...
				INNER JOIN STB_ProductionOrderRouting POR WITH(NOLOCK)		ON POR.PONo = SI.PONo AND POR.RouteCode IN ('E-22','V-22', 'MV-01','V-22_BG')
		...
		WHERE 1=1
		     ...
		     -- AND SI.Barcode LIKE @Barcode 				                                                                                                                                      -- 원본백업    
				AND  (SI.Barcode = 'VVOO173R850602' OR SI.Barcode =@NewBarcode)            -- 2020.04.17 추가 (기종변경 바코드추가)	
