...
-- ==================================================================
-- EXEC usp_Medium_Daily_Input
---[실행문]    EXEC usp_Medium_Daily_Input  '2019-09-19', '' 

CREATE PROC [dbo].[usp_Medium_Daily_Input_jackaroe] 		
				@pDate DATE,
...
-- [수동으로 실행시 -> 지우지말것]
	--DECLARE @FromDt             VARCHAR(19) =  '2019-08-31 08:30:00'            -- 전일                    
	--DECLARE @ToDt                VARCHAR(19) =  '2019-09-01 08:30:00'            -- 금일
	--DECLARE @ToDay              VARCHAR(02) =   '31'                                -- 전일자로 넣음
	...
	DECLARE @BaseYm    VARCHAR(6) = LEFT(@FromDt, 4) + SUBSTRING(@FromDt, 6, 2)                                                                             --  SELECT LEFT('2019-09-22 08:40:00', 4) + SUBSTRING('2019-09-22 08:40:00', 6, 2)   
...
--DECLARE @ToDay VARCHAR(10) = '08'
   ...
   --SET @sqlStr = ' UPDATE MEDIUM_PROD  SET DAY' + @ToDay + ' = ISNULL(B.[일일총수량], 0)  FROM MEDIUM_PROD A INNER JOIN  #TEMP_TABLE51 B ON A.[사이즈] = B.[사이즈]                                            AND 기준년월 = ''201909''   AND CompanyCode = ''VNT''    '
    -- SET @sqlStr = ' UPDATE MEDIUM_PROD  SET DAY' + @ToDay + ' = ISNULL(B.[일일총수량], 0)  FROM MEDIUM_PROD A INNER JOIN  #TEMP_TABLE51 B ON A.[사이즈] = B.[사이즈] AND A.[RouteCode] = B.[공정코드] AND 기준년월 = ''201909''   AND CompanyCode = ''VNT''    '
	 SET @sqlStr = '   UPDATE MEDIUM_PROD  SET DAY' + @ToDay + ' = ISNULL(B.[일일총수량], 0)  FROM MEDIUM_PROD A INNER JOIN  #TEMP_TABLE51 B ON A.[사이즈] = B.[사이즈] AND A.[RouteCode] = B.[공정코드] AND 기준년월 = ''' + @BaseYm + ''' AND A.RouteCode = ''' + @RouteCode + '''  AND A.CompanyCode = ''VNT''    '
