...
-- Modified:
-- =============================================
CREATE PROCEDURE [dbo].[usp_DoUpdateMaterialQcInfo_Hold]
    ...
    BEGIN TRY
        ...
        WHILE 1 = 1 BEGIN
			...
			SELECT
					...
					@VendorLotNo = isnull(MQI.VendorLotNo,'')
			FROM
			...
			IF @DecisionResult NOT IN ('None', 'Hold', 'Pass', 'CComplete') 
			
			BEGIN    -- 이미정요청
				EXEC usp_RaiseLocalizedError @pProcessLanguage, '[검사대기]와 [검사보류], [특성완료], [합격]인 상태만 변경이 가능합니다.'
				RETURN
			END
            ...
            UPDATE STB_MaterialQcInfo
				SET
				  ...
				  DecisionResult = CASE WHEN @DecisionResult = 'None' THEN 'Hold'   
				                                 WHEN @DecisionResult = 'Pass' THEN 'Hold' ELSE 'None'  END,
