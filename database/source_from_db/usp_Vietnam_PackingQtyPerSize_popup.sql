...
---------------------------------------
	if(@Barcode like 'VE%' OR @Barcode like 'SP%' or @Barcode like 'RW%') --Mr.Duy  get Qty after VE06 by Hanam factory with MarkingCode
	begin
	...
	select  (CL.Qty-DefectQty) as Seq,@ProdSize as ProdSize ,  (CL.Qty-DefectQty) as  PackingQty	
		...
		union all */
		select 100 as Seq ,@ProdSize as ProdSize , 100 as PackingQty union all
		select 50 as Seq ,@ProdSize as ProdSize , 50 as PackingQty union all
...
END
	...
	-------------------------------------------------------------------------------------------
	select * from STB_PackingQtyPerSize