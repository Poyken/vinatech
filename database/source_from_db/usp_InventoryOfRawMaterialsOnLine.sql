...
-- Description:	Tính tồn nvl trên từng line 
-- GCMDPT-382 mã này của VVC-1 đang thấy đúng
-- ============================================= exec usp_InventoryOfRawMaterialsOnLine '2024-07-02','VVT_F1'
CREATE PROCEDURE [dbo].[usp_InventoryOfRawMaterialsOnLine]
	@pDate date = NULL,
...
*/
					...
					table1  as ( -- Lấy danh sách đã xuất nvl ra ngoài sx 
							...
							,isnull(MDLI.StockQty,PRO.ExportSlitingLength) as CurrentQty

							 from STB_MaterialWarehouseInOutHist PRO
							left join STB_MaterialLotInfo MLI on PRO.LotID = MLI.LotID
							left join STB_MaterialDocLotInfo MDLI on PRO.LotID = MLI.LotID
					 ...
					 , BomCTE AS --Tính toán số lượng nvl đã sử dụng
						 ...
						 select InputLineCode,MaterialName,BomVersion,ChildMaterialCode,BomUnit,MaterialUnit,UsedQty,ParentBomHeaderUnitUsedQty,SUM(ParentBomHeaderUnitTotalQty) as ParentBomHeaderUnitTotalQty,
						   ...
						   totalQty as ( -- tổng số lượng nvl đã dùng cho hàng theo nhiều điều kiện group by
							...
							SELECT k.InputLineCode,k.MaterialName,k.ChildMaterialCode , isnull(max(tdk.InventoryNvl),0) as InventoryNvl ,  isnull(max(Tongsoluongxuat),0) as TongsoluongxuatTrongThang, 
						   isnull(max(Nhaplaitusx),0) as NhaplaitusxtrongThang,isnull(max(ip.PickingAssignQty),0) as NhaplaitusxtrongThang1, isnull(max(k.TotalNvlTToQty),0) as TotalNvlTToQty ,isnull(max(bp.Weights),0) as baophe,
						   (COALESCE(max(tdk.InventoryNvl),0) + COALESCE(max(Tongsoluongxuat),0) - COALESCE(max(Nhaplaitusx),0)- COALESCE(max(ip.PickingAssignQty),0) -COALESCE(max(k.TotalNvlTToQty),0) - COALESCE(max(bp.Weights),0))					   
						    as tontrenline,
						  ...
						  /*
						   insert into Stb_InventoryUpLine(InputLineCode,MaterialName,MaterialCode,BomUnit,UsedQty,InventoryNvl, Months, Years)
						   SELECT k.InputLineCode,MaterialName,k.ChildMaterialCode , k.BomUnit,k.UsedQty,
						   (COALESCE(max(tdk.Qty),0) + COALESCE(max(Tongsoluongxuat),0) - COALESCE(max(Nhaplaitusx),0) -COALESCE(max(k.TotalNvlTToQty),0) - COALESCE(max(bp.Weights),0)) as tontrenline
						   ,@month,@year
						   FROM totalQty k left outer join table3 tb3
						   on tb3.LineCode = k.InputLineCode and k.ChildMaterialCode = tb3.MaterialCode
						   left outer join tondauky tdk on tdk.Codeline = k.InputLineCode and k.ChildMaterialCode = tdk.Codename
						   left outer join baophenvl bp on k.InputLineCode = bp.LINENAME and k.ChildMaterialCode = bp.MaLotNguyenLieu
						   where
						   1=1
						   and InputLineCode='VVC-01' 
						   --and ChildMaterialCode='GBAKAC-039' 
						   group by k.InputLineCode,MaterialName,k.ChildMaterialCode,k.UsedQty,   k.BomUnit
						   order by ChildMaterialCode desc

						   select * from Stb_InventoryUpLine  where Months = @month and Years = @year  and  InputLineCode='VVC-01'  --and ChildMaterialCode='SRFYO85A'
						   */
						  ...
						  -- đúng với nvl này đang tính
						/*
						   SELECT k.InputLineCode,MaterialName,k.ChildMaterialCode , (COALESCE(max(tdk.Qty),0) + COALESCE(max(Tongsoluongxuat),0) -COALESCE(max(Nhaplaitusx),0) -COALESCE(max(k.TotalNvlTToQty),0)) as tontrenline FROM totalQty k left outer join table3 tb3
						   on tb3.LineCode = k.InputLineCode and k.ChildMaterialCode = tb3.MaterialCode
						   left join tondauky tdk on tdk.Codeline = k.InputLineCode and k.ChildMaterialCode = tdk.Codename
						   where
						   1=1
						   and InputLineCode='VVC-01' 
						   --and ChildMaterialCode='GBAKAC-039' 
						   group by k.InputLineCode,MaterialName,k.ChildMaterialCode
						   order by ChildMaterialCode desc
						   */
						   --245760
			/*	end
