...
-- Modified:
-- =============================================
CREATE PROCEDURE [dbo].[usp_DivideStampsFoxconn_iud]
	@pProcessUserID VARCHAR(20),
	@pProcessLanguage VARCHAR(20),
    @pProcessViewName VARCHAR(50),
	@pXml NVARCHAR(MAX) = null,
	@pTypeAction VARCHAR(20) = null
AS
BEGIN
	SET NOCOUNT ON;
	...
	DECLARE @TypeAction VARCHAR(20) =@pTypeAction
	...
	EXEC sp_xml_preparedocument @iDoc OUTPUT, @pXml
			...
			if(@TypeAction ='DELETE')
				begin
						Delete from STB_VN_STAMP_FOXCONN where id = @id or ParentPackingID = @id
						return
				end
			   ...
			   SELECT
								...
								FROM  STB_VN_STAMP_FOXCONN		where id = @id
									 
							         
	 
		 -- Tạo thùng con (2 thùng con)
					DECLARE @QtyPerChild NUMERIC(20, 5) = @Qty / 2

			
					-- Tạo thùng nhỏ (12 thùng nhỏ)
					DECLARE @QtyPerSmallBox NUMERIC(20, 5) = @Qty / 12
					DECLARE @i INT
					DECLARE @SmallPackingID VARCHAR(50)

					-- Cập nhật 12 thùng nhỏ
					SET @i = 1
					WHILE @i <= 14
							BEGIN
								if(@i <=2)
										...
										INSERT INTO STB_VN_STAMP_FOXCONN
											(CPN, Qty, MaterialCode, GRDate, LotNo, PackingID, COO, CreateDateTime, CreateUserID, ParentPackingID,TypeBox)
										VALUES
											(@CPN, @QtyPerChild, @MaterialCode, @GRDate, @LotNo, @SmallPackingID, @COO, GETDATE(), @ProcessUserID, @id, 1) 
											--(@CPN, @QtyPerChild, @MaterialCode, @GRDate, @LotNo, @PackingID, @COO, GETDATE(), @ProcessUserID, @id,1)
								...
								ELSE
										 ...
										 BEGIN
												-- Tạo PackingID cho thùng nhỏ theo format thùng cha + "_chỉ số"
												-- Nếu chỉ số nhỏ hơn 10, thêm số 0 vào phía trước
												SET @SmallPackingID = @PackingID + '_' + RIGHT('00' + CAST(@i AS VARCHAR(10)), 2)

												-- Chèn thông tin thùng nhỏ vào bảng
												INSERT INTO STB_VN_STAMP_FOXCONN
													(CPN, Qty, MaterialCode, GRDate, LotNo, PackingID, COO, CreateDateTime, CreateUserID, ParentPackingID,TypeBox)
												VALUES
													(@CPN, @QtyPerSmallBox, @MaterialCode, @GRDate, @LotNo, @SmallPackingID, @COO,GETDATE(), @ProcessUserID, @id,2)
...
--select * from STB_VN_STAMP_FOXCONN order by createDateTime DESC