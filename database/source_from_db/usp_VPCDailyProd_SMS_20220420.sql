...
-- ================================================================================
Create PROCEDURE [dbo].[usp_VPCDailyProd_SMS_20220420]
 ...
 -- SELECT @msg = @msg + Z.LineName + ' (' + Convert(Varchar(20), PlanQty) + ' / ' + Convert(Varchar(20), ProdQty) + ' / ' + Convert(Varchar(20), ProdRate) + '% ' + ')<br>'
  ...
  FROM  (  
  		 ...
  		 --1. 권취누계추가   							
				...
				, Round(Convert(NUMERIC(20,2), (Sum(A2.ProdQty) / 178000.00 * 100)), 1) AS ProdRate                  -- 고정값 세팅 (소숫점둘째자리표현)
				FROM (
				SELECT  B.LineCode
     ...
     FROM  (  
         ...
         --   Select  '[권취 실적]' + '<br>' + @msg + '<br>' 
			...
			SET @LineMessage = '금일 VPC 생산실적 공유드립니다.' + '<br>'
						...
						--+  '※ 라인별 실적은 조립(커링)의 실적입니다.  금일 설비기준으로 라인 재설정하였습니다.' + '<br>'      -- 전달사항 부분                    
