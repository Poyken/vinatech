...
-- Modified :
-- =============================================
CREATE PROCEDURE [dbo].[usp_DoCreateProductionOrderBatch_Redroping]
...
BEGIN
	...
	BEGIN
		...
		exec usp_DoCreateProductionOrder_ReDroping @pProcessUserID
                                        ,@pProcessLanguage
										,@PlanYearMonth
										,@PlanStartDate
										,@PlanEndDate
		...
		update STB_ReDropping set isStatus = 0 where DefectSummaryNo = @DefectSummaryNo 
		FETCH NEXT FROM cur INTO @TargetMaterialCode, @OrderRate
	END
