...
-- =============================================
CREATE PROCEDURE [dbo].[usp_DoFixMaterialLabelManual]
...
BEGIN
	...
	FROM
			...
			LEFT OUTER JOIN STB_MaterialType MT
	...
	--RETURN
	IF @AllowQty < @CurrentQty + @MakeQty BEGIN
	...
	DECLARE LabelInfoData CURSOR FOR
		SELECT LotNo, LotQty
		  FROM STB_MaterialBarcodeHist
		 WHERE MaterialDocDetailNo = @MaterialDocDetailNo
	...
	WHILE 1 = 1 BEGIN
		FETCH NEXT FROM LabelInfoData INTO
							@LotNo,
							@LotQty
