...
-- Description:	Thêm phần chọn chữ marking khi gộp box cho nhà máy hà nam
-- =============================================
CREATE PROCEDURE [dbo].[usp_Vietnam_DoProcessProdPacking_VVT_F3]
...
BEGIN
	...
	*/
	...
	DECLARE @MarkingCode VARCHAR(50) --= @pMarkingCode
    ...
    BEGIN TRY
		...
		if (@pMergeQty is null) begin
			...
			if (@pMarkingCode is null or @pMarkingCode ='') begin
				select @data1 = N'Bạn cần chọn Marking trong danh sách !' --bên cạnh Mã BARCODE ở chỗ V Tìm kiếm nhé!'
				raiserror(@data1,16,1)
				return
		end	
