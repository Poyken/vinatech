...
-- Browsable : true
-- Group : 자재관리 > [F750]자재재고실사 >  실사반영처리 버튼 처리 (1)
-- Modified:  2020.06.16 PackingID 파라미터 추가 (kilee)
-- Description:	실사정보를 재고에 반영합니다.
-- =============================================

CREATE PROCEDURE [dbo].[usp_DoApplyStocktakingToStock]
...
BEGIN
	...
	UPDATE STB_StocktakingPlanResult
	    SET	StocktakingQty = 0
	WHERE 1=1
			AND StocktakingDocNo = @StocktackingDocNo 
			AND	ISNULL(MaterialLotNo, '') <> '' 
			AND	IsStocktaking =  1 
			AND	ISNULL(IsApplied,0) = 0 
			AND	StocktakingQty IS NULL
...
-- SELECT * FROM STB_StocktakingPlanResult ORDER BY CreateDateTime desc
	...
	BEGIN
			...
			WHILE @ROW <= @COUNT           -- 두줄이상 등록하는 경우는 While문
			...
			BEGIN                   
				...
				EXEC usp_DoMakeMaterialDocDetailLotForStocktaking          -- STB_MaterialDocLotInfo 테이블 Insert되는부분
								@pProcessLanguage = @ProcessLanguage,
								@pProcessUserID = @ProcessUserID,
								...
								@pMaterialLocationCode = @MaterialLocationCode,
								@pRequestQty = @GRQty,
								@pPackingID = @PackingID                                          -- 2020.06.16 추가
    ...
    INNER JOIN (
		...
		FROM STB_StocktakingPlanResult SPR
		LEFT JOIN STB_MaterialLocation ML ON ML.MaterialLocationCode = SPR.StocktakingMaterialLocationCode
		WHERE StocktakingDocNo = @StocktackingDocNo
		AND IsStocktaking = 1
		AND ISNULL(SPR.IsApplied, 0) = 0
	) SPR
	ON (MLI.LotID = SPR.LotID AND MLI.MaterialLocationCode = SPR.MaterialLocationCode);
