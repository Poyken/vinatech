...
-- Description:	SalesOrder 조회합니다
		...
		--오더유형
-- =============================================
CREATE PROCEDURE [dbo].[usp_SalesOrder_get]
...
BEGIN
	...
	SELECT
			...
			SO.ChangeDateTime,
			SO.ChangeUserID,
			SO.FixOrderDate,       -- 2019.01.11 추가 (kilee)
			SO.IsFlxedCheck
	FROM
	...
	WHERE
			...
			(SO.OrderDate IS NULL OR (SO.OrderDate BETWEEN @FromOrderDate AND @ToOrderDate) AND (SO.RequestDeliveryDate IS NULL OR (SO.RequestDeliveryDate BETWEEN @FromDeliveryDate AND @ToDeliveryDate))) AND
			((@OrderType = '*') OR (SO.OrderType = @OrderType)) AND
			((@IncludeCancel = 'Y') OR (SO.IsCancel = 0))
END
