...
create PROCEDURE [dbo].[usp_Get_VVT_ModuleProd_Bad_Status_Dashboard]
	...
	,
	 ...
	 STB_Defect_VVT as (
	    SELECT --ROW_NUMBER() OVER(ORDER BY X.DefectQty DESC) AS ROWNUM
			...
			,  CASE WHEN di.DefectEnglishName IS NOT NULL THEN di.DefectEnglishName ELSE di.BasicDefectName END  AS 불량명 
			,  X.DefectQty AS 불량수량		
