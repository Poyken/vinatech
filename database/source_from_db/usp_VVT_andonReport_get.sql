--usp_VVT_andonReport_get '','','VVC-01'

CREATE  PROCEDURE [dbo].[usp_VVT_andonReport_get]
...
;with linedat as (
 ...
 ,case when isnull(DATEDIFF(MINUTE, beginFix2, isnull(finishFix2,getdate1) ),0)=0 
  ...
  from combinedat
  where (case when isnull(DATEDIFF(MINUTE, beginOccur, isnull(finishTime2,getdate1)  ),0)<=0 then 1 else DATEDIFF(MINUTE, beginOccur, isnull(finishTime2,getdate1)) end)<24*60*3
