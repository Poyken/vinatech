...
BEGIN
	...
	SELECT
		 ...
		 case when  isnull(MDLI.LotAttr10,'')='' then  ''
			  ...
			  when @companycode='VVT' and @workcentercode IN ('VVT_F4') and MDLI.MaterialCode NOT IN ('DOW01-001')  and ISDATE(MDLI.Lotattr10)=1  then CONVERT(VARCHAR(10), DATEADD(DAY, 0, CONVERT(VARCHAR(10), DATEADD(MM,  MM.MMExtInt01, MDLI.Lotattr10), 121)), 121)
