CREATE  PROCEDURE [dbo].[usp_VN_GraphichLineStage] --exec usp_VN_GraphichLineStage 'VVT','','','VVC-03'
...
BEGIN
 CREATE TABLE #Tbl
	...
	INSERT INTO #Tbl(LineName,RouteCode,RouteName,MaterialCode,Total,DefectQty)
	...
	select 
		  ...
		  ,RouteName,
		   MaterialCode,
		   SUM(total) OVER (PARTITION BY LineName ORDER BY LineName) AS totals,
		   SUM(DefectQty) OVER (PARTITION BY LineName ORDER BY LineName) AS DefectQtys
		  from deptrai
		 --group by  LineName,RouteCode,	RouteName, MaterialCode
		...
		--		RouteName,
		--		MaterialCode,
		--		SUM(total) OVER (PARTITION BY LineName ORDER BY LineName) AS total,
		--		SUM(DefectQty) OVER (PARTITION BY LineName ORDER BY LineName) AS DefectQty
		--FROM 
		--		deptrai
