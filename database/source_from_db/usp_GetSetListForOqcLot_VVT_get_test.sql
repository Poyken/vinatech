...
-- Description:	For test search Separated Lot function same C512
-- =============================================
CREATE PROCEDURE [dbo].[usp_GetSetListForOqcLot_VVT_get_test]
...
BEGIN
	...
	-- DinhManh update 2025-04-24: can search separated Lot 
	DECLARE @sepaBarcode VARCHAR(20) = NULL
	DECLARE @mergeBarcode VARCHAR(20) = NULL
	SELECT @sepaBarcode = lotid, @mergeBarcode = mergeid FROM VVT_OQC_REFER where lotid = @BarCode and (CreatedLot IS NULL or CreatedLot = '') AND isSeparated = 1

	 --raiserror (@NewBarcode, 16,1)
	;WITH SetList AS
	(
		SELECT
				DISTINCT
		...
		WHERE 1=1
		     ...
		     -- AND SI.Barcode LIKE @Barcode 				                                                                                                                                      -- 원본백업    
				AND  (SI.Barcode = @Barcode OR SI.Barcode =@NewBarcode OR SI.Barcode = @mergeBarcode)            -- 2020.04.17 추가 (기종변경 바코드추가)	
