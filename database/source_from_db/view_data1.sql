CREATE PROCEDURE view_data1
(

    @pFromDate DATETIME = NULL,
    @pToDate DATETIME = NULL,
    @pWorkCenterCode VARCHAR(30) = NULL  -- 👈 Thêm tham số bị thiếu
)
AS
BEGIN
    ;WITH ViewBarcode AS (
        ...
        WHERE b.CompanyCode = 'VVT'
            AND b.WorkCenterCode = @pWorkCenterCode
    ...
    RawView0 AS (
        SELECT 
            b.CreateUserID, c.Barcode, b.RouteCode, b.RouteCode AS FindRouteCode, c.ControlNo,
            c.MaterialCode, InputLineCode, b.MachineCode, b.WorkerCode, SIExtText07, SIExtInt01,
            MAX(b.ProdQty) AS ProdQty,
            CASE 
                WHEN a.DefectCode NOT IN (SELECT defectcode FROM [dbo].[fn_VVT_QCPARTCODE]())
                    AND a.DefectCode NOT LIKE 'V-29_XX1' AND a.DefectCode NOT LIKE 'V-29_XX2'
                    AND a.DefectCode NOT LIKE 'V-29_XX3' AND b.RouteCode NOT LIKE 'V-34_BG'
        ...
        WHERE c.Barcode IN (SELECT Barcode FROM ViewBarcode)
            ...
            AND b.CreateUserID NOT IN ('test_worker','assy_packing','assy_packing2','roh_worker','electrode_worker','dryroom_worker')
        GROUP BY 
            b.CreateUserID, c.Barcode, b.RouteCode, c.ControlNo, c.MaterialCode, InputLineCode,
            b.MachineCode, b.WorkerCode, SIExtText07, SIExtInt01, a.DefectCode, POI.DefectSummaryNoBeforeDroping
    )
    SELECT * FROM RawView0
END