...
-- [프로시저실행문]  usp_GetElectrodeInspectionHistoryForBarcode 'kilee','Korean','ROUTE_ELECTRODE_QUALITY','VJKP1720001E01','','','','','',''
-- =============================================

Create PROCEDURE [dbo].[usp_GetElectrodeInspectionHistoryForBarcode_20200812]
...
BEGIN
	...
	SELECT A.Barcode
			  ...
			  ,A.IsHolding
			  ,A.DisplayIndex
			  ,(CONVERT(NUMERIC(10, 5), A.FirstMeasureValue) 
					  + CONVERT(NUMERIC(10, 5), A.SecondMeasureValue) 
					  + CONVERT(NUMERIC(10, 5), A.ThirdMeasureValue)) / 3 AS ElectrodeSampleWeightAvgValue
			  ,dbo.fnGetElectrodeDensity (A.CommInspDocNo, A.DisplayIndex) AS ElectrodeDensityValue
			  ,A.RollingDensityMin
			  ,A.RollingDensityMax
			  ,A.MaterialCode 
	  FROM (
					SELECT @Barcode    AS Barcode,
							...
							CASE	WHEN VIEW_CIIT.CommInspInputTypeName = 'NUMERIC' THEN CONVERT(VARCHAR, CONVERT(NUMERIC(10,4), CIMH1.NumericMeasure))	ELSE CIMH1.MeasureResult  END AS FirstMeasureValue,           -- 1번째측정값
							CASE	WHEN VIEW_CIIT.CommInspInputTypeName = 'NUMERIC' THEN CONVERT(VARCHAR, CONVERT(NUMERIC(10,4), CIMH2.NumericMeasure))	ELSE CIMH2.MeasureResult  END AS SecondMeasureValue,       -- 2번째측정값
							CASE	WHEN VIEW_CIIT.CommInspInputTypeName = 'NUMERIC' THEN CONVERT(VARCHAR, CONVERT(NUMERIC(10,4), CIMH3.NumericMeasure))	ELSE CIMH3.MeasureResult  END AS ThirdMeasureValue,          -- 2020.01.13 추가
							CASE WHEN VIEW_CIIT.CommInspInputTypeName = 'NUMERIC' THEN CONVERT(VARCHAR, CONVERT(NUMERIC(10,4), CIMH.NumericMeasure))	ELSE CIMH.MeasureResult	   END AS OldLastMeasureValue,
							CIMH.CommInspMeasureNo,
							CIMH.MeasureSeq,
							''                                     AS DefectCode,
