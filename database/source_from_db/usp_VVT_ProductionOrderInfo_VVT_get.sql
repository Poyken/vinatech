...
CREATE PROCEDURE [dbo].[usp_VVT_ProductionOrderInfo_VVT_get]
...
BEGIN
    ...
    DECLARE @ProcessUserID      VARCHAR(20) = @pProcessUserID,
			    ...
			    @FromDate    DATETIME = @pFromDate,                                                                              -- select CONVERT(VARCHAR,'2019-09-26 00:00:00',120)
			    @ToDate        DATETIME = ISNULL(dateadd(day,1,@pToDate),'9999-12-31'),
  ...
  DECLARE @WorkCenterCode VARCHAR(20) = CASE WHEN ISNULL(@pWorkCenterCode,'') = '' THEN '%' ELSE @pWorkCenterCode END
	...
	WHERE 1=1
			--AND @FromDate <= POI.PlanYearMonth 
			--AND POI.PlanYearMonth <= @ToDate 
			...
			--AND  POI.IsFix ='1'
			and POI.CreateDateTime between @FromDate and @ToDate
