...
-- =============================================   select isnumeric('28.30700')
-- Author:	    mrtung
-- Create date: 2023-03-23
-- =====================================================================================================
--- ==================     usp_VVT_InventoryonSTAGE_get '2023-06-01','2023-06-10'
CREATE PROCEDURE [dbo].[usp_VVT_InventoryonSTAGE_get] 
...
--declare @tung1 varchar(100)=convert(varchar(10),@pMonth,120)
...
RawView as (
		...
		max(b.ProdQty) as ProdQty, sum(isnull(a.DefectQty,0.0)) as DefectQty,  sum(isnull(a.RepairQty,0.0)) as RepairQty ,max(b.ProdDateTime) as ProdDateTime, max(b.CreateDateTime) as CreateDateTime
...
DRI as(
		select  
		 ...
		 DATEPART(DAY, ProdDateTime)  as FindDateTime, 
		  ...
		  max(isnull(RV.ProdQty,0.0)) as ProdQty,
		  sum(isnull(RV.DefectQty,0.0) - isnull(RV.RepairQty,0.0) ) as DefectQty--, 
...
, materialOut as (
	 select 
	--''사업장, '' MaterialCode, mdi.materialdoctypecode MaterialName1, '' RouteCode, '' RouteName,
	--0 ProdQty,0 GoodQty,0 DefectQty,
	--MWIOH.SourceMaterialWarehouseCode matcode, 
	--MWIOH.TargetMaterialWarehouseCode, 
	--mm.MaterialName, 
	--'' ProductGroupCode,0 usedqty,
	MLI.MaterialCode ChildMaterialCode,
	--'' routeexpress,'' routemodule, 
	--MM.MaterialUnit, 
	--'' Ma_Lieu_DAU_KY,''  Ma_BTP_DAU_KY,''  Donvi_DAU_KY,0  Ton_DAU_KY,
	   ...
	   left join STB_MaterialDocInfo mdi WITH(NOLOCK) ON  MDLI.MaterialDocNo = mdi.MaterialDocNo
		-- LEFT OUTER JOIN STB_MaterialMaster MM with(nolock)   ON MM.MaterialCode = MDLI.MaterialCode   
		 where (
		  ...
		  and isnull(MLI.MaterialCode,'')<>''
		  and MWIOH.CreateDateTime between @FromDate and @ToDate
		 group by mdi.materialdoctypecode,MWIOH.SourceMaterialWarehouseCode,MWIOH.TargetMaterialWarehouseCode,MLI.MaterialCode
		 --,MM.MaterialUnit,mm.MaterialName 
)
, materialOut1 as (
	 select 
	--''사업장, '' MaterialCode,  max(MaterialName1)MaterialName1, '' RouteCode, '' RouteName,
	--0 ProdQty,0 GoodQty,0 DefectQty,
	-- max(matcode) matcode, 
	--max(TargetMaterialWarehouseCode) as TargetMaterialWarehouseCode, 
	--max(MaterialName) MaterialName, 
	--'' ProductGroupCode,0 usedqty, 
	ChildMaterialCode,
	--'' routeexpress,'' routemodule, 
	--max(MaterialUnit) MaterialUnit, 
	--'' Ma_Lieu_DAU_KY,''  Ma_BTP_DAU_KY,''  Donvi_DAU_KY,0  Ton_DAU_KY,
max(Xuat_trong_ky)  Import_in_stage,
max(Tra_lai_trong_ky)   Return_in_stage--,
	--0  Tong_TRONG_KY,0  Good_TRONG_KY,0  Defect_TRONG_KY,0  Ton_CUOI_KY
	  FROM materialOut 
...
, lastqty as (
select /*isnull(deptrai.MaterialCo,bom1.MaterialCode) MaterialCot,deptrai.* ,bom1.**/
ChildMaterialCode
,bom1.usedqty
,replace(MaterialUnit,'KP','EA') as MaterialUnit  --isnull(b1.MaterialUnit,mm.MaterialUnit)MaterialUnit
--,''   Ma_Lieu_DAU_KY
--,''   Ma_BTP_DAU_KY
--,''  Donvi_DAU_KY
--,0 as Ton_DAU_KY
--,0 Xuat_trong_ky
--,0 Tra_lai_trong_ky
,convert(float,bom1.usedqty)/1000000.0*convert(float,ProdQty)     as Tong_TRONG_KY 
,convert(float,bom1.usedqty)/1000000.0*convert(float,GoodQty)     as Good_TRONG_KY 
,convert(float,bom1.usedqty)/1000000.0*convert(float,DefectQty)   as Defect_TRONG_KY 
--,0 as Ton_CUOI_KY 
from deptrai 
...
ChildMaterialCode,
MaterialUnit,
sum(convert(float,isnull(Tong_TRONG_KY,0.0)))   total_in_stage,
sum(convert(float,isnull(Good_TRONG_KY,0.0)))   good_in_stage,
sum(convert(float,isnull(Defect_TRONG_KY,0.0))) defect_in_stage,
sum(convert(float,isnull(Ton_DAU_KY_723,0.0))) *convert(float,max(usedqty))/1000000.0 as material_Beginning,
sum(convert(float,isnull(Ton_Cuoi_KY_723,0.0)))*convert(float,max(usedqty))/1000000.0 as material_Ending,
sum(convert(float,isnull(NG_Trong_KY_726,0.0)))*convert(float,max(usedqty))/1000000.0 as material_defect_in_stage--,
--usedqty/1000000 as BOM_KY_THUAT--,

from lastqty


  full outer join (select CODENAME,sum(QTY)Ton_DAU_KY_723,max((UNIT))UNIT_DAU_KY_723 
					...
					where CreateDateTime between convert(varchar(7),dateadd(month,-1,convert(date,@FromDate,120)),120)+'-25'
						 ...
						 and qty is not null
						 --and ISNUMERIC(QTY)=1
						 --and (TYPES='NVL' or isnull(types,'')='' and substring(TYPEINPUT,1,2) not in ('TP','BT'))
					group by CODENAME
					) 
					vic on ChildMaterialCode=vic.CODENAME --723

  full outer join (select CODENAME Ma_Cuoiky_723,sum(QTY)Ton_Cuoi_KY_723,max((UNIT)) UNIT_CUOI_KY_723
					from STB_VN_ITEM_CHECK  with(nolock) 
					...
					vic1plus on ChildMaterialCode=vic.CODENAME

  full outer join (select (case when CODEPRODUTION is not null and CODEPRODUTION<>''  then CODEPRODUTION else CODENAME end)CODENAME1,
					...
					--  isnull(PRODUCTIONNAME,INPUT)PRODUCTIONNAME,
					 sum(qty)NG_TRong_KY_726,max(UNIT)UNIT_726 
					...
					vsa on ChildMaterialCode=vsa.CODENAME1
					 
group by ChildMaterialCode,MaterialUnit
)
select isnull(last1.ChildMaterialCode,materialOut1.ChildMaterialCode) as Ma_Lieu,*,'caculating...' Ending_ByCaculate
from last1
full outer join materialOut1 on last1.ChildMaterialCode=materialOut1.ChildMaterialCode

-- full outer join ( 
--		select MaLotCapThu, MaLotNguyenLieu as MaLieu_NG_598 
...
--		)  vpe on    last1.ChildMaterialCode=vpe.MaLieu_NG_598  or materialOut1.ChildMaterialCode=vpe.MaLieu_NG_598
		--select top 10 * from STB_VN_PRODUCTION_ERROR where CreateDateTime>getdate()-10
...
--		 --ri.RouteName, 		 
--		 --MM2.MaterialName,	 
--		 RV.ProdDateTime --	, li.LineName, pwi.WorkerName,mm.MachineName		 
--		--order by RV.Barcode,RV.RouteCode 
--)
--,
--deptrai as (
...
--, b11 as (
--	select  b1.MaterialCode,	max(b1.BomVersion)BomVersion,ChildMaterialCode
--	--, mm.ProductGroupCode
--	from STB_BomDetail b1  with(nolock) 
--	join STB_BomHeader bh  with(nolock) on b1.MaterialCode=bh.MaterialCode and b1.BomVersion=bh.BomVersion
--	--join STB_MaterialMaster mm  with(nolock) on b1.ChildMaterialCode=mm.MaterialCode
...
--	b1.BomVersion='99' 
--	 --(b1.BomVersion='99' and len(rtrim(ltrim(b1.BomVersion)))='99' and b1.BomVersion='99' )
--	 group by b1.MaterialCode,	ChildMaterialCode
--	 --, mm.ProductGroupCode
--	 --,mm.MaterialName
--	 --having  mm.ProductGroupCode in ('SLITTING-ROLL','COATING-ROLL','JELLY-ROLL','BINDER','SLEEVE','SURFACTANT','FOIL','A.C','CARBON','HC-EDLC','HC-VPC') 
--)
...
--	--MWIOH.TargetMaterialWarehouseCode, 
--	--mm.MaterialName, 
--	'' ProductGroupCode,0 usedqty,MLI.MaterialCode ChildMaterialCode,
--	'' routeexpress,'' routemodule, 
--	--MM.MaterialUnit, 
--	'' Ma_Lieu_DAU_KY,''  Ma_BTP_DAU_KY,''  Donvi_DAU_KY,0  Ton_DAU_KY,
--case when MWIOH.TargetMaterialWarehouseCode='ROUTE_VN_WH' then	sum(MLI.CurrentQty) end  Xuat_trong_ky,
...
--	--max(TargetMaterialWarehouseCode) as TargetMaterialWarehouseCode, 
--	--max(MaterialName) MaterialName, 
--	'' ProductGroupCode,0 usedqty, ChildMaterialCode,
--	'' routeexpress,'' routemodule, 
--	--max(MaterialUnit) MaterialUnit, 
--	'' Ma_Lieu_DAU_KY,''  Ma_BTP_DAU_KY,''  Donvi_DAU_KY,0  Ton_DAU_KY,
...
--, lastqty as (
--select deptrai.* ,bom1.*
----,MaterialUnit  --isnull(b1.MaterialUnit,mm.MaterialUnit)MaterialUnit
--,''   Ma_Lieu_DAU_KY
--,''   Ma_BTP_DAU_KY
--,''  Donvi_DAU_KY
--,0 as Ton_DAU_KY
...
--Modelcode Modelcode_DAU_KY_723, CodeBtp Ma_BTP_DAU_KY_723, RouteCode RouteCode_DAU_KY_723, 
...
--FROM (
--select tb1.*,vic.CODENAME,vic.QTY as FirstQty,Ton_Cuoi_KY_723,UNIT_CUOI_KY_723,vic.UNIT_DAU_KY_723 ,vsa.CODENAME AS Ma_HAI_QUAN_726,
...
--  )tb2 
--  )tb1 
--  full outer join (select CODENAME,sum(QTY)QTY,max((UNIT))UNIT_DAU_KY_723 
--					from STB_VN_ITEM_CHECK  with(nolock) 
--					where CreateDateTime between convert(varchar(7),dateadd(month,-1,convert(date,@FromDate,120)),120)+'-25'
--										 and     substring(@FromDate,1,7)+'-05'
--						 and qty is not null
...
--  full outer join (select (case when CODEPRODUTION is not null and CODEPRODUTION<>''  then CODEPRODUTION else CODENAME end)CODENAME,
--					 (case when PRODUCTIONNAME is not null and PRODUCTIONNAME<>''  then PRODUCTIONNAME else INPUT end)PRODUCTIONNAME,
--					--  isnull(PRODUCTIONNAME,INPUT)PRODUCTIONNAME,
...
-- ) TBNEW
--)TBStage on isnull(lastqty.materialco,lastqty.materialcode)=TBStage.Modelcode_DAU_KY_723 and lastqty.RouteCode=TBStage.RouteCode_DAU_KY_723
--       or isnull(lastqty.materialco,lastqty.materialcode)=TBStage.Modelcode_DAU_KY_723 and lastqty.routeexpress=TBStage.RouteCode_DAU_KY_723
--	   or lastqty.RouteCode is null and lastqty.routeexpress is null and TBStage.RouteCode_DAU_KY_723 is null 
--	   and (isnull(lastqty.materialco,lastqty.materialcode)=TBStage.Modelcode_DAU_KY_723 )   --or lastqty.materialcode=TBStage.Modelcode_DAU_KY_723)

--)
--, lastqty1 as (
--	select 
--	--''사업장, ''MaterialCode,  ''MaterialName1 , '' RouteCode,   ''RouteName,
--	sum( ProdQty)ProdQty,
--	sum(GoodQty) GoodQty,
--	sum(DefectQty) DefectQty, 
--	--''matcode, 	''TargetMaterialWarehouseCode,
--	'' MaterialName,   '' ProductGroupCode,
...
--case when mo.GoodQty<>0 then mo.GoodQty else lq.GoodQty end GoodQty,
...
--case when mo.ChildMaterialCode<>'' and mo.ChildMaterialCode is not null then mo.ChildMaterialCode else (
--			case when lq.ChildMaterialCode<>'' and lq.ChildMaterialCode is not null  
--			then lq.ChildMaterialCode else '' end
--) end 	ChildMaterialCode,
...
----(isnull(tcd.QTY,0) + 
--(case when isnull(mo.Xuat_trong_ky,0)<>0 then isnull(mo.Xuat_trong_ky,0) else isnull(lq.Xuat_trong_ky,0) end
---case when isnull(mo.Tong_TRONG_KY,0)<>0 then isnull(mo.Tong_TRONG_KY,0) else isnull(lq.Tong_TRONG_KY,0) end
--)as Ton_CUOI_KY
...
----full outer join (
----			select CODENAME,sum(isnull(qty,0)) qty,unit from 
----			STB_VN_ITEM_CHECK with(nolock) 
----			where CreateDateTime between  
----			CONVERT(VARCHAR(7), DATEADD(MONTH, -1, CONVERT(datetime, @pMonth)), 120)  +'-25' and  
----			CONVERT(VARCHAR(7), DATEADD(MONTH,  0, CONVERT(datetime, @pMonth)), 120)  +'-05' 
----			and (TYPEINPUT like 'NVL%' or [TYPES]='NVL') 
----			and isnull(CODENAME,'')<>''
----			group by  CODENAME,unit
----			) tcd on mo.ChildMaterialCode=tcd.CODENAME or lq.ChildMaterialCode=tcd.CODENAME 

----full outer join (
----			select CODENAME,sum(isnull(qty,0)) qty,unit from 
----			STB_VN_ITEM_CHECK with(nolock) 
----			where CreateDateTime between  
----			CONVERT(VARCHAR(7), DATEADD(MONTH, 0, CONVERT(datetime, @pMonth)), 120)  +'-25' and  
----			CONVERT(VARCHAR(7), DATEADD(MONTH, 1, CONVERT(datetime, @pMonth)), 120)  +'-05' 
----			and (TYPEINPUT like 'NVL%' or [TYPES]='NVL') 
----			and isnull(CODENAME,'')<>''
----			group by  CODENAME,unit
----			) tck on mo.ChildMaterialCode=tck.CODENAME or lq.ChildMaterialCode=tck.CODENAME or tck.CODENAME=tcd.CODENAME

----left outer join stb_materialmaster mm  with(nolock) on  (case when mo.ChildMaterialCode<>'' and mo.ChildMaterialCode is not null then mo.ChildMaterialCode else (
----																	case when lq.ChildMaterialCode<>'' and lq.ChildMaterialCode is not null  
----																	then lq.ChildMaterialCode else tcd.CODENAME end
----														) end) =mm.MaterialCode
