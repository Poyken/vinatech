...
-- =============================================
CREATE PROCEDURE usp_MaterialQcInfo_iud_BendingCutting
  ...
  DECLARE @IQCSampleLotList NVARCHAR(MAX)
    ...
    IF @IsAutoKey = 0 AND @IsLoopIUD = 0 BEGIN
	    ...
	    BEGIN TRY
            ...
            MERGE STB_MaterialQcInfo_BendingCutting AS TargetTable
			USING
				(
					SELECT
							CASE
        ...
        BEGIN TRY
            ...
            WHILE 1 = 1 BEGIN
                ...
                IF @IUD_FLAG = 'INSERT' BEGIN
                    ...
                    IF EXISTS (SELECT 1 FROM STB_MaterialQcInfo_BendingCutting WHERE MaterialQcNo = @MaterialQcNo) BEGIN
						RAISERROR('Duplicate Data : KeyField = %s', 16, 1, @MaterialQcNo)
					END

                    IF @IsAutoKey = 1 BEGIN
                        EXEC usp_DoCreateSerial 'STB_MaterialQcInfo_BendingCutting',@MaterialQcNo OUTPUT
                    END
				...
				BEGIN 
					...
					IF @CompanyCode = 'VVT' BEGIN -- add by Jackaroe on 2023-01-02
						...
						select  @ccount=count(*) 
						from    STB_MaterialQcInfo_BendingCutting  with(nolock) 
