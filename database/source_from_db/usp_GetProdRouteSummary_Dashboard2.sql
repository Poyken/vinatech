...
-- 실행 :    usp_GetProdRouteSummary_DashBoard2  '','','VNT','VNT_F1','','','','2021-01-01','2021-01-05','',0                               -- 582개
-- =====================================================================================
CREATE PROCEDURE [dbo].[usp_GetProdRouteSummary_Dashboard2]
...
BEGIN
	...
	SELECT
		  ...
		  ,  (	SELECT BaseMonth
			...
			--, ( Select   WorkerName From STB_ProdWorkerInfo SP Where SP.WorkerCode = MAX(SPM.WorkerCode )         ) As WorkerName2

			--, CASE WHEN SPH.WorkerCode =  '' THEN  Select SP.WorkerName From STB_ProdWorkerInfo SP Where SP.WorkerCode = SPH.WorkerCode 
			           --ELSE  Select SP.WorkerName From STB_ProdWorkerInfo SP Where SP.WorkerCode = MAX(SPM.WorkerCode  END   As WorkerName
	...
	FROM 	                      STB_ProdRouteSummary    PRS WITH(NOLOCK)
		...
		--LEFT OUTER JOIN STB_ProdRouteHist  SPM ON PRS.CompanyCode = SPM.CompanyCode
																		--AND PRS.WorkCenterCode = SPM.WorkCenterCode
																		--AND PRS.JobDate = SPM.JobDate
																		--AND PRS.ShiftCode = SPM.ShiftCode
																		--AND PRS.PONo = SPM.PONo
																		--AND PRS.MaterialCode = SPM.MaterialCode
																		--AND PRS.LineCode = SPM.LineCode
																		--AND PRS.RouteCode = SPM.RouteCode
																	--	AND SPH.WorkerCode = SPM.WorkerCode
