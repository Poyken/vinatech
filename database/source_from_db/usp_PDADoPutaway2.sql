...
-- Description:	적치 처리를 합니다. (활성탄 예외처리)
-- =============================================
CREATE PROCEDURE [dbo].[usp_PDADoPutaway2]
...
BEGIN
	...
	SELECT
			...
			@IsToRouteMaterial = CASE WHEN ISNULL(ML.IsUseLotID, 0) = 1 THEN 0	ELSE 1		END
	FROM
	...
	BEGIN
			...
			FROM 
					STB_MaterialLotInfo MLI
			WHERE 1=1
				--AND MLI.PackingID = @LotID 
				AND	MLI.LotID <> @LotID


			IF @AnotherPackingID IS NOT NULL 
	...
	IF @IsToRouteMaterial = 1
		BEGIN
				EXEC usp_MaterialLotInfoToRouteMaterialLotInfo2 @pMaterialLotNo = @MaterialLotNo
		END
