...
--  [usp_GetDefectRepairInfoByDefect_PowerBI] '','','','','','','','','2021-05-26','2021-06-25'
-- =============================================================================================
CREATE PROCEDURE [dbo].[usp_GetDefectRepairInfoByDefect_PowerBI]
...
BEGIN
	...
	print convert(varchar(10), @fromdate, 121)
	print convert(varchar(10), @ToDate, 121)
	...
	WHERE 1=1
			...
			AND (@IsIncludeRouteInspDefect = CONVERT(BIT, 1) OR ISNULL(SI.SIExtInt01, 0) <> 1)                                      -- 추가
			 AND DRI.FindJobdate > dateadd(day,-1,getdate())
	GROUP BY
			DRI.PONo,
			DRI.DayPlanNo,
			DRI.MaterialCode,
