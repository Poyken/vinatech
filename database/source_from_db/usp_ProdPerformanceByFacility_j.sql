...
-- Modified: 누계 관련 내용 추가 --#220222
-- ======================================================================================================
CREATE PROCEDURE usp_ProdPerformanceByFacility_j
...
BEGIN
	...
	--#220222 최종 쿼리에는 공정코드가 없으므로 공정코드 기준으로 공정명을 가져온다.
	IF @RouteCode = '*' BEGIN
		SET @RouteName = '*'
	END ELSE BEGIN
   ...
   ORDER BY CONVERT(CHAR(10), FIN.ProdDateTime, 120)
		...
		SET @TotDefectRate = @TotDefectQty / @TotInputQty * 100

		INSERT INTO @First_Table VALUES ('', '누계', '투입수량', 0, @TotInputQty)
		INSERT INTO @First_Table VALUES ('', '누계', '불량수량', 0, @TotDefectQty)
		INSERT INTO @First_Table VALUES ('', '누계', '불량률', 0, @TotDefectRate)
   ...
   UNPIVOT (
	...
	INSERT INTO #Third_Table (PlanDate, Qty, RouteName, MachineName, QtyCode)
		...
		UNION ALL
		SELECT PlanDate
		      ,CONVERT(NUMERIC(20,2), SUM(CASE WHEN QtyCode = '불량수량' THEN Qty ELSE 0 END)) 
				/ SUM(CASE WHEN QtyCode = '투입수량' THEN Qty ELSE 0 END) * 100
			  ,''
			  ,'누계'
			  ,'불량률'
		  FROM #Third_Table
		 WHERE QtyCode IN ('투입수량', '불량수량')
		   AND (@RouteName = '*' OR RouteName = @RouteName)
		 GROUP BY PlanDate
