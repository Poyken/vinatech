...
-- Browsable : true
-- Group : 지지체
-- Description:	지지체 소분정보를 재고에 반영합니다.
-- =============================================
CREATE PROC [dbo].[usp_DoMakeStocktakingPlanResultForSupport]
...
BEGIN
	...
	SELECT @NextSeq = ISNULL(MAX(SDNSeqNo) + 1, 1) FROM STB_StocktakingPlanResult WHERE StocktakingDocNo = @StocktakingDocNo
