...
/* exec usp_VVT_ESRdata_get '','','VVT','2024-08-18','2024-08-20','','','',''  */

CREATE  PROCEDURE [dbo].[usp_VVT_ESRdata_get]
...
AS
	...
	DECLARE @FromDate   VARCHAR(19)   =  case when datepart(HOUR,@pFromDate)>0  and datepart(HOUR,@pFromDate)<22 then CONVERT(VARCHAR(19), @pFromDate, 120) else CONVERT(VARCHAR(10), @pFromDate, 120)+' 10:00:00' end                                                           -- SELECT  CONVERT(VARCHAR(10), '2019-09-16', 121) + ' 08:30:00' 
	DECLARE @ToDate      VARCHAR(19)  =  case when datepart(HOUR,@pToDate)>0  and datepart(HOUR,@pToDate)<22 then CONVERT(VARCHAR(19), DATEADD(DAY, 1, CONVERT(smalldatetime, @pToDate)), 120) else CONVERT(VARCHAR(10), DATEADD(DAY, 1, CONVERT(smalldatetime, @pToDate)), 120)+' 10:00:00' end                                           -- 다음날 SELECT CONVERT(VARCHAR(10), DATEADD(DAY, 1, CONVERT(smalldatetime, '2019-09-17 00:01:09')), 121) + ' 08:30:00'    		
...
where machinecode='DataQC' and inspecttime> dateadd(day,-10, getdate())
	...
	begin
			...
			,CompanyCode  as Barcode
			 ...
			 , ip
		,inspecttime as inspectime --	 ,  inspectime
	...
	begin
		...
		SELECT top 10000000		
			...
			,mbi.ModelSize  + '('+(mbi.MBIExtText05) + 'F)'
			 ...
			 select * from STB_VVT_ESRDATA with(nolock) where (isnull(@pLotNo,'')='' or CompanyCode=@pLotNo) 
