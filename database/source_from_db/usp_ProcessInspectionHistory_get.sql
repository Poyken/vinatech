...
--  프로시저실행 :   usp_ProcessInspectionHistory_get 'kilee','Korean','VNT','VNT_F1','2020-04-28 00:00:00','2020-04-30  00:00:00','VVKM273R010504'
--                      usp_ProcessInspectionHistory_get 'kilee','Korean','VNT','VNT_F1','2020-04-28 00:00:00','2020-04-30  00:00:00',''
-- =============================================	
CREATE PROCEDURE [dbo].[usp_ProcessInspectionHistory_get]
...
BEGIN
	...
	SELECT A.CommInspDocNo
			...
			, ISNULL(A.CommInspItemSpec, 0) AS CommInspItemSpec
			, ISNULL(A.CommInspUpper, 0)    AS CommInspUpper
			, ISNULL(A.CommInspLower, 0)    AS CommInspLower

			, A.CommInspItemDesc		
			,  ISNULL(A.NumericMeasure, 0) AS NumericMeasure
			, B.CommInspItemDesc
			, ISNULL(B.NumericMeasure, 0) AS NumericMeasure
...
FROM (	 
		...
		WHERE 1=1	
		   AND CIDI.commInspItemCode in ('V_RQ_THICKP')    -- 전극두께(+)  [RQ가 붙으면 공정검사 / RQ없으면 자주검사]		  
		) A
			   LEFT OUTER JOIN (
										 SELECT	      																	
													CIDI.CommInspDocItemNo,
											...
											FROM
													...
													LEFT OUTER JOIN STB_CommInspMeasureHist CIMH                 ON CIMH.CommInspDocItemNo = CIDI.CommInspDocItemNo
													LEFT OUTER JOIN VW_ModelBasicInfo           VM                  ON SI.MaterialCode = VM.ModelCode
													--LEFT OUTER JOIN STB_MaterialMaster           MM                  ON SI.MaterialCode = VM.ModelCode
											WHERE 1=1										 
												AND CIDI.commInspItemCode in ('V_RQ_THICKM')    -- 전극두께(-)  [RQ가 붙으면 공정검사 / RQ없으면 자주검사]
										 
									) B ON A.Barcode = B.Barcode  
WHERE 1=1	
	AND (@Barcode = '*' Or  A.Barcode  LIKE @Barcode)	
	AND A.CreateDateTime between  @FromDate And @ToDate	
	--AND A.commInspItemCode in ('V_RQ_THICKP')   -- RQ가 붙으면 공정검사 / RQ없으면 자주검사	
	AND (@SizeCode = '*' Or  A.SizeCode  LIKE @SizeCode)
