...
-- Browsable : True
-- Group : 
-- Description:  당일날 VPC 권취실적을 업데이트 해주는 프로시저 
                   
-- Modified:  
--                2022-03-13 실행

-- 프로시저 실행 :  usp_VPCWindingTodayProd_iud  '',''

-- 테이블 조회 :     Select * from VPC_Performance     
-- ===============================================================================================
CREATE PROCEDURE [dbo].[usp_VPCWindingTodayProd_iud]
...
BEGIN
	...
	DECLARE @FromDate DateTime = Convert(Varchar(10), GetDate(), 121) + ' 08:30:00'            -- 전일8시반        Select Convert(Varchar(10), GetDate()-1, 121) + ' 08:30:00'  
	...
	DECLARE @ToDay      Varchar(02) = Substring(Convert(Varchar(10), GetDate(), 121), 9, 2)  --테이블에 데이터 입력되는날짜 오늘날짜로 찍혀야므로 당일임!!(다른프로시저랑 헷갈리지말것) / Select Substring(Convert(VARCHAR(10), GetDate()-11, 121), 9, 2) 
	DECLARE @BaseYm    Varchar(06)
	DECLARE @OneDay    Varchar(11) = Substring(Convert(Varchar(10), GetDate(), 121), 0, 11)    -- 금일(2022-03-10)   /   SELECT Substring(Convert(VARCHAR(10), GetDate()-1, 121), 0, 11)
