...
CREATE PROCEDURE [dbo].[usp_Vietnam_ProductsReceipt_autoinsertBACKUP]
		@truedate date = null
AS
--- 'VNHK220905-002',
--  'VNHK220905-001',
...
;with	 	ta as (	select top 100000 
				...
				,tb as (
				select Barcode,ProdQty , PackingDate
				...
				and ta.PackQty=tb.ProdQty and convert(varchar(10),ta.DateExport,120)=convert(varchar(10),tb.PackingDate,120)
			...
			select
			fish.lotno,SOINVOICE,convert(varchar(10),fish.DateExport,120),convert(varchar(10),case when upper(TRANSPORT)='AIR' then dateadd(day,1,fish.DateExport) else dateadd(day,2,fish.DateExport) end,120)
				,replace(replace(replace(TRANSPORT,'Truck','03'),'AIR','01'),'SEA','02') as TRANSPORT,materialcode,
				fish.packqty AS qtypackqty,partno,--'' REMARK,
rtrim(ltrim(replace(replace(replace(replace(replace(replace(replace(replace(replace(replace(replace(replace(replace(replace(replace(replace(replace(replace(replace(replace(replace(replace(replace(replace(country,'(',''),')',''),N'Kỹ thuật sản phẩm',''),N'Kĩ thuật sản phẩm',
''),N'Tây Ban Nha',
''),N'Thổ Nhĩ Kỳ',
''),N'Trung Quốc',
''),N'Sản Xuất',
...
''),N'Pháp',
''),N'Mỹ',
''))),CUSTOMERNAME Customer,--ProductionSize,
				fish.DateExport,USERID--,TYPEEXPORT
			from STB_VN_FINISHGOODS fish with(nolock) 
			join notexist on fish.LotNo=notexist.LotNo and fish.packqty=notexist.PackQty and fish.DateExport = notexist.DateExport 
			where fish.id >= @limitid and fish.flag=1
			and SoInVoice not in ('VNHK220905-002')
