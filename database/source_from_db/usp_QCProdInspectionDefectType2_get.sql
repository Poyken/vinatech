...
-- Modified :
-- =============================================
CREATE PROCEDURE [dbo].[usp_QCProdInspectionDefectType2_get]
	...
	SELECT A.IDX
	  ...
	  FROM (
			SELECT ROW_NUMBER() OVER(ORDER BY SUM(NCNFRMITY_ERROR_CNT) DESC) AS IDX
				  ,CASE WHEN B.DTL_NM LIKE '%치수%' THEN '치수'
						WHEN B.DTL_NM LIKE '%특성%' THEN '특성'
						ELSE '외관' END AS DefectName
				 ...
				 GROUP BY CASE WHEN B.DTL_NM LIKE '%치수%' THEN '치수'
							   WHEN B.DTL_NM LIKE '%특성%' THEN '특성'
							   ELSE '외관' END
			) A
		   INNER JOIN (
		   ...
		   ON 1=1
	 WHERE A.DefectQty <> 0
	 GROUP BY A.IDX, A.DefectName, A.DefectQty
