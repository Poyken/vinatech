...
-- =============================================
CREATE PROCEDURE [dbo].[usp_InfomationWoker_uid_testttt] 
...
BEGIN
	...
	BEGIN TRY
		...
		MERGE STB_ProdWorkerInfo_Test_forLap AS TargetTable
		USING 
			(
					SELECT
							CASE
			...
			WHEN NOT MATCHED THEN
				...
				VALUES
					...
					--RAISERROR (@InsertTableName, 0, 1) WITH NOWAIT
        ...
        BEGIN TRY
            ...
            WHILE 1 = 1 BEGIN
                ...
                IF @IUD_FLAG = 'INSERT' BEGIN
                    ...
                    IF EXISTS (SELECT 1 FROM STB_ProdWorkerInfo_Test_forLap WHERE --CompanyCode = @CompanyCode AND WorkCenterCode = @WorkCenterCode AND
					 WorkerCode = @WorkerCode) BEGIN
						RAISERROR('Duplicate Data : KeyField = %s', 16, 1, @CompanyCode)
					END

                    IF @IsAutoKey = 1 BEGIN
                        EXEC SmartFramework.dbo.usp_DoCreateSerial 'STB_ProdWorkerInfo_Test_forLap',@CompanyCode OUTPUT
                    END

                    INSERT INTO STB_ProdWorkerInfo_Test_forLap
						(
						    CompanyCode,
						    WorkCenterCode,
						    WorkerCode,
				...
				END ELSE IF @IUD_FLAG = 'UPDATE' BEGIN
                    UPDATE STB_ProdWorkerInfo_Test_forLap
						SET
                ...
                END ELSE IF @IUD_FLAG = 'DELETE' BEGIN
                    DELETE FROM STB_ProdWorkerInfo_Test_forLap
						WHERE
						    CompanyCode = @OldCompanyCode AND
						    WorkCenterCode = @OldWorkCenterCode AND
						    WorkerCode = @OldWorkerCode
