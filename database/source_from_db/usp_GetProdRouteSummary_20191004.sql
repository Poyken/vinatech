...
-- =============================================
 -- 공정별 생산현황 전체조회 ->  EXEC  usp_GetProdRouteSummary_20191004 '','','','','ASSYLINE-05','E-28','','2019-10-01','2019-10-04',''


CREATE PROCEDURE [dbo].[usp_GetProdRouteSummary_20191004]
...
BEGIN
	...
	SELECT
			...
			--CASE WHEN CONVERT(BIT, ISNULL(NP.AftProdQty,0) > 0 THEN SUM(PRS.InputQty) ELSE 0 END InputQty,
			--CASE WHEN CONVERT(BIT, ISNULL(NP.AftProdQty,0) > 0 THEN SUM(PRS.OutputQty) ELSE 0 END OutputQty,
			--CASE WHEN CONVERT(BIT, ISNULL(NP.AftProdQty,0) > 0 THEN SUM(PRS.DefectQty) ELSE 0 END DefectQty,
			--CASE WHEN CONVERT(BIT, ISNULL(NP.AftProdQty,0) > 0 THEN SUM(PRS.RepairQty) ELSE 0 END RepairQty,
			--CASE WHEN CONVERT(BIT, ISNULL(NP.AftProdQty,0) > 0 THEN SUM(PRS.LossQty) ELSE 0 END LossQty,
			SUM(PRS.InputQty)              AS InputQty,
			SUM(PRS.OutputQty)            AS OutputQty,
			(SUM(PRS.OutputQty) - SUM(PRS.DefectQty))  AS GoodsQty,                      --양품수량 추가 (2019.09.23 주영진 요청)
		  ...
		  , dbo.fnGetWastePrice(PRS.LineCode, PRS.RouteCode, PRS.MaterialCode, SUM(PRS.DefectQty))                                    AS DefectPrice
	FROM
	...
	WHERE 1=1
	     AND PRS.TimeCode <> 'E'                                                                                             -- MES 등록한것만 (2019.07.15)
		  ...
		  --AND ISNULL(NP.AftProdQty,0) > CASE WHEN @RouteCode = '%' THEN -1 ELSE 0 END    --추가
		  --and PRS.LineCode = 'ASSYLINE-12'
		  --and prs.MaterialCode = 'ECVT30-115'
