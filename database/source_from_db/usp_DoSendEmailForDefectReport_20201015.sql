...
-- Modified:
 ...
 -- usp_DoSendEmailForDefectReport_20201015 '','','VN200924-02'
-- =============================================
CREATE PROCEDURE [dbo].[usp_DoSendEmailForDefectReport_20201015]
...
BEGIN
	Declare @DefectReportNo VARCHAR(20) = @pDefectReportNo
		   ...
		   --,@CcAddress VARCHAR(MAX) = 'v.qc@vina.co.kr;'
		   		   -- 생산부문, 베트남부문
		   ,@ToAddress VARCHAR(MAX) = 'kilee@vina.co.kr;yjyu@vina.co.kr'	
	...
	SELECT @DefectDivisionName = BC1.Description
		  ...
		  ,@DefectRate = CONVERT(NUMERIC(20,2), CONVERT(NUMERIC(10,3), QDR.DefectErrorCnt) / CONVERT(NUMERIC(10,3), QDR.DefectSampleCnt) * 100)
 ...
 --   SET @EmailBody = @EmailBody + '<body>'
    SET @EmailBody =                     N'    <table width="900" cellspacing="1" cellpadding="1" border="1">'
    ...
    SET @EmailBody = @EmailBody + N'          <td valign="top" align="left">' + CASE WHEN @DefectDivisionName = '제품검사' THEN '■' ELSE '□' END + N'제품검사<br/>Kiểm tra sản phẩm </b></td>'
