...
-- Modified:
-- =============================================
CREATE PROCEDURE [dbo].[usp_VVT_GetOutSparePartHistoryManagement]
...
BEGIN
	...
	WHERE
	        ...
	        (SPIOTC.IOType  in ('U','M') or 
				(SPIOTC.IOType ='O' and  (substring(isnull(SPIOH.SparePartCode,'  '),1,2) not in ('CS','SP') ) )
			)  
			AND SPIOH.Attribute1 IS NULL AND 
