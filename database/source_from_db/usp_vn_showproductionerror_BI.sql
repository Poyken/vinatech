...
-- Description:	Tổng hợp dữ liệu 2 nhà máy lên BI
-- ============================================= exec [usp_vn_showproductionerror_BI] '2025-07-01','2025-07-30','','',''
CREATE PROCEDURE [dbo].[usp_vn_showproductionerror_BI] 
@pFromdate DATE = NULL,
	@pToDate DATE = NULL,
		...
		(case  when   (DATEPART(HOUR, createdatetime)>10)   or  (DATEPART(HOUR, createdatetime)=10 and DATEPART(MINUTE, createdatetime)>30)     
		  ...
		  as  JobDate,   -- add by Mr.Tung on 31-July-2021
			...
			WHERE 
					...
					AND 
							(WorkCenterCode in ('VVT_F1','VVT_F2'))
