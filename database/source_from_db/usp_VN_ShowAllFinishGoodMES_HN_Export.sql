...
-- Description:	Hiển thị danh sách xuất kho thành phẩm
-- exec usp_VN_ShowAllFinishGoodMES_HN_Export'2025-07-14','2025-07-31'
-- =============================================
CREATE PROCEDURE [dbo].[usp_VN_ShowAllFinishGoodMES_HN_Export]
...
BEGIN
  --select top 2 * from STB_VN_FINISHGOODS_HN_Export
	...
	SELECT
		...
		*/
		COALESCE(T9.MergeBoxSmallID, T6.MergeParentID, T5.MergeParentID) AS MergeParentID, 	
		SUM(T2.Qty) AS Qty,
		/*
		CASE 
	...
	END AS MaterialCode,
	*/    COALESCE(T9.MaterialCode, T6.MaterialCode, T5.MaterialCode) AS MaterialCode,
		T3.MarkingCode,
		T3.MarkingName,
		T2.Country,
		T2.SoPhieuXuatKho,
	...
	LEFT JOIN FinishGoodMESInstock_HN T9 ON T2.LotNo=T9.LotNo and T9.PackingID=T2.PackingID
    ...
    END,
		...
		ELSE T5.MaterialCode
	END,
	*/  COALESCE(T9.MergeBoxSmallID, T6.MergeParentID, T5.MergeParentID), 	
    COALESCE(T9.MaterialCode, T6.MaterialCode, T5.MaterialCode), 	
		T3.MarkingCode,
		T3.MarkingName,
		T2.Country,
		T2.SoPhieuXuatKho,
