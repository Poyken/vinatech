...
--             usp_GetProdRouteSummary_DashBoard  'VNT' , 'VNT_F1' , 'ASSYLINE-07',  '', '' ,'2021-01-01' , '2021-01-05'    , '' , ''          -- 19개
--  usp_GetProdRouteSummary_DashBoard  '','','','','','','','2021-04-26','2021-05-25',''
-- ==========================================================================================

CREATE PROCEDURE usp_GetProdRouteSummary_Dashboard_20210525
...
BEGIN
	...
	FROM
			...
			LEFT OUTER JOIN (SELECT CompanyCode 
							  ...
							  WHERE JobDate BETWEEN @FromDate AND @ToDate
							  GROUP BY CompanyCode, WorkCenterCode, JobDate, ShiftCode, PONo
								      ,MachineCode, LineCode, RouteCode
							) PRH	                    
			  ON PRH.CompanyCode = PRS.CompanyCode
			 AND PRH.WorkCenterCode = PRS.WorkCenterCode
			 AND PRH.JobDate = PRS.JobDate
			 AND PRH.ShiftCode = PRS.ShiftCode
			 AND PRH.PONo = PRS.PONo
			 AND PRH.MachineCode = PRS.MachineCode
			 AND PRH.LineCode = PRS.LineCode
			 AND PRH.RouteCode = PRS.RouteCode
			 RIGHT OUTER JOIN STB_ProdWorkerInfo PWI			   ON PWI.WorkerCode = PRH.WorkerCode
	...
	WHERE 1=1	 
	  ...
	  --AND ISNULL(NP.AftProdQty,0) > CASE WHEN @RouteCode = '%' THEN -1   
			...
			--							        OR dbo.fnGetNextRouteCode(PRS.PONo, @RouteCode) IN ('E-28', 'V-28') ) THEN -1  ELSE 0 END
		  	 AND ((@CompanyCode = '*') OR (PRS.CompanyCode = @CompanyCode))   	        
			 AND ((@WorkCenterCode = '*') OR (PRS.WorkCenterCode = @WorkCenterCode)) 
