...
-- Browsable : True
-- Group : E-Mail >전일!!! VPC 실적을 전직원에게 메일로 발송
-- Description: 
-- Modified:  데이터 E-Mail 

-- 전일 프로시저 실행 :  usp_VPC_BeforePerformance_EMail '', ''
-- ===============================================================================================
CREATE PROCEDURE [dbo].[usp_VPC_BeforePerformance_EMail]
...
BEGIN
    ...
    --DECLARE @htmlText NVARCHAR(MAX) = '<table border=1><thead><tr><th>라인명</th><th>공정명</th><th>전일실적</th><th>08:30~13:00</th><th>08:30~20:30</th><th>금일합계</th></tr></thead><tbody>'
	DECLARE @htmlText NVARCHAR(MAX) = '<table border=1><thead><tr><th>라인명</th><th>목표수량</th><th>8:30~13:30</th><th>13:30~20:30</th><th>전일 합계</th><th>성취률</th></tr></thead><tbody>'
 ...
 -- DECLARE @ToAddress VARCHAR(MAX) = 'v.all@vina.co.kr;'             -- 전직원
  DECLARE @ToAddress VARCHAR(MAX) = 'v.manufacturing@vina.co.kr;'             -- 생산
	--DECLARE @ToAddress VARCHAR(MAX) = 'nicekangil@naver.com;'     -- 테스트 용도 (내메일)
	 
--    DECLARE @CcAddress VARCHAR(MAX) = 'kilee@vina.co.kr;'
	DECLARE @CcAddress VARCHAR(MAX) = 'oqc@vina.co.kr;v.planning@vina.co.kr;'
...
;WITH SampleData AS (
							...
							select         LineName 
										  ...
										  , Format(SUM(CASE WHEN CONVERT(VARCHAR(19), ProdDateTime, 121) BETWEEN CONVERT(VARCHAR(10), GetDate()-1, 121) + ' 08:30:00' AND CONVERT(VARCHAR(10), GetDate()-1, 121)+ ' 13:30:00' THEN CONVERT(INT, ProdQty)  ELSE 0 END),  'N0')  AS FirstProdQty
										  , Format(SUM(CASE WHEN CONVERT(VARCHAR(19), ProdDateTime, 121) BETWEEN CONVERT(VARCHAR(10), GetDate()-1, 121) + ' 13:30:00' AND CONVERT(VARCHAR(10), GetDate()-1, 121) + ' 20:30:00' THEN CONVERT(INT, ProdQty) ELSE 0 END),  'N0')  AS SecondProdQty
										  , Format(SUM(CASE WHEN CONVERT(VARCHAR(19), ProdDateTime, 121) BETWEEN CONVERT(VARCHAR(10), GetDate()-1, 121) + ' 08:30:00' AND CONVERT(VARCHAR(10), GetDate(), 121) + ' 20:30:00' THEN CONVERT(INT, ProdQty) ELSE 0 END),  'N0')  AS TotalProdQty
...
--SELECT @htmlText
    ...
    -- 메일 Seting부분
   SET @MailSubject = '전일 VPC 권취실적 공유드립니다.' + ' (' + CONVERT(CHAR(5), GETDATE()-1, 101)   + ') '          -- 제목
