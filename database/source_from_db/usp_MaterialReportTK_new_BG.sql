...
-- =============================================
CREATE PROCEDURE [usp_MaterialReportTK_new_BG] -- exec usp_MaterialReportTK_new '2024-12-01','2024-12-30'
...
DECLARE @pMaterialWarehouseCode VARCHAR(50) = 'ROH_BG_WH'
...
if(@FromDate>='2023-01-01' and @FromDate<='2024-11-30') begin
	print 'a'
			...
			(
					SELECT						
									 ...
									 , MM.MaterialCode						 
									  ...
									  ,MWIOH.LineCode
									 ,LI.LineName, 
									 case when WarehouseInOutCode='O' and SourceMaterialWarehouseCode='ROH_BG_WH'  and MWIOH.TargetMaterialWarehouseCode<>'HOLDING_BG_WH' and MWIOH.linecode='R-KR' then 'Tai Xuat'
										  when WarehouseInOutCode='O' and SourceMaterialWarehouseCode='ROH_BG_WH'  and MWIOH.TargetMaterialWarehouseCode<>'HOLDING_BG_WH' and  MWIOH.linecode  in ('XCDMDSD','XTH') then 'Xuat chuyen doi, tieu huy'
										  when WarehouseInOutCode='O' and SourceMaterialWarehouseCode='ROH_BG_WH'  and MWIOH.TargetMaterialWarehouseCode<>'HOLDING_BG_WH' and  MWIOH.linecode='XK' then 'Xuat khac'
										   when WarehouseInOutCode='O' and SourceMaterialWarehouseCode='ROH_BG_WH'  and MWIOH.TargetMaterialWarehouseCode='ROH_BG_WH'/* and  MWIOH.linecode='BG'*/ then 'Xuat BG'
										  when WarehouseInOutCode='O' and SourceMaterialWarehouseCode='ROH_BG_WH'  and MWIOH.TargetMaterialWarehouseCode<>'HOLDING_BG_WH' and  MWIOH.linecode not in ('R-KR','XCDMDSD','XTH','XK','KVHD') then'Xuat san xuat' 
										  when   MWIOH.SourceMaterialWarehouseCode='HOLDING_BG_WH' and MWIOH.TargetMaterialWarehouseCode='ROH_BG_WH'  then 'Holding_Back'
										   when  MWIOH.SourceMaterialWarehouseCode='ROH_BG_WH' and MWIOH.TargetMaterialWarehouseCode='HOLDING_BG_WH'  then 'To_Holding'
										  when  WarehouseInOutCode='I' and TargetMaterialWarehouseCode='ROH_BG_WH' /*and MWIOH.linecode not in ('R-KR','XCDMDSD','XTH','XK','KVHD' )*/ then 'Nhap Lai Tu SanXuat' 
											when WarehouseInOutCode='O' and SourceMaterialWarehouseCode='ROH_BG_WH'  and MWIOH.TargetMaterialWarehouseCode='ROH_BG_WH'  then 'Nhap lai tu BG'
						...
						FROM            STB_MaterialWarehouseInOutHist MWIOH with(nolock)  
								 ...
								 WHERE 1=1	   
								   ...
								   and  (isnull(MWIOH.ProcessedLotID,'') <> '' or MWIOH.SourceMaterialWarehouseCode = 'HOLDING_BG_WH' or MWIOH.TargetMaterialWarehouseCode = 'HOLDING_BG_WH') 
		...
		print 'b'
			;with
			InventoryMaterial as( -- tồn đầu kì được kiểm kê đầu năm 2023
				select materialcode, firstinventory from Stb_InventoryMaterialLiquidation FIM with (nolock) where [Period]='2025-01-01' and workcentercode='VVT_F2'
			),
			InputMaterialCode as ( -- lấy ra nhập vào kho nvl
					select  (case when mli.materialcode<> mm.MaterialCode then mli.materialcode else mm.MaterialCode end) MaterialCode
					  , mm.materialName, mm.MMExtText03 as NameHQ, mm.MMExtText04 as CodeHQ ,  --mm.MaterialUnit,
					...
					where   mdi.TargetCompanyCode='VVT' --and mdi.TargetWorkCenterCode='VVT_F1' 
					and		mdi.materialDocType='GR' and mdi.materialdoctypecode in ('GR_NORMAL','GR_RETURN_MATERIAL') and mdi.docstatus in ('FIX','FINISH','ARRIVAL')
				--	and		mli.MaterialLocationCode in ('ROH_BG_WH_01')
					and		ui.Workcentercode ='VVT_F2'
			),
			 table1  as (
				select MaterialCode, NameHQ, CodeHQ , 
				sum(NhapDauKy) as NhapDauKy, sum(NhapDauKy1) as NhapDauKy1, sum(NhapTrongKy) as NhapTrongKy from InputMaterialCode 
			...
			(
					SELECT						
									 ...
									 ,LI.LineName, 
									 case 
										  when   MWIOH.SourceMaterialWarehouseCode in ('ROH_BG_WH','HOLDING_BG_WH') and MWIOH.TargetMaterialWarehouseCode in ('NUOC_NGOAI_BG','NOI_DIA_BG')	and MWIOH.linecode in ('TVNCC_BG')	then 'Tralaincc' -- trả lại nhà cc

										  when   MWIOH.SourceMaterialWarehouseCode in ('ROH_BG_WH','HOLDING_BG_WH') and MWIOH.TargetMaterialWarehouseCode='NG_RAW_BG_WH' and MWIOH.linecode in ('XTH_BG')   then 'XuatTieuHuy' -- xuất tiêu hủy

										  when   MWIOH.SourceMaterialWarehouseCode in ('ROH_BG_WH','HOLDING_BG_WH') and MWIOH.TargetMaterialWarehouseCode='ROH_WH'		and MWIOH.linecode in ('R-KR_BG')		then 'ReturnVinatech' -- trả lại công ty mẹ
										
										  when WarehouseInOutCode='O' and SourceMaterialWarehouseCode in ('ROH_BG_WH','HOLDING_BG_WH')   and MWIOH.TargetMaterialWarehouseCode='ROUTE_BG_WH' and  MWIOH.linecode='XK_BG' then 'Xuat khac'
										
										  when WarehouseInOutCode='O' 
										  and SourceMaterialWarehouseCode in ('ROH_BG_WH','HOLDING_BG_WH')  
										  and MWIOH.TargetMaterialWarehouseCode IN ('ROUTE_BG_WH','MODULE_BG_WH')    
										  and MWIOH.linecode not in ('R-KR_BG','XCDMDSD_BG','XTH_BG','XK_BG','KVHD_BG','TVNCC_BG')
										  then'Xuat san xuat'
										  ...
										  and TargetMaterialWarehouseCode in ('ROH_BG_WH','HOLDING_BG_WH') 
										  and MWIOH.SourceMaterialWarehouseCode<>'HOLDING_BG_WH' 
										  and MWIOH.SourceMaterialWarehouseCode<>'ROH_BG_WH'  
										  ...
										  when WarehouseInOutCode='O' 
										  and SourceMaterialWarehouseCode in ('ROH_BG_WH','HOLDING_BG_WH')  
										  and MWIOH.TargetMaterialWarehouseCode in ('ROH_BG_WH','HOLDING_BG_WH')  
										  and MWIOH.linecode ='BN'
										   then 'Nhap lai tu BG'
										   */
										  --when WarehouseInOutCode='O' and SourceMaterialWarehouseCode='ROH_BG_WH' and MWIOH.TargetMaterialWarehouseCode not in ('HOLDING_BG_WH') then 'Xuat_khac'
				...
				select  a1.materialcode, a1.materialName,a1.MMExtText03 as NameHQ,a1.MMExtText04 as CodeHQ,a1.MaterialUnit,
						...
						case when a3.outdate < @FromDate and typeOut = 'Xuat BG' then isnull(a3.OutQty,0) end as XuatBNDauKy,
						case when a3.outdate < @FromDate and typeOut = 'Xuat san xuat' then isnull(a3.OutQty,0) end as XuatSanXuatDauKy,
						case when a3.outdate < @FromDate and typeOut = 'Nhap lai tu BG' then isnull(a3.OutQty,0) end as NhaplaiBNdauky,
						*/
						...
						case when a3.outdate between '2025-01-01 10:00:00' and @daten and typeOut = 'Xuat BN' then isnull(a3.OutQty,0) end as XuatBN1,
						...
						case when a3.outdate between '2025-01-01 10:00:00' and @daten and typeOut = 'Nhap lai tu BN' then isnull(a3.OutQty,0) end as NhaplaiBN1,
						...
						case when a3.outdate between @FromDate and @ToDate  and typeOut = 'Xuat san xuat' then isnull(a3.OutQty,0) end as XuatSanXuat,
						case when a3.outdate between @FromDate and @ToDate  and typeOut = 'Xuat BN' then isnull(a3.OutQty,0) end as XuatBN,
						...
						case when a3.outdate between @FromDate and @ToDate  and typeOut = 'Nhap lai tu BN' then isnull(a3.OutQty,0) end as NhaplaiBN,
						case when a3.outdate between @FromDate and @ToDate  and typeOut = 'XUAT_KHO_AO' then isnull(a3.OutQty,0) end as XUAT_KHO_AO,
			...
			CalculatorInventoryMaterial as (
				...
				case  when @FromDate = '2025-01-01 10:00:00'  then COALESCE(max(openninginventory),0)
					  ...
					  then 
						COALESCE(max(openninginventory),0) + COALESCE(max(NhapDauKy1),0)+COALESCE(sum(NhaplaiBN1),0) +COALESCE(sum(NhaplaiSX1),0) 
						- COALESCE(sum(Tralaincc1),0) - COALESCE(sum(XuatTieuHuy1),0)  - COALESCE(sum(ReturnVinatech1),0) - COALESCE(sum(Xuat_khac1),0)  
						- COALESCE(sum(XuatSanXuat1),0) - COALESCE(sum(XuatBN1),0)  - COALESCE(sum(XUAT_KHO_AO1),0)
					  else 
							N'2025-01-01'
					  end
					  as TonDauKy,
				...
				COALESCE(sum(XuatSanXuat),0) as XuatSanXuat,
				COALESCE(sum(XuatBN),0) as XuatBN,
				COALESCE(sum(NhaplaiSX),0) as NhaplaiSX,
				COALESCE(sum(NhaplaiBN),0) as NhaplaiBN,
				COALESCE(sum(XUAT_KHO_AO),0) as XUAT_KHO_AO,
				...
				COALESCE(sum(XuatSanXuat1),0) as XuatSanXuat1,
				COALESCE(sum(XuatBN1),0) as XuatBN1,
				COALESCE(sum(NhaplaiSX1),0) as NhaplaiSX1,
				COALESCE(sum(NhaplaiBN1),0) as NhaplaiBN1,
				COALESCE(sum(XUAT_KHO_AO1),0) as XUAT_KHO_AO1,
 

				case  when @FromDate = '2025-01-01 10:00:00' then COALESCE(max(openninginventory),0) +COALESCE(max(NhapTrongKy),0) +COALESCE(sum(NhaplaiBN),0)  +COALESCE(sum(NhaplaiSX),0) 
				-COALESCE(sum(XuatBN),0) - COALESCE(sum(XuatSanXuat),0)
					  when  @FromDate > '2025-01-01 10:00:00'  then 
						COALESCE(max(openninginventory),0) + COALESCE(max(NhapDauKy1),0)+COALESCE(sum(NhaplaiBN1),0) +COALESCE(sum(NhaplaiSX1),0) 
						+COALESCE(max(NhapTrongKy),0)+COALESCE(sum(NhaplaiBN),0) +COALESCE(sum(NhaplaiSX),0) 
						- COALESCE(sum(Tralaincc1),0) - COALESCE(sum(XuatTieuHuy1),0)  - COALESCE(sum(ReturnVinatech1),0) - COALESCE(sum(Xuat_khac1),0)  
						- COALESCE(sum(XuatSanXuat1),0) - COALESCE(sum(XuatBN1),0)  - COALESCE(sum(XUAT_KHO_AO1),0)
						- COALESCE(sum(Tralaincc),0) - COALESCE(sum(XuatTieuHuy),0)  - COALESCE(sum(ReturnVinatech),0) - COALESCE(sum(Xuat_khac),0)  
						- COALESCE(sum(XuatSanXuat),0) - COALESCE(sum(XuatBN),0)  - COALESCE(sum(XUAT_KHO_AO),0)
					  else 
						N'2025-01-01'
					  end
					  as TonCuoi 					 
			...
			select * from CalculatorInventoryMaterial BB
				where (TonDauKy > 0 or NhapTrongKy > 0  or xuatsanxuat > 0 or XuatBN > 0 or NhaplaiBN > 0 or TonCuoi>0  ) 
