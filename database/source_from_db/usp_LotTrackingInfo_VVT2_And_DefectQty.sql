CREATE PROCEDURE [dbo].[usp_LotTrackingInfo_VVT2_And_DefectQty](
...
(ROW_NUMBER() over (partition by LotNo,isPrinted order by PackQty desc,printtime desc) ) as RowPackQty2
	  ...
	  join STB_RouteInfo d  WITH(NOLOCK)  on d.RouteCode=c.RouteCode
	  where 1=1 
	   and  EmpNo not in ('test_worker','assy_packing' )
	   --and c.RouteCode in ('V-28','V-28_BG')
	...
	DRI.id ,
	 newLotno,LotNo,PackingID,DRI.MaterialCode,MaterialName,EmpNo,PrintTime,RouteCode,RouteName,PackQty 
	 ...
	 left join  STB_DefectInfo     di   with(nolock)  on  a.DefectCode = di.DefectCode
	  ...
	  where 1=1 
	   and  EmpNo not in ('test_worker','assy_packing' )
		...
		group by DRI.id,newLotno,LotNo,PackingID,DRI.MaterialCode,MaterialName,EmpNo,PrintTime,PackQty ,RouteCode,RouteName,isPrinted,
