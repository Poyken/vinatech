...
-- Description:	Lấy dữ liệu của Bending,Cutting
--  exec usp_Vietnam_TappingBennding_get_PowerBI '','','2026-03-01','2026-03-18',''
-- =============================================
CREATE PROCEDURE [dbo].[usp_Vietnam_TappingBennding_get_PowerBI]
...
BEGIN
	...
	-- Mr.Manh update 2025-11-27
	--DECLARE @NewBarcode VARCHAR(50) = NULL
  ...
  for routecode in ([V-22],[V-23],[V-24],[V-25],[V-26],[V-27],[V-28])
	...
	select *from (
		SELECT 
			...
			MarkingLetters,
			CASE 
                WHEN UPPER(RTRIM(LTRIM(TYPESS))) IN ('TAPPING', 'TAPING') THEN 'Tapping'
                WHEN UPPER(RTRIM(LTRIM(TYPESS))) IN ('CUTTING', 'CUTING') THEN 'Cutting'
                WHEN UPPER(RTRIM(LTRIM(TYPESS))) = 'BENDING' THEN 'Bending'
				WHEN UPPER(RTRIM(LTRIM(TYPESS))) = 'SORTING' THEN 'Sorting'
				WHEN UPPER(RTRIM(LTRIM(TYPESS))) = 'REWORK' THEN 'Rework'
                ELSE TYPESS 
            END AS ProcessGroup
		FROM	
		...
		where 
		-- Trường hợp 1: Nêu mà không truyền vào j cả thì lấy là tapping
		(
        (@pType IS NOT NULL AND @pType <> '' AND (TYPESS = @pType OR (UPPER(@pType) = 'TAPPING' AND UPPER(TYPESS) = 'TAPPING')))
        
        OR 
        
        -- Trường hợp 2: Nếu KHÔNG truyền gì vào, chỉ lấy 3 loại chỉ định
        ((@pType IS NULL OR @pType = '') AND UPPER(TYPESS) IN ('TAPPING', 'BENDING', 'CUTTING','SORTING','REWORK'))
    )
	...
	select 
	Dates,
	--si.materialcode as ModelCode, -- comment 2025-11-27
	-- UPDATE to show materialcode for changed lot
	CASE WHEN EXISTS (SELECT 1 FROM STB_SetInfo SI2 where SI2.Barcode = rawdata.LOTNO) THEN si.materialcode
		ELSE (select top 1 BefMaterialCode FROM STB_LotChangeMaterialHistory LCMH WHERE LCMH.OldBarcode = rawdata.LOTNO)
		END AS ModelCode,
	-- END UPDATE
	codeproduction,LotNo,
	case when machinename IS NOT NULL then machinename
             else isnull(@pType, 'N/A') + ' ￡1' end as MachineName,
	...
	((sum(distinct total))-sum(isnull(([RÁCH VỎ])          ,0) + isnull(([XƯỚC CHÂN])        ,0) + isnull(([TANCHA BIẾN SẮC])  ,0) + isnull(([BẸP VỎ NHÔM])      ,0) + isnull(([NGƯỢC CỰC])        ,0) + isnull(([CONG CHÂN])        ,0) + isnull(([DỊ VẬT])           ,0) + isnull(([TRÀN DỊCH])        ,0) + isnull(([BIẾN SẮC ĐÁY])     ,0) + isnull(([RÁCH CAO SU])      ,0) + isnull(([LỒI ĐÁY])          ,0) + isnull(([BẸP ĐÁY])          ,0) + isnull(([NG MARKING])       ,0) + isnull(([ESR NG])   ,0)+isnull(([THIẾU SỐ LƯỢNG])   ,0) + isnull(([NG TAPE])          ,0) + isnull(([LỖI KHÁC])          ,0)) )  as OK_rate,
	sum(isnull(([RÁCH VỎ])          ,0) + isnull(([XƯỚC CHÂN])        ,0) + isnull(([TANCHA BIẾN SẮC])  ,0) + isnull(([BẸP VỎ NHÔM])      ,0) + isnull(([NGƯỢC CỰC])        ,0) + isnull(([CONG CHÂN])        ,0) + isnull(([DỊ VẬT])           ,0) + isnull(([TRÀN DỊCH])        ,0) + isnull(([BIẾN SẮC ĐÁY])     ,0) + isnull(([RÁCH CAO SU])      ,0) + isnull(([LỒI ĐÁY])          ,0) + isnull(([BẸP ĐÁY])          ,0) + isnull(([NG MARKING])       ,0) + isnull(([THIẾU SỐ LƯỢNG])   ,0) + isnull(([NG TAPE])          ,0)+ isnull(([ESR NG])          ,0) + isnull(([LỖI KHÁC])          ,0)) as Total_NG,
	convert(numeric(10,2),100*(sum(isnull(([RÁCH VỎ])          ,0) + isnull(([XƯỚC CHÂN])        ,0) + isnull(([TANCHA BIẾN SẮC])  ,0) + isnull(([BẸP VỎ NHÔM])      ,0) + isnull(([NGƯỢC CỰC])        ,0) + isnull(([CONG CHÂN])        ,0) + isnull(([DỊ VẬT])           ,0) + isnull(([TRÀN DỊCH])        ,0) + isnull(([BIẾN SẮC ĐÁY])     ,0) + isnull(([RÁCH CAO SU])      ,0) + isnull(([LỒI ĐÁY])          ,0) + isnull(([BẸP ĐÁY])          ,0) + isnull(([NG MARKING])       ,0) + isnull(([THIẾU SỐ LƯỢNG])   ,0) + isnull(([NG TAPE])          ,0)+isnull(([ESR NG])          ,0) + isnull(([LỖI KHÁC])          ,0)) )
	...
	isnull(sum([LỖI KHÁC])          ,0)                      as  [LỖI KHÁC],
		isnull(sum([ESR NG])          ,0)                      as  [ESR NG]
	, weight3.valweight as WeightUnit 
	, ISNULL(sum(isnull(([RÁCH VỎ])          ,0) + isnull(([XƯỚC CHÂN])        ,0)+ isnull(([ESR NG])        ,0) + isnull(([TANCHA BIẾN SẮC])  ,0) + isnull(([BẸP VỎ NHÔM])      ,0) + isnull(([NGƯỢC CỰC])        ,0) + isnull(([CONG CHÂN])        ,0) + isnull(([DỊ VẬT])           ,0) + isnull(([TRÀN DỊCH])        ,0) + isnull(([BIẾN SẮC ĐÁY])     ,0) + isnull(([RÁCH CAO SU])      ,0) + isnull(([LỒI ĐÁY])          ,0) + isnull(([BẸP ĐÁY])          ,0) + isnull(([NG MARKING])       ,0) + isnull(([THIẾU SỐ LƯỢNG])   ,0) + isnull(([NG TAPE])          ,0)+ isnull(([NG OCV])          ,0)  + isnull(([LỖI KHÁC])          ,0)), 0) * weight3.valweight/1000 as WasteWeight
	...
	group by dates,si.materialcode,codeproduction,lotno,machinename,pwi.workername,MarkingLetters,weight3.valweight,ProcessGroup
	--,total
	...
	--where lotno='VVLL303R033531'
		...
		SELECT
					...
					-- update 2025-05-08 following Mrs.Phuong's request (Final)
					WHEN ModelCode = 'RDMD00-301' AND codeproduction = 'WEC6R0504QD-IC030'		THEN Total_NG * 0.1831576
					WHEN codeproduction = 'WEC3R0335QG-C035 (0820)' THEN Total_NG * 0.0751269329508698
					WHEN ModelCode = 'ECVT30-346' AND codeproduction = 'WEC3R0106QG-C035'	THEN Total_NG * 0.118464496445016
					WHEN codeproduction = 'WEC6R0155QG-ILB'			THEN Total_NG * 0.247961574159429
					WHEN codeproduction = 'HY-CAP VEL08253R8506G-B034 (0825)'	THEN Total_NG * 0.156291643809569
					WHEN codeproduction = 'HY-CAP VEC3R0507QG (3582)'		THEN Total_NG * 2.91983268937296
					WHEN codeproduction = 'WEC3R0126HC-B0202R'		THEN Total_NG * 0.128810246738122
					WHEN codeproduction = 'VEC3R0256QG-B'			THEN Total_NG * 0.238492086494935
					WHEN codeproduction = 'HY-CAP VEC3R0727QG (35105)'			THEN Total_NG * 3.47144

					WHEN ModelCode = 'RDMD00-329' AND codeproduction ='WEC6R0355QG-OL-C68M42' THEN Total_NG * 0.2747284

					--2025-08-27
					WHEN ModelCode = 'ECVT30-268' THEN Total_NG * 0.075126933
					WHEN ModelCode = 'ECVT30-116' THEN Total_NG * 2.244622237
					WHEN ModelCode = 'RE3000-100' THEN Total_NG * 0.0686092

					--2025-10-02
					WHEN ModelCode = 'ECVT30-235' THEN Total_NG * 0.061 
					WHEN ModelCode = 'ECVT27-403' THEN Total_NG * 0.30461885496369
					--WHEN ModelCode = '' THEN Total_NG * 

					--2025-10-23
					WHEN ModelCode = 'RE3000-109' THEN Total_NG * 0.09288 
					WHEN ModelCode = 'RE3000-120' THEN Total_NG * 0.295 
					WHEN ModelCode = 'ECVT27-404' THEN Total_NG * 0.12244 

					--2025-11-02
					WHEN ModelCode = 'ECVT30-369' THEN Total_NG * 0.0929 
					WHEN ModelCode = 'ECVT30-260' THEN Total_NG * 1.5157781

					--2025-12-09
					WHEN ModelCode = 'ECVT30-356' THEN Total_NG * 0.061778277

					--2025-12-19
					WHEN ModelCode = 'ECVT30-366' THEN Total_NG * 0.115990703
					...
					CASE  
					   WHEN CODEPRODUCTION='HY-CAP WEC3R0186QC (1325)' then 'VVT_F1'
					   WHEN CODEPRODUCTION='HY-CAP VEC3R0727QG (35105)' THEN 'VVT_F2'
					   when CODEPRODUCTION='HY-CAP VEC3R0367QG (3562)' THEN 'VVT_F2'
					   when CODEPRODUCTION='HY-CAP WEC3R0106QA (1025)' then 'VVT_F1'
					   when CODEPRODUCTION='HY-CAP VEC2R7506QG (1840)' then 'VVT_F2'
					   WHEN CODEPRODUCTION='WEC3R0106QG-B060R' THEN 'VVT_F1'
					   when CODEPRODUCTION='HY-CAP VEC3R0107QG-C (2245)' then 'VVT_F2'
					   ELSE T2.WorkCenterCode
				   END AS WorkCenterCode
					
		FROM
				#t
				LEFT JOIN STB_ItemCode  T2 ON T2.MaterialCode=#t.ModelCode
