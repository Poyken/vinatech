...
-- Modified:           [usp_VVT_GetOutSparePartHistoryIMPROVE] '','','VVT','VVT_F1','','2022-09-17','2022-09-19',''
-- =============================================
CREATE PROCEDURE [dbo].[usp_VVT_GetOutSparePartHistoryMAINTENAINCE]
...
SELECT
	  ...
	  --      SPI.CompatibilityGroup,
	       ...
	       -- SPIOH.SPLocationCode,
	        ...
	        (isnull(SPI.BasicUnitPrice,SPIOH.UnitPrice)* case when SPIOH.ProcessQty=0 then 1 else isnull(SPIOH.ProcessQty,1) end ) as Price,

			case when SPIOH.ProcessQty=0 then 1 else isnull(SPIOH.ProcessQty,1) end  as ProcessQty,
	...
	WHERE
			...
			SPCH.MachineCode=@pMachineCode and 
	        ...
	        (SPIOTC.IOType  in ('M') )  AND SPIOH.Attribute1 IS NULL AND 
