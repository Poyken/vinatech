...
-- Create date : 2020-08
-- =========================================================================================================
CREATE PROC [dbo].[usp_Vietnam_Day_Total_get]
...
--for Vietnam only because Manual Lines have PLAN LineCode <> PRODUCTION lineCode       usp_VVT_Day_Total_get '','','2020-08-27', '', 'VVT'
		--select @ToDay		 = SUBSTRING(CONVERT(VARCHAR(10), GetDate(), 121), 9, 2)  
		--select @Month        = CONVERT(VARCHAR(6), GETDATE(), 112) 

DECLARE @currentdate datetime = DATEADD(DAY,0,getdate()	)
DECLARE @jodate     varchar(10)= convert(varchar(10),CONVERT(datetime,@currentdate,120),120)
...
--	begin
--	     DECLARE @er varchar(500) = convert(varchar(10),(@currentdate),120) + ',' + ',' +  + ',' + @tmpdate + ',' + @jodatefrom + ',' + @jodateto + ',' + @FromDate + ',' + @ToDate ;
--		 raiserror (@er,16,1);
--	end
...
--begin
		...
		--	select @jodate = convert( varchar(10),DATEADD(MONTH,0, @jodate),120) 			 

		--end
		 
		--	select @FromDate   = CONVERT(VARCHAR(10), GetDate(), 120) + ' 10:29:59'    			  
		--	select @ToDate     = CONVERT(VARCHAR(10), DATEADD(DAY, 1, CONVERT(smalldatetime, GetDate())), 120) + ' 10:30:01'      
		

		 --select @jodate,@jodatefrom,@jodateto,@FromDate,@ToDate
		--return;		 
