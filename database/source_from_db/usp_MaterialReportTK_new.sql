...
-- =============================================
CREATE PROCEDURE [dbo].[usp_MaterialReportTK_new] -- exec usp_MaterialReportTK_new '2025-10-01','2025-10-29'
...
declare @FromDate varchar(19) =   convert(varchar(10),@pFromDate,120)           	+ ' 08:30:00' --  + ' 10:00:00'

declare @ToDate  varchar(19)  =   convert(varchar(10),dateadd(day,1,@pToDate),120)	+ ' 08:30:00' -- + ' 10:00:00'
...
if(@FromDate>='2023-01-01' and @FromDate<='2024-11-30') begin
	print 'a'
			;with
			InventoryMaterial as( -- tồn đầu kì được kiểm kê đầu năm 2023
				select materialcode, firstinventory from Stb_InventoryMaterialLiquidation FIM with (nolock) where [Period]='2023-01-01' and workcentercode='VVT_F1'
			),
...
else if(@FromDate>='2025-01-01') begin
		print 'b'
			;with
			InventoryMaterial as( -- tồn đầu kì được kiểm kê đầu năm 2023
				select materialcode, firstinventory from Stb_InventoryMaterialLiquidation FIM with (nolock) where [Period]='2025-01-01' and workcentercode='VVT_F1'
			),
			InputMaterialCode as ( -- lấy ra nhập vào kho nvl
					select  (case when mli.materialcode<> mm.MaterialCode then mli.materialcode else mm.MaterialCode end) MaterialCode
					  , mm.materialName, mm.MMExtText03 as NameHQ, mm.MMExtText04 as CodeHQ ,  --mm.MaterialUnit,
					...
					--end)  between '2025-01-01 10:00:00' and @daten   then StockQty  end as NhapDauKy1,
					end)  between '2025-01-01 08:30:00' and @daten   then StockQty  end as NhapDauKy1,
				...
				--	and		mli.MaterialLocationCode in ('ROH_VN_WH_01')
					and		ui.Workcentercode ='VVT_F1'

					and		mli.MaterialLocationCode NOT IN				-- Mr.Manh add 2025-12-08: Không tính lượng hàng nhập vào từ Groupware
															(	'W01BG',
																'W02BN',
																'W03BG',
																'W04BN',
			...
			(
					SELECT						
									 ...
									 ,LI.LineName, 
									 case 
										  when   MWIOH.SourceMaterialWarehouseCode in ('ROH_VN_WH','HOLDING_VN_WH') and MWIOH.TargetMaterialWarehouseCode in ('NUOC_NGOAI_VN','NOI_DIA_VN')	and MWIOH.linecode in ('TVNCC')	then 'Tralaincc' -- trả lại nhà cc

										  when   MWIOH.SourceMaterialWarehouseCode in ('ROH_VN_WH','HOLDING_VN_WH') and MWIOH.TargetMaterialWarehouseCode='NG_RAW_VN_WH' and MWIOH.linecode in ('XTH')   then 'XuatTieuHuy' -- xuất tiêu hủy

										  when   MWIOH.SourceMaterialWarehouseCode in ('ROH_VN_WH','HOLDING_VN_WH') and MWIOH.TargetMaterialWarehouseCode='ROH_WH'		and MWIOH.linecode in ('R-KR')		then 'ReturnVinatech' -- trả lại công ty mẹ
										
										  when WarehouseInOutCode='O' and SourceMaterialWarehouseCode in ('ROH_VN_WH','HOLDING_VN_WH')   and MWIOH.TargetMaterialWarehouseCode='ROUTE_VN_WH' and  MWIOH.linecode='XK' then 'Xuat khac'
										
										  when WarehouseInOutCode='O' 
										  and SourceMaterialWarehouseCode in ('ROH_VN_WH','HOLDING_VN_WH')  
										  and MWIOH.TargetMaterialWarehouseCode IN ('ROUTE_VN_WH','MODULE_VN_WH')    
										  and MWIOH.linecode not in ('R-KR','XCDMDSD','XTH','XK','KVHD','TVNCC')
										  then'Xuat san xuat'
										 
										  when WarehouseInOutCode='O'
										  and MWIOH.SourceMaterialWarehouseCode='ROH_VN_WH'
										  and MWIOH.TargetMaterialWarehouseCode='ROH_BG_WH'
										  and MWIOH.linecode ='BG'
										  then 'Xuat BG'

										  -- Mr.Manh update for Module BG
										  when WarehouseInOutCode='O'
										  and MWIOH.SourceMaterialWarehouseCode='ROH_VN_WH'
										  and MWIOH.TargetMaterialWarehouseCode='MODULE_BG_WH'
										  and MWIOH.linecode ='VVBGMDL'
										  then 'Xuat BG'
										  -- END UPDATE
										  ...
										  and TargetMaterialWarehouseCode in ('ROH_VN_WH','HOLDING_VN_WH') 
										  and MWIOH.SourceMaterialWarehouseCode<>'HOLDING_VN_WH' 
										  and MWIOH.SourceMaterialWarehouseCode<>'ROH_BG_WH'  
										  then 'Nhap Lai Tu SanXuat' 

										  /*
										  when WarehouseInOutCode='O' 
										  and SourceMaterialWarehouseCode in ('ROH_BG_WH','HOLDING_BG_WH')  
										  and MWIOH.TargetMaterialWarehouseCode in ('ROH_VN_WH','HOLDING_VN_WH')  
										  and MWIOH.linecode ='BN'
										   then 'Nhap lai tu BG'
										   */
						...
						FROM            STB_MaterialWarehouseInOutHist MWIOH with(nolock)  
								 ...
								 WHERE 1=1	   
								   ...
								   And (MWIOH.WorkCenterCode = 'VVT_F1') 
								   and  (isnull(MWIOH.ProcessedLotID,'') <> '' )--or MWIOH.SourceMaterialWarehouseCode = 'HOLDING_VN_WH' or MWIOH.TargetMaterialWarehouseCode = 'HOLDING_VN_WH') 
								   --and  MWIOH.CreateDateTime >= '2025-01-01 10:00:00'
								   and  MWIOH.CreateDateTime >= '2025-01-01 08:30:00'
					...
					SELECT						
									 ...
									 case 
										  ...
										  and MWIOH.linecode ='BN'
										   then 'Nhap lai tu BG'
										  --when WarehouseInOutCode='O' and SourceMaterialWarehouseCode='ROH_VN_WH' and MWIOH.TargetMaterialWarehouseCode not in ('HOLDING_VN_WH') then 'Xuat_khac'
										  when WarehouseInOutCode='O' and SourceMaterialWarehouseCode<>TargetMaterialWarehouseCode then 'XUAT_KHO_AO_BG'
										  end as typeOut 
						...
						FROM            STB_MaterialWarehouseInOutHist MWIOH with(nolock)  
								 ...
								 WHERE 1=1	   
								   And ( MWIOH.CompanyCode = 'VVT') 
								   And (MWIOH.WorkCenterCode = 'VVT_F2') 
								   and  (isnull(MWIOH.ProcessedLotID,'') <> '' )
								   and MWIOH.SourceMaterialWarehouseCode = 'ROH_BG_WH' 
								   and MWIOH.TargetMaterialWarehouseCode = 'ROH_VN_WH' 
								   --and  MWIOH.CreateDateTime >= '2025-01-01 10:00:00'
								   and  MWIOH.CreateDateTime >= '2025-01-01 08:30:00'
			),
			getExportMaterialCode as(
				select  a1.materialcode, a1.materialName,a1.MMExtText03 as NameHQ,a1.MMExtText04 as CodeHQ,a1.MaterialUnit,
						...
						*/
						-- Các tính toán từ 2023-12-05 -> đến ngày bắt đầu tìm kiếm
						 NhapDauKy1,
						--case when a3.outdate between '2025-01-01 10:00:00' and @daten and typeOut = 'Tralaincc' then isnull(a3.OutQty,0) end as Tralaincc1,
						--case when a3.outdate between '2025-01-01 10:00:00' and @daten  and typeOut = 'XuatTieuHuy' then isnull(a3.OutQty,0) end as XuatTieuHuy1,
						--case when a3.outdate between '2025-01-01 10:00:00' and @daten  and typeOut = 'ReturnVinatech' then isnull(a3.OutQty,0) end as ReturnVinatech1,
						--case when a3.outdate between '2025-01-01 10:00:00' and @daten  and typeOut = 'Xuat khac' then isnull(a3.OutQty,0) end as Xuat_khac1,
						--case when a3.outdate between '2025-01-01 10:00:00' and @daten and typeOut = 'Xuat san xuat' then isnull(a3.OutQty,0) end as XuatSanXuat1,
						--case when a3.outdate between '2025-01-01 10:00:00' and @daten and typeOut = 'Xuat BG' then isnull(a3.OutQty,0) end as XuatBG1,
						...
						--case when a3.outdate between '2025-01-01 10:00:00' and @daten and typeOut = 'Nhap Lai Tu SanXuat' then isnull(a3.OutQty,0) end as NhaplaiSX1,
						--case when a3.outdate between '2025-01-01 10:00:00' and @daten and typeOut = 'Nhap lai tu BG' then isnull(a3.OutQty,0) end as NhaplaiBG1,
						--case when a3.outdate between '2025-01-01 10:00:00' and @daten and typeOut = 'XUAT_KHO_AO' then isnull(a3.OutQty,0) end as XUAT_KHO_AO1,
						--case when a3.outdate between '2025-01-01 10:00:00' and @daten and typeOut = 'XuatSai' then isnull(a3.OutQty,0) end as XuatSai1,

						case when a3.outdate between '2025-01-01 08:30:00' and @daten and typeOut = 'Tralaincc' then isnull(a3.OutQty,0) end as Tralaincc1,
						case when a3.outdate between '2025-01-01 08:30:00' and @daten  and typeOut = 'XuatTieuHuy' then isnull(a3.OutQty,0) end as XuatTieuHuy1,
						case when a3.outdate between '2025-01-01 08:30:00' and @daten  and typeOut = 'ReturnVinatech' then isnull(a3.OutQty,0) end as ReturnVinatech1,
						case when a3.outdate between '2025-01-01 08:30:00' and @daten  and typeOut = 'Xuat khac' then isnull(a3.OutQty,0) end as Xuat_khac1,
						case when a3.outdate between '2025-01-01 08:30:00' and @daten and typeOut = 'Xuat san xuat' then isnull(a3.OutQty,0) end as XuatSanXuat1,
						case when a3.outdate between '2025-01-01 08:30:00' and @daten and typeOut = 'Xuat BG' then isnull(a3.OutQty,0) end as XuatBG1,
						--case when a3.outdate between '2023-12-03 10:00:00' and @daten  and typeOut = 'To_Holding' then isnull(a3.OutQty,0) end as To_Holding1,
						--case when a3.outdate between '2023-12-03 10:00:00' and @daten and typeOut = 'Holding_Back' then isnull(a3.OutQty,0) end as Holding_Back1,
						case when a3.outdate between '2025-01-01 08:30:00' and @daten and typeOut = 'Nhap Lai Tu SanXuat' then isnull(a3.OutQty,0) end as NhaplaiSX1,
						case when a3.outdate between '2025-01-01 08:30:00' and @daten and typeOut = 'Nhap lai tu BG' then isnull(a3.OutQty,0) end as NhaplaiBG1,
						case when a3.outdate between '2025-01-01 08:30:00' and @daten and typeOut = 'XUAT_KHO_AO' then isnull(a3.OutQty,0) end as XUAT_KHO_AO1,
						case when a3.outdate between '2025-01-01 08:30:00' and @daten and typeOut = 'XuatSai' then isnull(a3.OutQty,0) end as XuatSai1,
			...
			CalculatorInventoryMaterial as (
				...
				select AA.MaterialCode, AA.MaterialName,AA.NameHQ,AA.CodeHQ,AA.MaterialUnit,
				--case  when @FromDate = '2025-01-01 10:00:00'  then COALESCE(max(openninginventory),0)
				--	  when  @FromDate > '2025-01-01 10:00:00'  
				case  when @FromDate = '2025-01-01 08:30:00'  then COALESCE(max(openninginventory),0)
					  when  @FromDate > '2025-01-01 08:30:00'  
					  then 
						COALESCE(max(openninginventory),0) + COALESCE(max(NhapDauKy1),0)+COALESCE(sum(NhaplaiBG1),0) +COALESCE(sum(NhaplaiSX1),0) 
						- COALESCE(sum(Tralaincc1),0) - COALESCE(sum(XuatTieuHuy1),0)  - COALESCE(sum(ReturnVinatech1),0) - COALESCE(sum(Xuat_khac1),0)  
						- COALESCE(sum(XuatSanXuat1),0) - COALESCE(sum(XuatBG1),0)  - COALESCE(sum(XUAT_KHO_AO1),0)
					  else 
							N'2025-01-01'
					  end
					  as TonDauKy,
				...
				-- update 2025-10-29
				--case  when @FromDate = '2025-10-29 10:00:00' then COALESCE(max(openninginventory),0) +COALESCE(max(NhapTrongKy),0) +COALESCE(sum(NhaplaiBG),0)  +COALESCE(sum(NhaplaiSX),0) 
				---COALESCE(sum(XuatBG),0) - COALESCE(sum(XuatSanXuat),0)
				--	  when  @FromDate > '2025-01-01 10:00:00'  then 
				case  when @FromDate = '2025-10-29 08:30:00' then COALESCE(max(openninginventory),0) +COALESCE(max(NhapTrongKy),0) +COALESCE(sum(NhaplaiBG),0)  +COALESCE(sum(NhaplaiSX),0) 
				-COALESCE(sum(XuatBG),0) - COALESCE(sum(XuatSanXuat),0)
					  when  @FromDate > '2025-01-01 08:30:00'  then 
						COALESCE(max(openninginventory),0) + COALESCE(max(NhapDauKy1),0)+COALESCE(sum(NhaplaiBG1),0) +COALESCE(sum(NhaplaiSX1),0) 
						+COALESCE(max(NhapTrongKy),0)+COALESCE(sum(NhaplaiBG),0) +COALESCE(sum(NhaplaiSX),0) 
						- COALESCE(sum(Tralaincc1),0) - COALESCE(sum(XuatTieuHuy1),0)  - COALESCE(sum(ReturnVinatech1),0) - COALESCE(sum(Xuat_khac1),0)  
						- COALESCE(sum(XuatSanXuat1),0) - COALESCE(sum(XuatBG1),0)  - COALESCE(sum(XUAT_KHO_AO1),0)
						- COALESCE(sum(Tralaincc),0) - COALESCE(sum(XuatTieuHuy),0)  - COALESCE(sum(ReturnVinatech),0) - COALESCE(sum(Xuat_khac),0)  
						- COALESCE(sum(XuatSanXuat),0) - COALESCE(sum(XuatBG),0)  - COALESCE(sum(XUAT_KHO_AO),0)
					  else 
						N'2025-01-01'
					  end
					  as TonCuoi 					 
			...
			select * from CalculatorInventoryMaterial BB
				where (TonDauKy > 0 or NhapTrongKy > 0  or xuatsanxuat > 0 or XuatBG > 0 or NhaplaiBG > 0 or TonCuoi>0  ) 
		...
		begin 
			...
			end)  between '2022-01-01 08:30:00' and @daten   then StockQty  end as NhapDauKy1,
			...
			--					 end
			--					 as TonDauKy,
			case  when @FromDate = '2022-01-01 08:30:00'  then COALESCE(max(openninginventory),0)
				  when  @FromDate > '2022-01-01 08:30:00'  then COALESCE(max(openninginventory),0) + COALESCE(max(NhapDauKy1),0)+ COALESCE(max(NhapDauKy2),0) - COALESCE(sum(XuatTieuHuy1),0) - COALESCE(sum(XuatKhac1),0) - COALESCE(sum(XuatSanXuat1),0)   - COALESCE(sum(TaiXuat1),0) /*- COALESCE(sum(XUAT_KHO_AO),0)*/
								 else COALESCE(max(NhapDauKy),0) - COALESCE(sum(XuatTieuHuyDauKy),0) - COALESCE(sum(XuatKhacDauKy),0) - COALESCE(sum(XuatSanXuatDauKy),0) - COALESCE(sum(TaiXuatDauKy),0) /*- coalesce(sum(XUAT_KHO_AO_DAU_KY),0) */
								 end
								 as TonDauKy,
			...
			--					 end
			--					 as TonCuoi 		
			case  when @FromDate = '2022-01-01 08:30:00' then COALESCE(max(openninginventory),0) +COALESCE(max(NhapTrongKy),0) /*+ COALESCE(sum(Holding_Back),0) - COALESCE(sum(To_Holding),0)*/ -COALESCE(sum(XuatTieuHuy),0) -COALESCE(sum(XuatKhac),0) - COALESCE(sum(XuatSanXuat),0) - COALESCE(sum(TaiXuat),0) /*-  COALESCE(sum(XUAT_KHO_AO),0)*/ 
				  when  @FromDate > '2022-01-01 08:30:00'  then COALESCE(max(openninginventory),0) + COALESCE(max(NhapDauKy1),0) + COALESCE(max(NhapDauKy2),0) - COALESCE(sum(XuatTieuHuy1),0) - COALESCE(sum(XuatKhac1),0) - COALESCE(sum(XuatSanXuat1),0) - COALESCE(sum(TaiXuat1),0) /*-  COALESCE(sum(XUAT_KHO_AO),0)*/	  +COALESCE(max(NhapTrongKy),0) /*+ COALESCE(sum(Holding_Back),0) - COALESCE(sum(To_Holding),0)*/ -COALESCE(sum(XuatTieuHuy),0) -COALESCE(sum(XuatKhac),0) - COALESCE(sum(XuatSanXuat),0) - COALESCE(sum(TaiXuat),0) /*-  COALESCE(sum(XUAT_KHO_AO),0)*/
				  else COALESCE(max(NhapDauKy),0) - COALESCE(sum(XuatTieuHuyDauKy),0) - COALESCE(sum(XuatKhacDauKy),0) - COALESCE(sum(XuatSanXuatDauKy),0) - COALESCE(sum(TaiXuatDauKy),0) /*-coalesce(sum(XUAT_KHO_AO_DAU_KY),0) */
								 +COALESCE(max(NhapTrongKy),0) /*+ COALESCE(sum(Holding_Back),0) - COALESCE(sum(To_Holding),0)*/ -COALESCE(sum(XuatTieuHuy),0) -COALESCE(sum(XuatKhac),0) - COALESCE(sum(XuatSanXuat),0) - COALESCE(sum(TaiXuat),0) /*-  COALESCE(sum(XUAT_KHO_AO),0) */
								 end
								 as TonCuoi 		
			...
			(select  a1.materialcode, a1.materialName,a1.MMExtText03 as NameHQ,a1.MMExtText04 as CodeHQ,a1.MaterialUnit,
					...
					case when a3.outdate between '2022-01-01 08:30:00' and @daten  and typeOut = 'Xuat chuyen doi, tieu huy' then isnull(a3.OutQty,0) end as XuatTieuHuy1,
					case when a3.outdate between '2022-01-01 08:30:00' and @daten and typeOut = 'Xuat khac' then isnull(a3.OutQty,0) end as XuatKhac1,
					case when a3.outdate between '2022-01-01 08:30:00' and @daten and typeOut = 'Xuat san xuat' then isnull(a3.OutQty,0) end as XuatSanXuat1,
					case when a3.outdate between '2022-01-01 08:30:00' and @daten  and typeOut = 'Tai Xuat' then isnull(a3.OutQty,0) end as TaiXuat1,
					case when a3.outdate between '2022-01-01 08:30:00' and @daten  and typeOut = 'XUAT_KHO_AO' then isnull(a3.OutQty,0) end as XUAT_KHO_AO1,
...
END
 ...
 -- exec usp_MaterialReportTK_new '2025-11-01','2025-11-30'
