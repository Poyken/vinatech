...
-- =============================================
CREATE PROCEDURE [dbo].[usp_VN_ShowAllFinishGoodMES_forQCAudit_iud]
	...
	--select * from STB_VN_FINISHGOODS
	  --STB_VN_FINISHGOODS
		 ...
		 EXEC SmartFramework.dbo.usp_GetSerialRule 
			@pTableName = 'STB_VN_FINISHGOODS_forQCAudit',
  ...
  IF @IsAutoKey = 0 AND @IsLoopIUD = 0 BEGIN
   ...
   BEGIN TRY
   	-- Process Insert Table
			 MERGE STB_VN_FINISHGOODS_forQCAudit
			 AS TargetTable
			USING
				(
					SELECT
					...
					FROM
							...
							WITH  (
										...
										FGLocation NVARCHAR(30),
										StatusCheck NVARCHAR(20),
			...
			WHEN MATCHED THEN
				...
				UPDATE SET
					...
					FGLocation = SourceTable.FGLocation,
					StatusCheck = SourceTable.StatusCheck,
...
WHEN NOT MATCHED THEN
				...
				VALUES
					(
							...
							SourceTable.FGLocation,
							SourceTable.StatusCheck,
							SourceTable.CreateDateChange,
							SourceTable.USERIDChange
					);
-- Process Update Table
	 MERGE STB_VN_FINISHGOODS_forQCAudit AS TargetTable
			USING
				(
					SELECT
							CASE
...
SELECT
	   ...
	   WHILE 1 = 1 BEGIN
		  ...
		  IF @IUD_FLAG = 'INSERT' BEGIN
                    ...
                    IF EXISTS (SELECT 1 FROM STB_VN_FINISHGOODS_forQCAudit WHERE ID = @OldCompanyCode) BEGIN
						RAISERROR('Duplicate Data : KeyField = %s', 16, 1, @OldCompanyCode)
					END
			 IF @IsAutoKey = 0 BEGIN
						EXEC SmartFramework.dbo.usp_DoCreateSerial 'STB_VN_FINISHGOODS_forQCAudit', @OldCompanyCode OUTPUT
...
UPDATE STB_VN_FINISHGOODS_forQCAudit
			...
			Descrption =   CASE
					...
					StatusSystem =   CASE
						...
						MaterialCode =   CASE
							...
							ProductionSize =   CASE
									...
									FGLocation =   CASE
						                WHEN @FGLocation IS NOT NULL THEN @FGLocation
						                ELSE FGLocation
						            END,

									StatusCheck =   CASE
						                WHEN @StatusCheck IS NOT NULL THEN @StatusCheck
						                ELSE StatusCheck
						            END
					...
					WHERE
                        ...
                        DELETE FROM STB_VN_FINISHGOODS_forQCAudit
						WHERE
						    ID = ''
                    END
                END
