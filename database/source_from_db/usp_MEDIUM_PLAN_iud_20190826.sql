...
-- =============================================
Create PROCEDURE [dbo].[usp_MEDIUM_PLAN_iud_20190826]
		   ...
		   ----                      ELSE ISNULL(SourceTable.월간계획,TargetTable.월간계획) END 월간계획,
			...
			USING
				(
					SELECT
							...
							END AS Old사이즈,
							CASE
							    WHEN Old공정코드 IS NULL THEN 공정코드
							    ELSE Old공정코드
							END AS Old공정코드,
							기준년월,
							사이즈,
			...
			ON
				(
					...
					TargetTable.공정코드 = SourceTable.Old공정코드
				)
			...
			ON
				(
					...
					TargetTable.공정코드 = SourceTable.공정코드
				)
        ...
        BEGIN TRY
			...
			BEGIN
                ...
                IF @IUD_FLAG = 'INSERT' BEGIN
                    ...
                    IF EXISTS (SELECT 1 FROM MEDIUM_PLAN WHERE 기준년월 = @기준년월 AND 사이즈 = @사이즈 AND 공정코드 = @공정코드) BEGIN
