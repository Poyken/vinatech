...
-- Modified:
--  2020.07.01 수입검사 샘플수량 자동반영전 (박진호 대리)
-- =============================================
CREATE PROCEDURE [dbo].[usp_DoMakeMaterialIQCDetailList_20200701]
    ...
    BEGIN
        BEGIN TRY		
			...
			WHILE 1 = 1 BEGIN
						...
						(
							...
							FROM
									...
									LEFT OUTER JOIN STB_QcInspectionGroup IIG
										ON (MIG.QcInspectionGroupCode = IIG.QcInspectionGroupCode)
									LEFT OUTER JOIN STB_QcInspectionItem III
										ON (MIG.QcInspectionGroupCode = III.QcInspectionGroupCode)
							WHERE
							...
							FROM
									...
									LEFT OUTER JOIN STB_QcInspectionItem III
										ON (MIG.QcInspectionGroupCode = III.QcInspectionGroupCode)
									LEFT OUTER JOIN STB_MaterialQcInspectionItem MII
										ON (MIG.MaterialCode = MII.MaterialCode AND
											III.QcInspectionItemCode = MII.QcInspectionItemCode)
							WHERE
									III.QcInspectionItemCode = @QcInspectionItemCode AND
									MIG.MaterialCode = @MaterialCode AND
				...
				IF @SampleQty <> 0 BEGIN
					exec usp_DoCreateMaterialQcSampleResult @MaterialQcNo, @MaterialQcDetailNo, @SampleQty, 0
				END
