...
-- 2021.03.20 Lot Number도 추가 (최은화)
-- Exec usp_DoSendLineMessageForDefectReport 'kilee' , 'Korean', 'VN210320-01'
-- =============================================
CREATE PROCEDURE [dbo].[usp_DoSendLineMessageForDefectReport]
...
BEGIN
	Declare @DefectReportNo VARCHAR(20) = @pDefectReportNo
		...
		--   , @Spec VARCHAR(20)
		, @LotNo VARCHAR(20) -- 2021.03.20 추가
		, @QcDefectClassCode VARCHAR(10)
		, @TitleText NVARCHAR(100)
	...
	SELECT @DefectDivisionName = BC1.Description
			...
			--  ,@PublishName = isnull(EI1.EmployeeName, QDR.PublishEmpID)         --발행자, 2020.11.04 추가
			 , @PublishName = isnull(EI1.EmployeeName, isnull((select top 1 WorkerName from [SmartFactoryV2].[dbo].[STB_ProdWorkerInfo] where WorkerCode=QDR.PublishEmpID),QDR.PublishEmpID) )   
			 -- ,@Spec = VW.MBIExtText04  + '_' + VW.MBIExtText05   
			  ,@LotNo = QDR.LotNo                                                                  --2021.03.20 추가 (최은화)
	...
	IF @QcDefectClassCode = '01' BEGIN
		SET @TitleText = '부적합보고서가 발행되었습니다'
	END ELSE BEGIN
		SET @TitleText = '공정이상품보고서가 발행되었습니다'
	END

	SET @LineMessage = @TitleText + ' / [부적합구분] : ' + ISNULL(@DefectDivisionName,'')
...
END
 ...
 -- select * from STB_QcDefectReport where DefectReportNo = 'VN210320-01'