...
-- =============================================
CREATE PROCEDURE [dbo].[usp_DayMaterialOrderAndAdditional_NotExported_TEST1]
	...
	SET NOCOUNT ON;
		...
		DECLARE @PoNo VARCHAR(30) = @pPoNo
		 --raiserror(@pPoNo,16,1)
				...
				ha as (
				 ...
				 test as (
					 ...
					 WHERE 1=1	   
					   ...
					   And MWIOH.CreateDateTime  BETWEEN '2024-12-02 00:00:00' AND '2024-12-30 23:59:59'
						...
						--or (MWIOH.SourceMaterialWarehouseCode like 'ROH_BG_WH%' and MWIOH.TargetMaterialWarehouseCode like 'ROUTE_BG_WH') 
							...
							select materialorderno ,linecode,MaterialCode, isnull(max(ExportChangeTime),'2024-12-02 10:00:00') as ExportChangeTime , StatusWarehouseConfirm from ha
				...
				, b2 as (
							 select 
							 a.materialorderno , b.MaterialCode AS MaterialCodeB, b.linecode, sum(b.OutQty) as OutQty, max(a.ExportChangeTime) as ExportChangeTime , max(b1.ExportChangeTime1) as ExportChangeTime2,a.StatusWarehouseConfirm from b1  a 
						...
						where b.createdatetime >=ExportChangeTime1 AND StatusWarehouseConfirm =1
							group by  a.materialorderno , b.MaterialCode, b.linecode, a.StatusWarehouseConfirm
							--order by ExportChangeTime desc
							 --left join b1 b on a.MaterialCode = b.MaterialCode and a.LineCode = b.LineCode
							 --where 
				)
				
				 /*select *,b.OutQty from ha a
					left join b2 b with(nolock)  on a.linecode = b.linecode and a.ChildMaterialCode = b.MaterialCode 
				where a.linecode = 'VVC-16'
				*/
				select * from b2 where linecode='vvc-16'



				
				 
				--select a.* , b.* from ha  a
				-- LEFT   JOIN test b  with(nolock)  on a.linecode = b.linecode and a.ChildMaterialCode = b.MaterialCode 
				--	 --and CAST(a.Plandate AS DATE)   =b.ngayxuat
				--where  a.linecode = 'VVC-16'
END
----       usp_DayMaterialOrderAndAdditional_NotExported_TEST1 '','','VVT','VVT_F1','2024-11-26','2024-12-26','','','' 
