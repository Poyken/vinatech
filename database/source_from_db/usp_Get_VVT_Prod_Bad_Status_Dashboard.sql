...
CREATE PROCEDURE [dbo].[usp_Get_VVT_Prod_Bad_Status_Dashboard]
...
X as(
		select  
		 ...
		 li.LineName, pwi.WorkerName,mm.MachineName,
		  RV.ProdDateTime	
		 , (case  when   (DATEPART(HOUR, ProdDateTime)>10)     or (DATEPART(HOUR, ProdDateTime)=10 and DATEPART(MINUTE, ProdDateTime)>0)  --update time here
	 ...
	 STB_Defect_VVT as (
	    SELECT --ROW_NUMBER() OVER(ORDER BY X.DefectQty DESC) AS ROWNUM
			...
			,  case when di.DefectEnglishName IS NOT NULL then di.DefectEnglishName 
			        when di.DefectEnglishName IS NULL AND di.BasicDefectName = di.DefectDesc then di.DefectDesc 
			        else di.BasicDefectName  +'_'+ di.DefectDesc end AS 불량명 
