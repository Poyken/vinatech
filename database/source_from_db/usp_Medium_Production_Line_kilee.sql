...
-- [프로시저 실행문]  
--   Exec usp_Medium_Production_Line_kilee @pProcessUserID='kilee',@pProcessLanguage='Korean',@pMonth='2019-10-20',@pCompanyCode = VNT, @pSizeCode='1840', @pRouteCode= 'E-28'

CREATE PROC [dbo].[usp_Medium_Production_Line_kilee] 		
...
---------------------------------- 세부현황부분
		   ...
		   SELECT 사이즈
		      --  , 라인코드
				...
				, CASE WHEN SUM(총합) = 0 THEN NULL ELSE SUM(총합) END 합계      
			, SUM(Day26) AS Day26, SUM(Day27) AS Day27 , SUM(Day28) AS Day28, SUM(Day29) AS Day29, SUM(Day30) AS Day30
						, SUM(Day31) AS Day31
		...
		FROM (
					...
					--	 AND ((@CompanyCode = '*') OR (CompanyCode = @CompanyCode))   
					  ...
					  WHERE 1=1																									 				  
						...
						--AND 사이즈 = '1840'                                                                                                        -- TEST용
						AND ((@CompanyCode = '*') OR (CompanyCode = @CompanyCode))   
						GROUP BY  A.사이즈,  A.RouteCode--, A.LineCode
