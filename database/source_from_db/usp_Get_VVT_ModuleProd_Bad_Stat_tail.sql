...
-- =============================================
CREATE PROCEDURE [dbo].[usp_Get_VVT_ModuleProd_Bad_Stat_tail]
...
if @CompanyCode='VVT' begin
	...
	select @FromDate   = CONVERT(VARCHAR(10), @pFromDate, 120) + ' 10:30:00'                                                            -- SELECT  CONVERT(VARCHAR(10), '2019-09-16', 121) + ' 08:30:00' 
...
;with ViewBarcode as (
		 ...
		 where b.CompanyCode='VVT'  and b.ProdDateTime>@FromDate  and b.ProdDateTime<@ToDate and  SUBSTRING (Barcode, 1, 1) ='M'
	 ),
RawView as (
		...
		from  STB_SetInfo c with(nolock) 
		 ...
		 group by c.Barcode,b.RouteCode,c.ControlNo,c.MaterialCode,InputLineCode,b.MachineCode,b.WorkerCode,SIExtText07,SIExtInt01,b.CreateDateTime,shiftcode,c.PONo
)
,
--AlloyView as (
--select RawView.* --, 
...
X as(
		select  
		 RV.Barcode,RV.RouteCode, RV.RouteCode as FindRouteCode ,ControlNo,RV.MaterialCode,RV.InputLineCode as LineCode,RV.MachineCode,RV.WorkerCode,RV.SIExtText07,RV.SIExtInt01,shiftcode,PONo,
		 ri.RouteName, 
		 MM2.MaterialName,
		...
		group by 		 RV.Barcode,RV.RouteCode,ControlNo,RV.MaterialCode,RV.InputLineCode,RV.MachineCode,RV.WorkerCode,RV.SIExtText07,RV.SIExtInt01,shiftcode,PONo,
