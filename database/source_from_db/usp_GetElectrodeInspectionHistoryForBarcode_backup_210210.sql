...
CREATE PROCEDURE [dbo].[usp_GetElectrodeInspectionHistoryForBarcode_backup_210210]
...
BEGIN
	...
	SELECT A.Barcode
			  ...
			  ,A.IsHolding
			  ,A.DisplayIndex
			  ,(CONVERT(NUMERIC(10, 5), A.FirstMeasureValue) + CONVERT(NUMERIC(10, 5), A.SecondMeasureValue) + CONVERT(NUMERIC(10, 5), A.ThirdMeasureValue)) / 3 AS ElectrodeSampleWeightAvgValue   -- 전극두께
			  ,dbo.fnGetElectrodeDensity (A.CommInspDocNo, A.DisplayIndex)   AS ElectrodeDensityValue    -- 밀도
			  ,A.RollingDensityMin
			  ,A.RollingDensityMax
			  ,A.MaterialCode 
			  ,A.CIDHExtText02
