...
-- Browsable : true
-- Group : 자재관리
-- Description:	자재발주전표 CANCEL
-- Modified: 
-- =============================================
CREATE PROCEDURE [dbo].[usp_DoChangeCancelMaterialOrder]
        ...
        BEGIN TRY
            ...
            WHILE 1 = 1 BEGIN
                ...
                IF @IUD_FLAG = 'UPDATE' BEGIN
					...
					--발주전표 마감상태인 경우 발주전표취소 불가 
					IF @OrderStatus NOT IN ('REQUEST','FIX') BEGIN
						RAISERROR('Can not Change CANCEL, Current OrderStatus : %s ',16,1,@OrderStatus)
					END
					
					
					--선택된 발주서에 해당하는 납품서 존재여부 확인('납품취소되지 않은 납품서 존재시 발주전표취소 불가')
					--EXEC usp_GetMaterialDeliveryCurrentStatusIsCancelMaterialOrder @MaterialOrderNo
					...
					SET
							OrderStatus = 'CANCEL',
							IsAllCancel = 1,
							AllCencelUserID = @pProcessUserID,
					...
					WHERE
							MrpTargetNo IN (SELECT MrpTargetNo FROM STB_MaterialOrderItem WHERE MaterialOrderNo = @OldMaterialOrderNo)
