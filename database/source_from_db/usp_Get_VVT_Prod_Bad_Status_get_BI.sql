...
-- Description:	<Gộp chung dữ liệu 2 nhà máy VVT_F1,VVT_F2 đẩy lên BI>
-- EXEC usp_Get_VVT_Prod_Bad_Status_get_BI  '','','','VVT','','','','','2024-01-28','2024-01-30',''
-- =============================================
CREATE PROCEDURE [dbo].[usp_Get_VVT_Prod_Bad_Status_get_BI]
...
;with ViewBarcode as (
		 ...
		 where b.CompanyCode='VVT' and b.WorkCenterCode in ('VVT_F1','VVT_F2')  and isnull(b.ProdDateTime,b.CreateDateTime)>@FromDate 
...
RawView as (
 	select  c.Barcode,isnull(b.RouteCode,a.FindRouteCode) RouteCode,isnull(b.RouteCode,a.FindRouteCode) as FindRouteCode,c.ControlNo,c.MaterialCode,
		...
		from  STB_SetInfo c with(nolock) 
		 ...
		 and    (isnull(b.ProdDateTime,a.CreateDateTime)>@FromDate and b.WorkCenterCode in ('VVT_F1','VVT_F2')  and isnull(b.ProdDateTime,a.CreateDateTime)<@ToDate)
	...
	,
	 ...
	 SELECT  /* dbo.fnGetLocalTime(*/A.작업일자--,@pUtcOffset) AS 작업일자
				...
				case when A.작업일자<'2023-10-01' then ISNULL(vwup.ProcessUnitPriceEA,0) 
					when A.작업일자>='2023-10-01' then ISNULL(vwupNEW.ProcessUnitPriceEA,vwupNEW1.ProcessUnitPriceEA) 
					else  0  end   
					as     DefectWastePrice 
