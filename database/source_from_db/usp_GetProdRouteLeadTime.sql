...
-- Browsable : true
-- Group : 생산관리
-- Description: [B610] 공정별 리드타임을 가져옵니다
-- Modified: 라인변경
-- =============================================
CREATE PROCEDURE [dbo].[usp_GetProdRouteLeadTime]
...
BEGIN
	...
	FROM
			...
			INNER JOIN STB_ProductionOrderRouting POR WITH(NOLOCK)				ON POR.PONo = PRH.PONo AND				POR.RouteCode = PRH.RouteCode
			LEFT OUTER JOIN STB_ProductionOrderRouting BPOR WITH(NOLOCK)				ON BPOR.PONo = PRH.PONo AND				BPOR.RouteIndex = POR.RouteIndex - 1
			LEFT OUTER JOIN STB_ProdRouteHist BPRH WITH(NOLOCK)				ON BPRH.ControlNo = PRH.ControlNo AND				BPRH.RouteCode = BPOR.RouteCode
	...
	WHERE
			PRH.CompanyCode LIKE @CompanyCode 
			AND	PRH.WorkCenterCode LIKE @WorkCenterCode 
			AND	PRH.LineCode LIKE @LineCode 
			AND  PRH.MaterialCode LIKE @MaterialCode AND			(PRH.JobDate BETWEEN @FromDate AND @ToDate)
	GROUP BY
			SI.Barcode,
			PRH.MaterialCode,
			MM.MaterialName,
