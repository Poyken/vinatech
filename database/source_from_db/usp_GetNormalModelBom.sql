...
-- Description:	BOM 정전개
-- =============================================
CREATE PROCEDURE [dbo].[usp_GetNormalModelBom]
...
BEGIN
	...
	DECLARE @BomCTE TABLE (
		...
		SELECT 
				...
				RouteCode,
				--MM.MaterialUnit,
				A.ChildBomUnit AS BomUnit,
				UnitQty,
				UnitTotalQty,
				TotalQty,
				MM.MaterialUnit,
				CONVERT(NUMERIC(20,5), SmartFramework.dbo.fnConvertUnit(A.ChildBomUnit, MM.MaterialUnit, A.TotalQty)) AS MaterialUnitTotalQty,
				IsOptionItem,
				IsUseOption,
				IsDelegate,
				MM.IsUseBackFlush,
				...
				A.BasicCostPrice,
				TotalCostPrice,
				dbo.fnGetAccPrice(ChildMaterialCode, ChildBomVersion) AS AccCostPrice
		FROM
				(
					SELECT 
							BCTE.Levels,
