...
-- =============================================
Create PROCEDURE [dbo].[usp_DoMakeMaterialDocDetailLotForStocktaking_20200714]
...
--- 2020.07.14 패킹아이디 부분 추가 Start
		...
		-- DECLARE @MaterialCode VARCHAR(50)
			...
			DECLARE @Month INT = DATEPART(MONTH,@ProcessDateTime)  -- select DATEPART(MONTH,getdate()) 
			DECLARE @YearCode VARCHAR(1)
			DECLARE @MonthCode VARCHAR(1) = CHAR(@Month + 73)            -- select     CHAR(7 + 73)                                         	-- 1월이 J부터 시작
			DECLARE @DayCode VARCHAR(2) = RIGHT('00' + CONVERT(VARCHAR,DATEPART(DAY,@ProcessDateTime)),2)  -- select RIGHT('00' + CONVERT(VARCHAR,DATEPART(DAY,getdate())),2) 
...
----- 추가부분 End
			...
			(
				...
				--@LotID,
				@PackingID,
				--'PT' + @YearCode + @MonthCode + @DayCode + RIGHT('00000' + CONVERT(VARCHAR,@SerialNo),5),
				GETDATE(),
				@ProcessUserID
			)
	END
