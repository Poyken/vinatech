...
-- [프로시저 실행문]  usp_Medium_Production_Line_CEO   @pProcessUserID='kilee',@pProcessLanguage='Korean',@pMonth='2019-11-06', @pCompanyCode = '', @pSizeCode='0813',  @pRouteCode='E-28'
--                        usp_Medium_Production_Line_CEO   @pProcessUserID='kilee',@pProcessLanguage='Korean',@pMonth='2019-12-04', @pCompanyCode = 'VNT', @pSizeCode='',  @pRouteCode='E-28'

CREATE PROC [dbo].[usp_Medium_Production_Line_CEO] 		
...
BEGIN
	...
	DECLARE @SizeCode             VARCHAR(10) = CASE WHEN ISNULL(@pSizeCode, '') = '' THEN '%' ELSE @pSizeCode END
...
FROM (
		...
		WHERE 1=1									  
			AND 기준년월 = @Month
			AND ((@CompanyCode = '*') OR (CompanyCode = @CompanyCode))                                   -- 2019.12.11 최덕렬과장 체크
			...
			SELECT 
			   ...
			   WHERE 1=1																									 				  
				 ...
				 AND ((@CompanyCode = '*') OR (A.CompanyCode = @CompanyCode))   
				  GROUP BY  A.사이즈, A.CompanyCode, A.RouteCode
				  UNION  ALL				
               ...
               GROUP BY  A.사이즈, A.CompanyCode, A.공정코드
				...
				WHERE 1=1						
				  ...
				  AND ((@CompanyCode = '*') OR (A.CompanyCode = @CompanyCode))   
			   GROUP BY ROLLUP(A.사이즈, A.CompanyCode, A.공정코드)
