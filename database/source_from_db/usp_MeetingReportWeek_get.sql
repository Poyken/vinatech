...
-- Modified:
-- =============================================
CREATE  PROCEDURE [dbo].[usp_MeetingReportWeek_get]
...
SELECT @query=N'	select  a.*,SUMM from (select  *
	...
	(
		...
		--,(DATEPART(week, Date_Meeting) - DATEPART(week, DATEADD(day, 1, EOMONTH(Date_Meeting, -1)))) + 1 as Week,
		''Ratio'' AS status
		--, concat(CAST (sum(case when status IN (''OPEN'',''CLOSE'') then 1 else 0 end) as nvarchar(20)),'''') as  SUMM
...
--exec usp_MeetingReportWeek_get 'ngoloan','vi',2020,04,01