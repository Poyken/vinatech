...
-- [프로시저 실행문]   EXEC usp_Medium_Total '' , '' , '2019-12-22 00:00:00' , '1080'                                           
--                                  usp_Medium_Total  '', '', '2019-12-22 00:00:00', '', 'VVT','V-28'
--                           EXEC usp_Medium_Total_20200909  'kilee', 'Korean', '2020-09-09 00:00:00', '', 'VVT', 'V-28'


CREATE PROC [dbo].[usp_Medium_Total_20200909] 
...
BEGIN
	...
	DECLARE @Month               VARCHAR(6) = CONVERT(VARCHAR(6), @pMonth, 112)            --  Select CONVERT(VARCHAR(6), '2020-09', 112)  
	--DECLARE @Month               VARCHAR(6) = REPLACE(CONVERT(VARCHAR(6), @pMonth, 112), '-', '')         
	...
	FROM 
		...
		ON AA.기준년월 = BB.기준년월  AND AA.사이즈 = BB.사이즈 AND AA.RouteCode = BB.공정코드 
		     AND AA.CompanyCode = BB.CompanyCode 
			 AND AA.LineCode = BB.LineCode

	WHERE 1=1
		-- AND AA.기준년월 = '201911'                                                              -- TEST	   
