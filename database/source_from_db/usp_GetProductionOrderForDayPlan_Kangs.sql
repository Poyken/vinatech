...
-- EXEC [usp_GetProductionOrderForDayPlan] '','','','','','' , 'ASSYLINE-01' ,'2019-09-01' ,'2019-09-10'


CREATE PROCEDURE [dbo].[usp_GetProductionOrderForDayPlan_Kangs]
...
BEGIN
	...
	SELECT
			...
			(SELECT ISNULL(SUM(PlanQty), 0) FROM STB_DayProdPlan SP 
			  WHERE  SP.PONO = POI.PONO AND SP.MaterialCode = POI.MaterialCode AND SP.CreateDateTime Between Substring(CONVERT(VARCHAR(10), @FromDate, 121), 0, 12)   AND   Substring(CONVERT(VARCHAR(10), @ToDate, 121), 0, 12)
			   AND SP.PlanShiftCode = SC.ShiftCode
				AND SP.LineCode = @LineCode
			  )  AS New_PlanQty
			--(SELECT ISNULL(PlanQty, 0) FROM STB_DayProdPlan SP WHERE  SP.PONO = POI.PONO AND SP.MaterialCode = POI.MaterialCode AND SP.CreateDateTime Between Substring(CONVERT(VARCHAR(10), @FromDate, 121), 0, 12)   AND   Substring(CONVERT(VARCHAR(10), @ToDate, 121), 0, 12))  AS PlanQty

			--(SELECT ISNULL(SUM(PlanQty), 0) FROM STB_DayProdPlan SP WHERE  SP.PONO = POI.PONO AND SP.MaterialCode = POI.MaterialCode  AND SP.CreateDateTime Between Substring(CONVERT(VARCHAR(10), GetDate()-1, 121), 0, 12)   AND   Substring(CONVERT(VARCHAR(10), GetDate(), 121), 0, 12))  AS NNN_PlanQty
	FROM
