...
-- Browsable : True
-- Group : 생산관리 > 생산현황관리 > 생산현황 > [B668] VPC일별공정별 생산현황
-- Description: [B668] 일별공정별설비별_생산현황!! (두번째 화면- 일반Grid)
-- Modified: Dynamic Grid용 (온정민)

-- 프로시저 실행 :                 usp_ProductByFacility  '','', '', '',''
-- ===============================================================================================
CREATE PROCEDURE [dbo].[usp_ProductByFacility]
...
BEGIN
	...
	DECLARE @FromDate DATETIME = '2022-02-26 08:30:00'
	DECLARE @ToDate DATETIME = '2022-03-25 08:30:00'
   ...
   SELECT CONVERT(CHAR(10), FIN.ProdDateTime, 120) AS ProdDateTime
	...
	--	, FIN.LotNo
		, Case When FIN.ShiftCode = 1 Then '주간' When FIN.ShiftCode = 2 Then '야간' Else '기타' End As ShiftCode
  -- INTO #ProdReportInfo
   FROM (
      SELECT   TOP 100000
                 PRH.RouteCode
			   ...
			   , SI.Barcode AS LotNo
			   , SI.InputShiftCode As ShiftCode
         FROM STB_SetInfo SI WITH(NOLOCK) 
               ...
               LEFT OUTER JOIN ( SELECT DRI2.ControlNo
										  ...
										  WHERE RepairType = 'NONE'
											...
											AND PRH2.ProdDateTime BETWEEN @FromDate AND @ToDate                             -- FromTo
   ...
   ORDER BY CONVERT(CHAR(10), FIN.ProdDateTime, 120) DESC
...
--	 -- 3. 수량으로 펼칠 데이터부분
   ...
   --      FROM #ProdReportInfo 	
   --UNPIVOT (
   --                PlanDate FOR ShiftCode IN ('1' , '2')
   --             )  AS unpvt


--  usp_ProductByFacility  'kilee2','Korean', 'E-22', '2022-03-02' ,'2022-03-07'
