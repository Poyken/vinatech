...
-- Modified :
-- =============================================
CREATE PROCEDURE [dbo].[usp_GetDeliveryHistory]
...
BEGIN
    ...
    DECLARE @ProcessUserID VARCHAR(20) = @pProcessUserID,
			...
			@ModelCode VARCHAR(20) = CASE WHEN ISNULL(@pModelCode,'') = '' THEN '%' ELSE @pModelCode END
	...
	FROM
			...
			LEFT OUTER JOIN VW_DocStatus DS				                        ON	DS.DocType = 'GI' AND DS.DocStatus = MDI.DocStatus
			LEFT OUTER JOIN STB_CustomerInfo CI WITH(NOLOCK)				ON	CI.CustomerCode = MDI.TargetCustomerCode
			LEFT OUTER JOIN STB_MaterialWarehouse MW WITH(NOLOCK)	ON	MW.MaterialWarehouseCode = MDI.SourceMaterialWarehouseCode
			LEFT OUTER JOIN STB_MaterialDocDetail MDD WITH(NOLOCK)		ON	MDD.MaterialDocNo = MDI.MaterialDocNo
	...
	WHERE 1=1
			AND  @FromDate <= MDI.BasicDate 
			AND	MDI.BasicDate <= @ToDate 
			AND	MDI.TargetCustomerCode LIKE @CustomerCode 
			AND	MDI.MaterialDocTypeCode = 'GI_SALES' 
			AND	MDI.DocStatus = 'FIX' 
			AND MDD.MaterialCode = @ModelCode
	ORDER BY
			MDI.MaterialDocNo
