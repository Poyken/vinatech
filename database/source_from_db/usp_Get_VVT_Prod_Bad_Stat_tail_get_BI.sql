...
-- Description:	<Gộp dữ liệu của màn hình B682 VVT_F1,VVT_F2 đẩy lên trên BI>
-- =============================================
-- EXEC usp_Get_VVT_Prod_Bad_Stat_tail_get_BI '','','','VVT','','','','','2024-01-01','2024-01-30',''
CREATE PROCEDURE [dbo].[usp_Get_VVT_Prod_Bad_Stat_tail_get_BI]
...
;with ViewBarcode as (
		 ...
		 where b.CompanyCode='VVT' and  b.WorkCenterCode in ('VVT_F1','VVT_F2') and  b.ProdDateTime>=@FromDate  and b.ProdDateTime<@ToDate and   SUBSTRING (c.Barcode, 1, 1) !='M' --and b.WorkCenterCode = @WorkCenterCode
	 ),
...
RawView as (
		...
		from  STB_SetInfo c with(nolock) 
		 ...
		 where  c.Barcode in (select  Barcode  from  ViewBarcode  with(nolock) ) and  b.WorkCenterCode in ('VVT_F1','VVT_F2') and b.ProdDateTime>=@FromDate  and b.ProdDateTime<@ToDate --and  b.WorkCenterCode = @WorkCenterCode
