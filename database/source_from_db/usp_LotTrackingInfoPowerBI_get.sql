...
-- 프로시저실행 (2021-03-12)  :   EXEC usp_LotTrackingInfoPowerBI_get 'VNT', '2020-01-01', '2021-03-29', '', '', '', '', 'LL'
-- ===========================================================================================================

CREATE PROCEDURE [dbo].[usp_LotTrackingInfoPowerBI_get]
...
BEGIN
	...
	select @FromDate    = CONVERT(VARCHAR(10), @pFromDate, 120) + ' 10:29:59'                                                           
	select @ToDate      = CONVERT(VARCHAR(10), DATEADD(DAY, 1, CONVERT(smalldatetime, @pToDate)), 120) + ' 10:30:01'      
	...
	select
		  ...
		  ,dbo.fnGetLocalTime (convert(datetime,DRI.ProdDateTime,120), 1170) as ProdDateTime
		  ,DRI.MachineCode
		  ,MM.MachineName
		  ,DRI.WorkerCode
		  ,PWI.WorkerName
