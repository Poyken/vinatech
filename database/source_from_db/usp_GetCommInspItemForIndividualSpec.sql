...
-- Browsable : true
-- Group : 공용검사관리
-- Description:	공용검사항목정보 조회(공용검사개별스펙화면용)
-- Modified:
-- =============================================

--usp_GetCommInspItemForIndividualSpec '','','ROUTE_TEST2_BG'
CREATE PROCEDURE [dbo].[usp_GetCommInspItemForIndividualSpec]
...
SELECT
	...
	FROM
			...
			LEFT OUTER JOIN STB_WorkCenterInfo WCI WITH(NOLOCK)
				ON WCI.WorkCenterCode = CII.WorkCenterCode
			LEFT OUTER JOIN STB_CompanyInfo CI WITH(NOLOCK)
				ON CI.CompanyCode = CII.CompanyCode
			LEFT OUTER JOIN STB_RouteInfo RI WITH(NOLOCK)
				ON RI.RouteCode = CII.RouteCode
			--LEFT OUTER JOIN STB_MoldProductMachine MPM WITH(NOLOCK)
			--	ON MPM.MachineCode = CII.MachineCode
			LEFT OUTER JOIN STB_MachineMaster MPM WITH(NOLOCK)
				ON MPM.MachineCode = CII.MachineCode
			LEFT OUTER JOIN STB_MoldBasicInfo MBI WITH(NOLOCK)
				ON MBI.MoldNumber = CII.MoldNumber
			LEFT OUTER JOIN STB_MaterialMaster MM WITH(NOLOCK)
				ON MM.MaterialCode = CII.MaterialCode
			LEFT OUTER JOIN STB_CommInspSelectGroup CISG WITH(NOLOCK)
				ON CII.CommInspSelectGroupCode = CISG.CommInspSelectGroupCode
			LEFT OUTER JOIN SmartFramework_File.dbo.STB_AttachedFileMaster AFM WITH (NOLOCK)
				ON (AFM.FileID = CII.ItemImageFileID)
			LEFT OUTER JOIN VW_CommInspInputType CIIT WITH(NOLOCK)
				ON CIIT.CommInspInputType = CII.CommInspInputType
			LEFT OUTER JOIN STB_ProductGroup PG WITH(NOLOCK)
				ON PG.ProductGroupCode = CII.ProductGroupCode
	WHERE
			((@CommInspTypeCode = '*') OR (CII.CommInspTypeCode = @CommInspTypeCode)) AND
			((CII.IsIndividualSpec = 1))
END
