...
-- EXEC [usp_GetProdPackingForBarcode_VNT] '','','VJJN023R033504', '', ''

-- EXEC [usp_GetProdPackingForBarcode_VNT] '','','', '', ''

Create PROCEDURE [dbo].[usp_GetProdPackingForBarcode_VVT]
...
BEGIN
	...
	--IF ISNULL(@DecisionResult,'None') NOT IN ('Pass','Special') 
	...
	SELECT
			...
			DPP.PlanQty,
			SI.ProdQty,
			SI.ProdQty - ISNULL(PR.ProdRouteQty,0) - ISNULL(PD.DefectQty, 0)  AS RemainQty, -- 불량수량 적용
