...
CREATE PROCEDURE [dbo].[usp_FourMChangHist_Second_get]
...
BEGIN
 ...
 -- SQL 조회부분
	...
	SELECT FMC.ChangesReasons As ChangesReasons
			,  FMC.LotNo As LotNo
			, FML.Unusual                                                 As Unusual
	 ...
	 FROM STB_FourMChangHist FMC
	          Left Outer Join STB_FourMLotNoHistory FML On FML.LotNo = FMC.LotNo
      Where FMC.LotNo = @LotNo
       ...
       SELECT FMC.ChangesReasons As ChangesReasons
			...
			, (SELECT  SPW.WorkerName From STB_ProdWorkerInfo SPW Where SPW.WorkerCode = FMC.Inspector ) As InspectorNM
	 FROM STB_FourMChangHist FMC
	          Left Outer Join STB_FourMLotNoHistory FML On FML.LotNo = FMC.LotNo2
	  Where FMC.LotNo =@LotNo
       ...
       SELECT FMC.ChangesReasons As ChangesReasons
			...
			, (SELECT  SPW.WorkerName From STB_ProdWorkerInfo SPW Where SPW.WorkerCode = FMC.Inspector ) As InspectorNM
	 FROM STB_FourMChangHist FMC
	          Left Outer Join STB_FourMLotNoHistory FML On FML.LotNo = FMC.LotNo3
	   Where FMC.LotNo =@LotNo
