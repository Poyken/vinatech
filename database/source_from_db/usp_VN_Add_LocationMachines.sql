CREATE PROC usp_VN_Add_LocationMachines
...
BEGIN
	...
	/***---------------------------------------------------------------Start automatic ID for add recorder-----------------------------------------------------------------***/
			...
			DECLARE @Prefix NVARCHAR(30) = 'LMC'
			DECLARE @Id INT

			SELECT @Id = ISNULL(MAX(ID),0) + 1 FROM STB_VN_LOCATIONMACHINES 
	...
	/***---------------------------------------------------------------End automatic ID for add recorder-------------------------------------------------------------------***/
		...
		DECLARE @NAMELOCATION NVARCHAR(50)
		...
		DECLARE @iDoc INT
		   ...
		   EXEC SmartFramework.dbo.usp_GetSerialRule 
			@pTableName = 'STB_VN_LOCATIONMACHINES',
	    ...
	    IF @IsAutoKey = 0 AND @IsLoopIUD = 0 BEGIN
		  ...
		  EXEC sp_xml_preparedocument @iDoc OUTPUT, @pXml
		   ...
		   BEGIN TRY
		       ...
		       MERGE STB_VN_LOCATIONMACHINES AS TargetTable
			USING
				(
					SELECT
							CASE
					...
					FROM
							...
							WITH  (
										OldCompanyCode INT,
										ID INT,
										CODELOCATIONMACHINES  NVARCHAR(50),
		...
		WHEN MATCHED THEN
				UPDATE SET
					CODELOCATIONMACHINES = @NewCode,
					NAMELOCATION = SourceTable.NAMELOCATION,
...
SELECT
  ...
  IF @IUD_FLAG = 'INSERT' BEGIN
		     ...
		     IF EXISTS (SELECT 1 FROM STB_VN_LOCATIONMACHINES WHERE ID = @OldCompanyCode) BEGIN
						RAISERROR('Duplicate Data : KeyField = %s', 16, 1, @OldCompanyCode)
					END
	 IF @IsAutoKey = 1 BEGIN
						EXEC SmartFramework.dbo.usp_DoCreateSerial 'STB_VN_LOCATIONMACHINES', @OldCompanyCode OUTPUT

INSERT INTO STB_VN_LOCATIONMACHINES
						(
							CODELOCATIONMACHINES,
						    NAMELOCATION,
							Descptions,
	...
	END ELSE IF @IUD_FLAG = 'UPDATE' BEGIN
	 UPDATE STB_VN_LOCATIONMACHINES
 
 SET


							NAMELOCATION =   CASE
						                WHEN @NAMELOCATION IS NOT NULL THEN @NAMELOCATION
						                ELSE NAMELOCATION
						            END,
