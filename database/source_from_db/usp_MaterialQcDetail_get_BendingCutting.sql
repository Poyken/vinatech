...
-- =============================================
CREATE PROCEDURE [dbo].[usp_MaterialQcDetail_get_Bending_Cutting]   
...
BEGIN
	...
	SELECT  @Barcode = Barcode
	  ...
	  FROM STB_SetInfo
	WHERE LotNumberBC  = @MaterialQcNo
	...
	SELECT @CompanyCode = CompanyCode ,  
	  ...
	  FROM STB_MaterialQcInfo_BendingCutting
	 WHERE MaterialQcNo = @MaterialQcNo

			--	SELECT   Barcode, LotNumberBC, *
			--  FROM STB_SetInfo
			--WHERE LotNumberBC  = '20092100004'
	  ...
	  IF @CompanyCode LIKE 'VVT'    -- 베트남 바코드의 경우
	   ...
	   BEGIN 
		...
		SELECT TOP 1 @PackQty =  PackQty FROM STB_SavePackingTime_VVT SPT where LotNo = @currentLotno AND SPT.PackQty > 0 AND SPT.IsPrinted = 0 -- lấy số lượng đóng gói ở B781
				-- Cập nhật lại số lượng hạng mục đóng gói
		...
		print @MaterialCodeMer
		 ...
		 from STB_MaterialQcDetail_BendingCutting mqd
		 WHERE 1=1
		...
		--SD add by loan change quality sd=10 ea
			...
			from STB_MaterialQcDetail_BendingCutting with(nolock) 
			WHERE 1=1 
			 AND (MaterialQcNo = @MaterialQcNo) 
    ...
    END AS InspectionLevel,
	                     ...
	                     --MQD.InspectionLevel,
							...
							MQD.ChangeDateTime,
							MQD.ChangeUserID				
					FROM  STB_MaterialQcDetail_BendingCutting MQD WITH(NOLOCK)
					WHERE 1=1
	...
	END ELSE BEGIN
			...
			END ELSE BEGIN
				...
				SELECT
					...
					MQD.Cpk				
			FROM  STB_MaterialQcDetail_BendingCutting MQD WITH(NOLOCK)
			LEFT OUTER JOIN STB_MaterialQcInfo_BendingCutting MQI
			  ON MQI.MaterialQcNo = MQD.MaterialQcNo
			WHERE 1=1
				 AND MQD.MaterialQcNo = @MaterialQcNo
				 AND MQD.QcInspectionItemCode NOT IN (SELECT QcInspectionItemCode 
