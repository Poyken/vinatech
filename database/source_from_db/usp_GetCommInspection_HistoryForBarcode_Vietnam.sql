...
--VVOJ122R718613
-- EXEC [usp_GetCommInspectionHistoryForBarcode] '','','VE_ROUTE_QUALITY','ve250226-006','','','','',''
-- EXEC [usp_GetCommInspectionHistoryForBarcode] '','','ROUTE_QUALITY2','VVOP113R038702','','','','',''
CREATE PROCEDURE [dbo].[usp_GetCommInspection_HistoryForBarcode_Vietnam]
...
BEGIN
	SET NOCOUNT ON;
	--	RAISERROR(@pCommInspTypeCode,16,1)
	--return;
	...
	if(@getWorkCenterCode='VVT_F1')
		...
		begin
			set @CommInspTypeCode='VE_ROUTE_QUALITY'
		end
	else if (@getWorkCenterCode='VVT_F4')
		begin
			set @CommInspTypeCode='ROUTE_QUALITY_PCBA_BG2'
		end
	--end
		--raiserror(@pCommInspRemark,16,1)
	SELECT
	...
	IF ISNULL(@Barcode,'') <> '' AND ISNULL(@CommInspTypeCode,'') <> ''      -- Developer에서 화면 만들때는 생성하지 않게
	 ...
	 BEGIN	
	 	-- @CommInspDocNo +'__'+@CompanyCode+'__'+@WorkCenterCode+'__'+@pLineCode+'__'+@pRouteCode+'__'+@pMachineCode+'__'+@pMoldNumber+'__'+@pCategoryName+'__'+@ProductGroupCode
	 ...
	 --RAISERROR(@CommInspDocNo,16,1)
		...
		IF ISNULL(@CommInspDocNo,'') = '' 
		   ...
		   BEGIN
				DECLARE @LineCode1 VARCHAR(20)  
				select @LineCode1=inputLineCode from stb_setInfo where barcode =@Barcode
	...
	-- 2025-11-10 Mr.Manh UPDATE for Bac Giang 1, following Ms.Hong QC 's request
	DECLARE @checkLineCode VARCHAR(20)
		SELECT	@checkLineCode = LineCode
		FROM STB_DayProdPlan WHERE DayPlanNo = (SELECT DayPlanNo FROM STB_SetInfo WHERE Barcode = @Barcode)

	IF @MaterialCode = 'ECVT30-357' AND @checkLineCode IN ('VVBGC-13')
		BEGIN
			update STB_CommInspDocItem 
			set CommInspItemSpec = N'0.22~0.32',
				CommInspUpper = '0.32',
				CommInspLower = '0.22'
			where 
			CommInspDocNo IN (select CommInspDocNo from STB_CommInspDocHistory where ProdNo = @ControlNo)
			and CommInspItemCode IN (
			'V_RQ_tM_BG',
			'V_RQ_tP_BG'
			)
		END
	...
	SELECT
		 ...
		 -- CIDI.CommInspRemark																			   											         AS CommInspRemark ,              -- 원본백업
		  ...
		  --SCH.CIDHExtText03                                                                                                                                                                                                   AS  NG_Check,                  -- 2020.01.31  검사결과 추가 (원본백업)
			...
			CASE 
				WHEN VIEW_CIIT.CommInspInputTypeName = 'NUMERIC' THEN CONVERT(VARCHAR, CONVERT(NUMERIC(10,3), CIMH1.NumericMeasure)) 	  
				WHEN ISNULL(CIMH.MeasureResult,'') = ''   THEN NULL    WHEN CIMH.MeasureResult = '1' OR CIMH.MeasureResult = 'OK'   
				THEN 'OK'    ELSE 'NG'    END AS FirstMeasureValue,           -- 1번째측정값
			...
			CASE 
			WHEN VIEW_CIIT.CommInspInputTypeName = 'NUMERIC' THEN CONVERT(VARCHAR, CONVERT(NUMERIC(10,3), CIMH11.NumericMeasure))	
			WHEN CIDI.CommInspInputType = '2' and CIDI.ItemTargetQty>=11 then CIMH.MeasureResult end as Value11,
			CASE
			WHEN VIEW_CIIT.CommInspInputTypeName = 'NUMERIC' THEN CONVERT(VARCHAR, CONVERT(NUMERIC(10,3), CIMH12.NumericMeasure))
			 WHEN CIDI.CommInspInputType = '2' and CIDI.ItemTargetQty>=12 then CIMH.MeasureResult end as Value12,
			CASE 
			WHEN VIEW_CIIT.CommInspInputTypeName = 'NUMERIC' THEN CONVERT(VARCHAR, CONVERT(NUMERIC(10,3), CIMH13.NumericMeasure))
			WHEN CIDI.CommInspInputType = '2' and CIDI.ItemTargetQty>=13 then CIMH.MeasureResult end as Value13,
			CASE 
			WHEN VIEW_CIIT.CommInspInputTypeName = 'NUMERIC' THEN CONVERT(VARCHAR, CONVERT(NUMERIC(10,3), CIMH14.NumericMeasure))
			WHEN CIDI.CommInspInputType = '2' and CIDI.ItemTargetQty>=14 then CIMH.MeasureResult end as Value14,
			CASE 
			WHEN VIEW_CIIT.CommInspInputTypeName = 'NUMERIC' THEN CONVERT(VARCHAR, CONVERT(NUMERIC(10,3), CIMH15.NumericMeasure))
			WHEN CIDI.CommInspInputType = '2' and CIDI.ItemTargetQty>=15 then CIMH.MeasureResult end as Value15,
			CASE 
			WHEN VIEW_CIIT.CommInspInputTypeName = 'NUMERIC' THEN CONVERT(VARCHAR, CONVERT(NUMERIC(10,3), CIMH16.NumericMeasure))
			WHEN CIDI.CommInspInputType = '2' and CIDI.ItemTargetQty>=16 then CIMH.MeasureResult end as Value16,
			CASE 
			WHEN VIEW_CIIT.CommInspInputTypeName = 'NUMERIC' THEN CONVERT(VARCHAR, CONVERT(NUMERIC(10,3), CIMH17.NumericMeasure))
			WHEN CIDI.CommInspInputType = '2' and CIDI.ItemTargetQty>=17 then CIMH.MeasureResult end as Value17,
			CASE 
			WHEN VIEW_CIIT.CommInspInputTypeName = 'NUMERIC' THEN CONVERT(VARCHAR, CONVERT(NUMERIC(10,3), CIMH18.NumericMeasure))
			WHEN CIDI.CommInspInputType = '2' and CIDI.ItemTargetQty>=18 then CIMH.MeasureResult end as Value18,
			CASE 
			WHEN VIEW_CIIT.CommInspInputTypeName = 'NUMERIC' THEN CONVERT(VARCHAR, CONVERT(NUMERIC(10,3), CIMH19.NumericMeasure))
			WHEN CIDI.CommInspInputType = '2' and CIDI.ItemTargetQty>=19 then CIMH.MeasureResult end as Value19,
			CASE 
			WHEN VIEW_CIIT.CommInspInputTypeName = 'NUMERIC' THEN CONVERT(VARCHAR, CONVERT(NUMERIC(10,3), CIMH20.NumericMeasure))
	...
	ORDER BY CII.DisplayIndex 
			, CIDI.RouteCode  --DinhManh update 2025-01-15
