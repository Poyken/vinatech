...
-- =============================================
CREATE PROCEDURE [dbo].[usp_DoMakeMaterialIQCDetailList_BendingCutting]
...
BEGIN
	...
	DECLARE @MaterialCode VARCHAR(50) = (SELECT MaterialCode FROM STB_MaterialQcInfo_BendingCutting WHERE MaterialQcNo = @MaterialQcNo)
	...
	FROM
			STB_MaterialQcInfo_BendingCutting MII  with(nolock) 
	WHERE
        ...
        BEGIN TRY	
			...
			IF @IQC_ITEM_OPTION = 'BY_MATERIAL'
					...
					BEGIN
						...
						--RAISERROR(@MaterialCode, 16, 1)
							...
							FROM
									STB_MaterialQcInspectionItem_BendingCutting MII WITH (NOLOCK)
           ...
           -- CURSOR
			...
			BEGIN
				...
				FROM
						STB_MaterialQcDetail_BendingCutting WITH (NOLOCK)
				WHERE
						MaterialQcNo = @MaterialQcNo	
				...
				WHERE
						...
						*/
						...
						SET @SampleQty= CASE 								
								...
								--											AND (
								--													@MaterialCode in ('ECVT30-076','ECVT30-117','ECVT30-098','ECVT30-197','ECVT30-294','ECVT30-116') or 
								...
								WHEN @QcInspectionItemCode IN (SELECT QcInspectionItemCode 
																FROM STB_MaterialQcInspectionItem_BendingCutting  WITH (NOLOCK)
			...
			ELSE   --  @IQC_ITEM_OPTION <> 'BY_MATERIAL' (위 IF문의 반대조건)
			    ...
			    BEGIN
						...
						(
							...
							FROM
									...
									LEFT OUTER JOIN STB_QcInspectionGroup IIG	 WITH (NOLOCK)		ON (MIG.QcInspectionGroupCode = IIG.QcInspectionGroupCode)
									LEFT OUTER JOIN STB_QcInspectionItem III WITH (NOLOCK)		ON (MIG.QcInspectionGroupCode = III.QcInspectionGroupCode)
									LEFT OUTER JOIN STB_MaterialQcInspectionItem_BendingCutting MII WITH (NOLOCK)		ON (MIG.MaterialCode = MII.MaterialCode AND											III.QcInspectionItemCode = MII.QcInspectionItemCode)
							WHERE 1=1
									AND III.QcInspectionItemCode = @QcInspectionItemCode
									AND MIG.MaterialCode = @MaterialCode 
				...
				-- 특성 검사항목에 대해 샘플리스트를 기본으로 생성해 줌. 품질부문요청 By Jackaroe 2019.10.29
					...
					PRINT 'LAST LAST IF'
					Exec usp_DoCreateMaterialQcSampleResult_BendingCutting @MaterialQcNo, @MaterialQcDetailNo, @SampleQty, 0       -- 시료별 수입검사결과에서 샘플수량만큼 셀이 자동생성되는 부분
