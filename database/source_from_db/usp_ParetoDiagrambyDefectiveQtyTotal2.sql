...
-- Browsable : True
-- Group : 생산현황 > 일별 공정별 생산실적 > 4번째 화면(2) - 내가하다만것
-- Description:	 공정별 불량 파래토 

--    usp_ParetoDiagrambyDefectiveQtyTotal  'kilee2','Korean', '', '2022-02-01' ,'2022-02-20'
-- ======================================================================================================
CREATE PROCEDURE [dbo].[usp_ParetoDiagrambyDefectiveQtyTotal2]
...
SELECT A.IDX
		  , A.BasicDefectName
		  , A.DefectCode
		  , SUM(A.DefectQty) AS DefectQtySum
		  , CONVERT(INT, CONVERT(NUMERIC(20, 3), SUM(A.DefectQty)) 	/ CONVERT(NUMERIC(20, 3), MAX(B.DefectQty)) * 1000000) AS DefectRatePPMSum
	  FROM (
							SELECT    ROW_NUMBER() OVER(ORDER BY MAX(RouteName) DESC) AS IDX
											, DI.BasicDefectName
											, DRI.DefectCode 
							 ...
							 WHERE 1=1
							     AND DRI.DefectCode NOT IN ('E-22_X03','E-22_X12','E-24_X03','E-24_X12','E-22_QQ','E-24_QQ')    --파괴검사,샘플진행, 장비셋업는 제외해야 함
	...
	-- ORDER BY A.DefectQty DESC
	  ORDER BY A.IDX
						, CASE CASE WHEN A.RouteCode = 'E-24' THEN '조립(커링)' ELSE A.RouteName END 
												WHEN '권취' THEN '01'
												WHEN '조립(커링)' THEN '02'
												WHEN '도핑' THEN '03' 
												WHEN '절곡' THEN '04'
												WHEN '재검' THEN '05'			ELSE '99' END
