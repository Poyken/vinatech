...
-- EXEC usp_LotTrackingInfo_Mornitorning_get 'kilee','Korean','','VNT', '2020-08-18 08:30:00', '2020-08-19 08:30:00', '', 'ASSYLINE-05', '', ''
-- EXEC usp_LotTrackingInfo_Mornitorning_get 'kilee','Korean', '' ,'VNT', '', 'ASSYLINE-05',  ''
-- EXEC usp_LotTrackingInfo_Mornitorning_get 'VNT', '', 'ASSYLINE-05', ''
-- EXEC usp_LotTrackingInfo_Mornitorning_get 'VNT', '', '', ''
-- =============================================

CREATE PROCEDURE [dbo].[usp_LotTrackingInfo_Mornitorning_get]					
...
BEGIN
	...
	SELECT PRH.ComPanyCode
			...
			-- , PRH.RouteCode
		     ...
		     , ISNULL(SUM(Round(DPP.PlanQty, 0) ),0)                                                 AS '계획량(갯수)'
			 , ISNULL(SUM(PRH.ProdQty),0)                                                              AS InputProdQty    -- 투입수량
			 , ISNULL(SUM(DRI.DefectQty), 0)                                                           AS DefectQty         -- 불량수량 
			 ,  SUM(Round(PRH.ProdQty, 0)) - ISNULL(SUM(Round(DRI.DefectQty, 0)), 0)    AS '생산량(갯수)'           -- 생산수량)
    ...
    --          , Case When SUM(PRH.ProdQty) = 0 Then 0 
	 ...
	 WHERE 1=1
	   ...
	   --AND ISNULL(NP.AftProdQty,0) > CASE WHEN @RouteCode = '%' THEN -1   
        ...
        AND PRH.RouteCode IN ( 'E-22',  'E-24',  'E-25',  'E-28')
...
--ALTER PROCEDURE [dbo].[usp_LotTrackingInfo_Mornitorning_get]
...
--		     , ISNULL(SUM(Round(DPP.PlanQty, 0) ),0)                                                 AS '계획(개)'
--			 , ISNULL(SUM(PRH.ProdQty),0)                                                              AS InputProdQty    -- 투입수량
--			 , ISNULL(SUM(DRI.DefectQty), 0)                                                           AS DefectQty         -- 불량수량 
--			 ,  SUM(Round(PRH.ProdQty, 0)) - ISNULL(SUM(Round(DRI.DefectQty, 0)), 0)    AS '생산(개)'           -- 생산수량)
...
--	  ORDER BY CASE WHEN LI.LineName = '셀 10라인' THEN '셀 99라인'
--	  --             WHEN DL.MeasureTimeCode = 'MIDDLE' THEN 2
--			--	   WHEN DL.MeasureTimeCode = 'LAST' THEN 3
--				   ELSE LI.LineName  END 
