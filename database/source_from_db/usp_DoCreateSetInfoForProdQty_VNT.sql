...
--- // 2021.12.07 추가  (MEA소재생산부문) Start  // -----------------------------------------------------------------------------------
 ...
 ---  소재생산 추가된부분 End // --------------------------------------------------------------------------------------------------------------------------------------------------------------------
			...
			IF  @WorkCenter = 'VNT_F3' BEGIN
				...
				SET @Barcode = @Header + CHAR(ISNULL(@SerialNo,0) + 64)
			...
			IF  @WorkCenter = 'VVT_F4' BEGIN -- 베트남 하남공장 추가
				    SET @Y = RIGHT(DATEPART(YEAR,@PlanDate), 2)
					...
					IF @MaterialCode IN ('EDVTMD-246', 'EDVTMD-246-001', 'EDVTMD-246-002', 'EDVTMD-246-003')
						BEGIN
							SET @Header = 'NM' + @BloomEnergyPartNumber + 'C1' + @Y + @Week
						END
					ELSE
						BEGIN
							SET @Header = 'B' + @BloomEnergyPartNumber + 'C1' + @Y + @Week  --2026.02.21  -- Just test, can change in the future, 2026.03.26 change by Jackaroe
						END
					...
					SET @Barcode = @Header + RIGHT('00000' + CONVERT(VARCHAR,ISNULL(@SerialNo,0)), 5)										--2022.08.10
