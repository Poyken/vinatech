...
-- Modified    : 매월 생산계획을 세우면 자동으로 실적테이블을 세팅


---[실행문]    EXEC usp_Prodduct_Plan_Month_iud       
-- ==================================================================
Create PROC [dbo].[usp_Prod_Month_iud]
				@pDate DATE
...
-- [수동으로 실행시 -> 지우지말것]	
  ...
  --DECLARE @BaseYm       VARCHAR(6) = LEFT(@FromDt, 4) + SUBSTRING(@FromDt, 6, 2)                                                                        -- 전일자 두자리           SELECT  LEFT(@FromDt, 4) + SUBSTRING(@FromDt, 6, 2)     
	...
	-- SELECT LEFT(@FromDt, 4) + SUBSTRING(@FromDt, 6, 2)  
...
WHERE 1=1
  --AND  CompanyCode = 'VNT'
  --AND GG.공정코드 IN ( 'E-28', 'V-28')
  --AND GG.공정코드  LIKE @RouteCode		

--AND 라인코드 <> 'ASSYLINE-12'
...
--DECLARE @ToDay VARCHAR(10) = '08'
   ...
   --SET @sqlStr = '    UPDATE CURLING_PROD   SET DAY' + @ToDay + ' = ISNULL(B.[일일총수량], 0)     FROM CURLING_PROD A   INNER JOIN #TEMP_TABLE100 B  ON A.[사이즈] = B.[사이즈] AND A.[RouteCode] = B.[공정코드] AND 기준년월 =  ''' + @BaseYm + '''  AND A.RouteCode = ''' + @RouteCode + '''  AND A.CompanyCode = ''VNT'' AND A.LineCode = B.라인코드    '
     SET @sqlStr = '    UPDATE CURLING_PROD   SET DAY' + @ToDay + ' = ISNULL(B.[일일총수량], 0)     FROM CURLING_PROD A   INNER JOIN #TEMP_TABLE100 B  ON A.[사이즈] = B.[사이즈] AND A.[RouteCode] = B.[공정코드] AND 기준년월 = ''201911''                AND A.RouteCode = ''' + @RouteCode + '''  AND A.CompanyCode = ''VNT''  AND A.라인코드 = B.라인코드   '
