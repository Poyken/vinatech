...
--   usp_GetProdRouteSummary_Monitoring  '','','','','ASSYLINE-13','','','2020-08-18','2020-08-18',''
--   usp_GetProdRouteSummary_Monitoring  '','','','','','','','2020-08-18','2020-08-18',''
--  =====================================================================================
CREATE PROCEDURE [dbo].[usp_GetProdRouteSummary_Monitoring]
  ...
  --DECLARE @OneDay				  VARCHAR(11) = SUBSTRING(CONVERT(VARCHAR(10), GetDate(), 121), 0, 11)   
	...
	(
		...
		FROM
           ...
           where 1=1
		   and si.CreateDateTime >= '2020-08-18 00:00:00'
		GROUP BY
		        PRH.PoNo
			 , SI.BarCode
			 , SI.ControlNo
	...
	SELECT
	      ...
	      --PRS.JobDate,                                                        -- 원본백업
			...
			(SUM(PRS.OutputQty) - SUM(PRS.DefectQty - ISNULL(RepairInfo.RepairQty, 0)))  AS GoodsQty,            -- 생산수량  (kilee, 2020.03.25)
	...
	FROM
			...
			LEFT OUTER JOIN (
										SELECT CompanyCode, WorkCenterCode, FindJobDate, FindShiftCode, PONo
										 ...
										 WHERE 1=1 
										    AND RepairType NOT IN ('MISSING') -- #20200129
										 GROUP BY CompanyCode, WorkCenterCode, FindJobDate, FindShiftCode, PONo ,MaterialCode, FindLineCode, FindRouteCode
										             , ControlNo
									) RepairInfo				ON RepairInfo.CompanyCode = PRS.CompanyCode  AND  RepairInfo.ControlNo = NP.ControlNo
