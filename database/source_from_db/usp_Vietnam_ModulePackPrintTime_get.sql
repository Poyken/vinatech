--        exec usp_Vietnam_ModulePackPrintTime_get '','','VVT','2024-09-11','2024-09-12','','','',''
-- =============================================
-- Author:	    Mr.Tung
...
-- Modified: Mr.Tung
-- =============================================
CREATE PROCEDURE [dbo].[usp_Vietnam_ModulePackPrintTime_get]
...
(ROW_NUMBER() over (partition by LotNo,isPrinted order by PackQty desc,printtime desc) ) as RowPackQty2
	...
	SELECT DISTINCT  id,
	 ...
	 , InputLineCode,
		( case  
				when LotNo in ('MVVOR056R015504')  then '2024-09-12'   
				else    ProdDateTime     
				end )    as  ProdDateTime,
	 --ProdDateTime,
	 ControlNo,
...
--           exec usp_Vietnam_ModulePackPrintTime_get '','','VVT','2024-09-04','2024-09-05','','','',''