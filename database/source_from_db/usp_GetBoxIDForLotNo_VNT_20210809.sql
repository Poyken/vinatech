...
-- Modified:
-- 2021.04.22 팅크웨어향 라벨 관련 (고객사)자재코드 추가 #210422

-- exec usp_GetBoxIDForLotNo_VNT 'kilee','Korean','VJJP223R010504', ''
-- exec usp_GetBoxIDForLotNo_VNT_20210809  'kilee','Korean','VJLQ023R830601', ''
-- =============================================
CREATE PROCEDURE [dbo].[usp_GetBoxIDForLotNo_VNT_20210809]                                  
...
BEGIN
	...
	SELECT
			...
			CASE WHEN CHARINDEX('-L', ModelName) > 0 THEN '-L' ELSE '' END AS PartNo_11111,	
			ISNULL(PLS.PartNo, RTRIM(LTRIM(SUBSTRING(ModelName, CHARINDEX(' ', ModelName), 12))))  AS PartNo_22222,	

			 ISNULL(PLS.PartNo, RTRIM(LTRIM(SUBSTRING(ModelName, CHARINDEX(' ', ModelName), 12))))  + (CASE WHEN CHARINDEX('-L', ModelName) > 0 THEN '-L' ELSE '' END) AS PartNo_3333,	


			ISNULL(PLS.PartNo, RTRIM(LTRIM(SUBSTRING(ModelName, CHARINDEX(' ', ModelName), 12)))) 		
		...
		--,	'' AS LotQtyTwo
		    ,CASE WHEN MLI.MaterialCode = 'ECVT30-333' THEN 'AASC-0K00005' --#210422
				  ...
				  WHEN MLI.MaterialCode = 'ECVT30-321' THEN 'AASC-4M00001'
				  ELSE '' END AS ThinkwareBarcode
	FROM
