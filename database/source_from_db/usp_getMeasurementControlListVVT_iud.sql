...
-- =============================================
CREATE PROCEDURE [dbo].[usp_getMeasurementControlListVVT_iud]
			...
			WHILE 1 = 1 BEGIN
				...
				IF @IUD_FLAG = 'INSERT' BEGIN
					...
					IF @pProcessUserID NOT IN ('62303001', '62505009', 'DinhManh')
						BEGIN
							RAISERROR( N'Bạn không có quyền thêm mới!' ,16, 1)
							RETURN
						END

					--RAISERROR('%s hihi', 16, 1, @IUD_FLAG)
					--return
					...
					INSERT INTO STB_MeasurementControlList_VVT
						(
							SerialNo,
							MeasurementNameE,
							MeasurementNameV,
				...
				END ELSE IF @IUD_FLAG = 'UPDATE' BEGIN
					...
					IF EXISTS (SELECT 1 FROM STB_MeasurementControlList_VVT WHERE ManagementNo = @ManagementNo) BEGIN


					--RAISERROR('%s hihi %s', 16, 1, @IUD_FLAG, @ManagementNo)
					--return
						...
						DECLARE @curDayOfCalibration DATE 
						SELECT @curDayOfCalibration = DayOfCalibration FROM STB_MeasurementControlList_VVT WHERE ManagementNo = @ManagementNo AND SerialNo = @SerialNo
						-- Nếu khác ngày thì tự hiểu đó là hiệu chuẩn
						--IF @DayOfCalibration <> @curDayOfCalibration
						--BEGIN
				...
				END ELSE IF @IUD_FLAG = 'DELETE' BEGIN
					...
					IF @pProcessUserID NOT IN ('62303001', '62505009', 'DinhManh')
						BEGIN
							RAISERROR( N'Bạn không có quyền xóa!' ,16, 1)
							RETURN
						END
					...
					DELETE FROM STB_MeasurementControlList_VVT
						WHERE
							ManagementNo = @ManagementNo
