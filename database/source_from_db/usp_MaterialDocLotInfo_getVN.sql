...
CREATE PROCEDURE [dbo].[usp_MaterialDocLotInfo_getVN] -- EXEC usp_MaterialDocLotInfo_getVN '','','ML20240619000330'              -- EXEC usp_MaterialDocLotInfo_getVN '','','ECVT27-370NM1000005'
...
BEGIN
	...
	SELECT
			...
			MDLI.StockAttrib2,
			MDLI.StockAttrib3,	
			format(round(cast(Replace(MDLI.StockQty,',','.') as float),0), N'#,###.##########')AS StockQty ,
			--ROUND(MDLI.StockQty, 4) AS StockQtyORigan ,
			  CASE 
				WHEN CAST(ROUND(CAST(MDLI.StockQty AS FLOAT), 2) AS FLOAT) = CAST(ROUND(CAST(MDLI.StockQty AS FLOAT), 2) AS INT) 
				THEN FORMAT(CAST(ROUND(CAST(MDLI.StockQty AS FLOAT), 2) AS INT), 'N0')
				ELSE FORMAT(ROUND(CAST(MDLI.StockQty AS FLOAT), 2), 'N2')
			END AS StockQtyORigan,
...
--select * from UPDATE STB_MaterialDocLotInfo SET Isused= '1'  where LotID = 'ML20240410000003'

--alter table STB_MaterialDocLotInfo
--add
--		Isused BIT NULL