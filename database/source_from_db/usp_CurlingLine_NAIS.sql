...
--                   2019-09-24 전반적으로 모두 수정 (kilee)
-- ==================================================================

--   EXEC usp_CurlingLine '','', '2019-09-27 08:30:00'                                             ---> 소스에서는 BETWEEN '2019-04-01 08:30:00' and '2019-04-02 08:30:00'  

CREATE PROC [dbo].[usp_CurlingLine_NAIS] 
																	...
																	LEFT OUTER JOIN (
																								...
																								WHERE 1=1																																																		 
																								 ...
																								 --   AND 기준년월 = '201909'			
																									--AND 공정코드 = 'E-28'		
																									AND CompanyCode = 'VNT'
																						) BB		ON   AA.라인코드 = BB.LineCode    AND AA.라인코드 = BB.LineCode  --AND AA.사이즈 = BB.사이즈
												-- GG END
																)   GG
																	...
																	-- NAIS 주간실적 
																			SELECT  (SELECT B.LineName FROM STB_LineInfo B WHERE B.LineCode = SI.InputLineCode ) AS 라인명
																				...
																				-- AND SI.CreateDateTime > @FromDt AND SI.CreateDateTime <= @ChangeTime
																					AND SI.CreateDateTime > '2019-09-25 08:30:00' AND SI.CreateDateTime <= '2019-09-25 20:30:00'
																	...
																	-- NAIS  야간실적 
																			SELECT  (SELECT B.LineName FROM STB_LineInfo B WHERE B.LineCode = SI.InputLineCode ) AS 라인명																									
																				...
																				FROM STB_SetInfo SI    
																						...
																						LEFT OUTER JOIN ERPSVR.ERPDB.DBO.product  SP	 ON SI.MaterialCode = SP.PRODCD																							
																				WHERE 1=1																								 
																					--  AND SI.CreateDateTime > @ChangeTime AND SI.CreateDateTime <= @ToDt	                    -- 원본으로 절대삭제금지!!!
																					AND SI.CreateDateTime > '2019-09-25 20:30:00'  AND SI.CreateDateTime <= '2019-09-26 08:30:00'  -- TEST부분 (주석처리)
								...
								-- NAIS 추가부분 (주간)
											SELECT  (SELECT B.LineName FROM STB_LineInfo B WHERE B.LineCode = SI.InputLineCode ) AS 라인명																									
												...
												--AND SI.CreateDateTime > @FromDt AND SI.CreateDateTime <= @ChangeTime                                  -- 지우지말것																							   
													AND SI.CreateDateTime > '2019-09-25 08:30:00' AND SI.CreateDateTime <=  '2019-09-25 20:30:00'   	      -- TEST용 : FromDt는 오전8시반, ChangTime는 당일 오후20시반
...
--																					-- NAIS 추가부분 (야간)
											SELECT  (SELECT B.LineName FROM STB_LineInfo B WHERE B.LineCode = SI.InputLineCode ) AS 라인명																									
												...
												WHERE 1=1				
													...
													AND SI.CreateDateTime > '2019-09-25 20:30:00' AND SI.CreateDateTime <= '2019-09-26 08:30:00'     -- TEST문
													--AND PRH.RouteCode LIKE  @RouteCode 
													AND SI.InputShiftCode = '2'																						
											GROUP BY SI.InputLineCode 
														, SP.사이즈																									
