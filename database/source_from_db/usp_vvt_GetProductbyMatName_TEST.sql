...
CREATE PROC [dbo].[usp_vvt_GetProductbyMatName_TEST]
...
----            usp_vvt_GetProductbyMatName_TEST'','',N'DUNG DỊCH -3.0V',N'VVBGC-15',1
----          usp_vvt_GetProductbyMatName_TEST '','',N'DUNG DỊCH 3.8V (VN-62)',N'VVBGC-15',1
	...
	select top 1 @code=materialcode from stb_setinfo with(nolock) where InputLineCode like @line
		...
		if(@pMatName like N'%DUNG%DỊCH%') begin
			...
			--	set @kind='3.0V'	
			--end			if(@pMatName like '%3.8V%'  ) begin
			--	set @kind='3.8V'	
			--end
