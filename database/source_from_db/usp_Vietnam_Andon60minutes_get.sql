...
-- Browsable : true
-- ============================================= [usp_Vietnam_Andon60minutes_get] '',''          
CREATE PROCEDURE [dbo].[usp_Vietnam_Andon60minutes_get]
...
SET NOCOUNT ON;
	...
	set @pFromDate = case when isnull(@pFromDate,'')='' then '2023-01-01 10:00:00' else @pFromDate end
...
;with linedat as (
 ...
 ,case when isnull(DATEDIFF(MINUTE, beginFix2, isnull(finishFix2,getdate1) ),0)=0 
  ...
  from combinedat
  where id is null  
  and (case when isnull(DATEDIFF(MINUTE, beginOccur, getdate1 ),0)<=0 then 1 else DATEDIFF(MINUTE, beginOccur, getdate1 ) end) % 60 = 0 
  and DATEDIFF(MINUTE, beginOccur, getdate1 ) > 59
  and DATEDIFF(MINUTE, beginOccur, getdate1) <= 24*60*3
