...
-- =============================================
CREATE PROCEDURE [dbo].[usp_DoCreateQCDefectDetailsRecordC586_iud]
      ...
      --              IF @IsAutoKey = 1 
				...
				ELSE IF @IUD_FLAG = 'UPDATE' BEGIN
					...
					IF EXISTS (SELECT 1 FROM STB_QCDefectDetailsRecord WHERE Barcode = @Barcode) BEGIN
						...
						--IF @pProcessUserID NOT IN ('DinhManh', 'doanhanh')
						--	BEGIN
						...
						SET
							...
							Cause = ISNULL(@Cause, Cause),
							Countermeasure = ISNULL(@Countermeasure, Countermeasure),
							FollowUp = ISNULL(@FollowUp, FollowUp),
