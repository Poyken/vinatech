...
-- Browsable : true
-- ============================================= 
--[usp_Vietnam_AndonDetail_get] '2024-04-01','2024-04-08','adon','','VVT_F1'       
--[usp_Vietnam_AndonDetail_get] '2024-04-01','2024-04-08','adon','','VVT_F2'       
--[usp_Vietnam_AndonDetail_get] '2024-04-01','2024-04-08','adon','',''       
CREATE PROCEDURE [dbo].[usp_Vietnam_AndonDetail_get_TEST2]
...
Begin
	...
	DeClare  @CheckWorkCenter int = 0 ;
	if @WorkCenterCode ='VVT_F2' 
		begin
			set @CheckWorkCenter=1
		end
	else
		begin
			set @CheckWorkCenter=0
		end
	 ...
	 BEGIN TRY  
			...
			update stb_linesituation_vvt
			set routecode = case
when		routename='WINDING-권취'	 and @WorkCenterCode ='VVT_F1'  then  'V-22'
								when		routename='WINDING-권취'	 and @WorkCenterCode ='VVT_F2'  then  'V-22_BG'
								 when		 routename='BEADING-비딩'	 	and @WorkCenterCode ='VVT_F1'		then  'V-23'
								 when		 routename='BEADING-비딩'		and @WorkCenterCode ='VVT_F2'		then  'V-23_BG'
								 when		 routename='CURLING-커링'		and @WorkCenterCode ='VVT_F1'		then  'V-24'
								 when		 routename='CURLING-커링'		and @WorkCenterCode ='VVT_F2'		then  'V-24_BG'
								 when		 routename='SLEEVE-슬리브'		and @WorkCenterCode ='VVT_F1'		then  'V-25'
								 when		 routename='SLEEVE-슬리브'		and @WorkCenterCode ='VVT_F2'		then  'V-25_BG'
								 when		 routename='AGING-에이징'		and @WorkCenterCode ='VVT_F1'		then  'V-26'
								 when		 routename='AGING-에이징'		and @WorkCenterCode ='VVT_F2'		then  'V-26_BG'
								 when		 routename='VISUAL-외관'		and @WorkCenterCode ='VVT_F1'			then  'V-27'
								 when		 routename='VISUAL-외관'		and @WorkCenterCode ='VVT_F2'			then  'V-27_BG'
								 when		 routename='PACKING-포장'		and @WorkCenterCode ='VVT_F1'		then  'V-28'
								 when		 routename='PACKING-포장'		and @WorkCenterCode ='VVT_F2'		then  'V-28_BG'
								 when		 routename='BENDING-벤딩'	and @WorkCenterCode ='VVT_F1'			then  'V-33'
								 when		 routename='BENDING-벤딩'	and @WorkCenterCode ='VVT_F2'			then  'V-33_BG'
								 else		 '.'						end
...
replace((select Item from dbo.fnSplitToTable('~~',ls.errorcode) where RowNo=2),'-Comment:','') 'Detector',
				...
				--and ls.linename not like '%BG%'
				 and PATINDEX('%[BG]%', ls.linename ) =@CheckWorkCenter
				order by ls.id desc 
	    return;
	end
...
--			update stb_linesituation_vvt
--			set routecode = case when        routename='WINDING-권취'				then  'V-22'
--								 when		 routename='BEADING-비딩'				then  'V-23'
--								 when		 routename='CURLING-커링'				then  'V-24'
--								 when		 routename='SLEEVE-슬리브'				then  'V-25'
--								 when		 routename='AGING-에이징'				then  'V-26'
--								 when		 routename='VISUAL-외관'					then  'V-27'
--								 when		 routename='PACKING-포장'				then  'V-28'
--								 when		 routename='BENDING-벤딩'				then  'V-33'
--								 else		 '.'						end
--			where routecode is null or routecode=''
