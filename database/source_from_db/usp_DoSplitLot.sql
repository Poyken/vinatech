...
-- =============================================
CREATE PROCEDURE [dbo].[usp_DoSplitLot]
...
BEGIN
	...
	--select @LastSerialNo=isnull(max(substring(LotID,11,6)),0)+1 from STB_MaterialLotInfo -->change PackingID to LotID
	--where left(LotID,2)='SP'     -->change PackingID to LotID
	--and substring(lotid,3,8)=@FourYearString+@MonthString+@DayString

	--declare @LotIDNew char(20)
	--set @LotIDNew ='SP'+@FourYearString+@MonthString+@DayString + REPLICATE('0',6-len(@LastSerialNo)) + rtrim(@LastSerialNo)
...
set @LotIDNew ='SP'+@FourYearString+@MonthString+@DayString +right(@LastSerialNo,6) -->M.sh
	...
	SELECT
			...
			StockAttrib3,
			--@LotIDNew,--PackingID,
			CASE WHEN LEFT(LotID, 2) = 'ML' THEN LotID ELSE 'PKSPK'+RIGHT(@LotIDNew, 6) END, -- 2025.03-05 by Jackaroe
			GRDate,
			MaterialDeliveryNo,
			MaterialDeliveryDetailNo,
