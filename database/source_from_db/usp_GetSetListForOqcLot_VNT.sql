...
-- EXEC usp_GetSetListForOqcLot_VNT @pProcessUserID='anhduy157',@pProcessLanguage='Korean',@pCompanyCode='VVT',@pWorkCenterCode='VVT_F1',@pLineCode=default,@pRouteCode='',@pBarCode = 'VJOR223R850607'
...
CREATE PROCEDURE [dbo].[usp_GetSetListForOqcLot_VNT]
...
BEGIN
	...
	SELECT @CountChange=count(*) ,@OldLotidChange=oldlotid FROM STB_ChangePartNoAndLotNo WHERE NewLotid = @Barcode
	group by oldlotid
	...
	(
		...
		FROM
				...
				--left outer join STB_ChangePartNoAndLotNo on 
		...
		WHERE 1=1
			 ...
			 -- AND SI.Barcode LIKE @Barcode 				                                                                                                                                      -- 원본백업    
				AND  (SI.Barcode = @Barcode  OR SI.Barcode = (SELECT NewBarcode  FROM STB_LotChangeMaterialHistory WHERE OldBarcode = 
