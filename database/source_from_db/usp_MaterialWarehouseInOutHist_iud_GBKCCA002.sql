...
-- Exec :  usp_MaterialWarehouseInOutHist_iud 'kilee','Korean','VNT','VNT_F1','','','','','','202600120#2200039818#H219112362#32#DLC3702'
-- =============================================================================================
Create PROCEDURE [dbo].[usp_MaterialWarehouseInOutHist_iud_GBKCCA002]
...
---------------------------------------------------------	
   ...
   --2020.04.27 추가사항 (활성탄 두제품의 경우에는 바코드를 찍으면 일괄처리되도록 - 구보겸요청)
	   IF @MaterialCode IN ('GAKCCA-002', 'GAKCCA-003') 	        BEGIN          --예외처리 제품인 경우
			...
			BEGIN
				EXEC usp_PDADoPutaway2 @pProcessLanguage, @pProcessUserID, @TargetLocation, @MaterialLotNo, 'N'                       -- 다른 프로시저 호출 : 정상출고 처리되는 부분
			END
...
END
	 ...
	 --SELECT * FROM STB_MaterialWarehouseInOutHist WHERE LOTID = '202600120#2200039818#H219112362#32#DLC3702'
	 --SELECT * FROM STB_MaterialLotInfo                  WHERE LOTNO = '202600120#2200039818#H219112362#32#DLC3702'