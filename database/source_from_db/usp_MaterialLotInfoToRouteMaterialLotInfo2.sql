...
-- Browsable: false
-- Description:	STB_RouteMaterialLotInfo --> STB_MaterialLotInfo  (활성탄예외처리부분)
-- =============================================
CREATE PROCEDURE [dbo].[usp_MaterialLotInfoToRouteMaterialLotInfo2]
	...
	BEGIN
			UPDATE STB_MaterialLotInfo 
				SET CurrentQty = CurrentQty + @CurrentQty
					, LotAttr01 = CASE WHEN ISNULL(LotAttr01, '') = '' THEN @LotAttr01 ELSE LotAttr01 END
					, LotAttr02 = CASE WHEN ISNULL(LotAttr02, '') = '' THEN @LotAttr02 ELSE LotAttr02 END
				, LotAttr03 = CASE WHEN ISNULL(LotAttr03, '') = '' THEN @LotAttr03 ELSE LotAttr03 END
			WHERE
					LotID = (CONVERT(VARCHAR, @MaterialCode)  + ':' +  CONVERT(VARCHAR, @MaterialLocationCode)) 
	
	END ELSE 
	           BEGIN
			            EXEC SmartFramework.dbo.usp_DoCreateSerial	'STB_MaterialLotInfo',  @NewMaterialLotNo OUTPUT
