...
-- 프로시저 실행 :              usp_VPCDailyProdCuring  '', ''
-- ================================================================================
CREATE PROCEDURE [dbo].[usp_VPCDailyProdCuring]
  ...
  FROM  (  
			...
			-- [Main 조회부분]
				SELECT A2.LineCode
						...
						, IsNull(Max(A2.RouteName), '커링(조립)') As RouteName
  										...
  										UNION ALL
										   ...
										   SELECT 'VPCLINE-합계 ' As LineCode
												  --, Format(Convert(BIGINT, Max(A2.PlanQty)),  'N0')AS PlanQty 
													...
													, Round(Convert(NUMERIC(20,2), (Sum(A2.ProdQty) / 217000 * 100)), 1) AS ProdRate                  -- 고정값 세팅
													FROM (
												SELECT  B.LineCode
															, IsNull(B.PlanQty, 0) As PlanQty
															, A.RouteName						
