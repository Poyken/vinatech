...
--					exec [usp_VNGetInSparePartHistoryManagement_TEST] '', '',null,'','','2023-01-01','2024-05-05','CSP000004','VVT_F1'
-- =============================================
CREATE PROCEDURE [dbo].[usp_VNGetInSparePartHistoryManagement_TEST]
...
BEGIN
    ...
    DECLARE @SPWarehouseCode VARCHAR(20) = CASE WHEN ISNULL(@pSPWarehouseCode,'') = '' THEN '*' ELSE @pSPWarehouseCode END
		...
		select SparePartCode, workCenterCode,SPWarehouseCode, SparePartIOTypeCode, 
			 sum (ProcessQty)  as qty
			into #aa
			from STB_VNSparePartIOHistory where   SparePartIOTypeCode= 'GI_REPARE'
			Group By SparePartCode,workCenterCode,SPWarehouseCode,SparePartIOTypeCode
			...
			select SparePartCode, workCenterCode,SPWarehouseCode, SparePartIOTypeCode, 
			 sum (ProcessQty)  as qty
			into #bb
			from STB_VNSparePartIOHistory where   SparePartIOTypeCode= 'GR_NORMAL'
			Group By SparePartCode,workCenterCode,SPWarehouseCode,SparePartIOTypeCode
			...
			;with mytable as (
			select 
				isnull( #aa.SparePartCode, #bb.SparePartCode) as SparePartCode ,
				isnull(#aa.workCenterCode,#bb.workCenterCode) as workCenterCode,
				isnull(#aa.SPWarehouseCode,#bb.SPWarehouseCode) as SPWarehouseCode,
				CASE
						WHEN #aa.qty is null THEN 0
						ELSE #aa.qty
				END as soluongxuat ,
				CASE
						WHEN #bb.qty is null THEN 0
						ELSE #bb.qty
				END as soluongnhap
				
				
			from #aa FULL OUTER JOIN #bb 
			on #aa.SparePartCode = #bb.SparePartCode 
			and #aa.workCenterCode= #bb.workCenterCode
			and #aa.SPWarehouseCode= #bb.SPWarehouseCode
				...
				(tb.soluongnhap- tb.soluongxuat) as tonkho
				into #ccc
				from mytable tb 
	...
	(
		...
		select SparePartCode, workCenterCode, 
						CASE
							WHEN kho1 is null THEN 0
							
							ELSE kho1
						END as CurrentStockQtyKho1,
							CASE
							WHEN kho2 is null THEN 0
							ELSE kho2
						END as CurrentStockQtyKho2
					  from (
					select * from #ccc 
					...
					PIVOT ( Max (tonkho) for SPWarehouseCode in ([Kho1]  ,[Kho2])) as Demo
	...
	SELECT
	      ...
	      --  SPSI.CurrentStockQty,
			...
			(t1.CurrentStockQtyKho1 + t1.CurrentStockQtyKho2)as CurrentStockQty,
	        SPIOH.HistoryText,
	...
	FROM
			...
			left join table1 t1 on  SPIOH.SparePartCode = t1.SparePartCode and SPIOH.WorkCenterCode = t1.WorkCenterCode
	WHERE
	    ...
	    (SPIOTC.IOType = 'I') 
		AND 
		--SPIOH.Attribute1 IS NULL 
	     	--AND 
			SPIOH.BasicDate between @pFromDate and @pToDate
			AND 
			(@pSparePartCode is null or SPIOH.SparePartCode = @pSparePartCode)
