...
-- [프로시저 실행문]  usp_Medium_Production_Line_CEO_ZZZ   @pProcessUserID='kilee',@pProcessLanguage='Korean',@pMonth='2019-11-06', @pCompanyCode = 'VVT', @pSizeCode='0820',  @pRouteCode='V-28'

CREATE PROC [dbo].[usp_Medium_Production_Line_CEO_ZZZ] 		
...
-- 밑 상세현황부분  ---->
			...
			-- 1. 일일실적 (2019.10.11)
			SELECT 
			            CASE WHEN MAX(A.CompanyCode) = 'VNT' THEN '전주본사' 
	                           WHEN MAX(A.CompanyCode) = 'VVT' THEN '베트남'   ELSE '기타' END AS 사업장   
			        , A.사이즈 AS 사이즈
					, '일일실적(B)'  AS 용도
					, MAX(A.기준년월)   AS 기준년월
					--, A.LineCode   AS 라인코드
					, MAX(A.RouteCode) AS 공정코드
					, (SELECT SR.RouteName FROM STB_RouteInfo  SR WHERE SR.RouteCode = MAX(A.RouteCode)) AS 공정명							
			   ...
			   WHERE 1=1																									 				  
				...
				-- 2. 목표  (2019.10.11)  
				SELECT  CASE WHEN MAX(A.CompanyCode) = 'VNT' THEN '전주본사' 
	                           WHEN MAX(A.CompanyCode) = 'VVT' THEN '베트남'   ELSE '기타' END AS 사업장     
			               , A.사이즈 AS 사이즈
							, '목표(A)'  AS 용도
							, MAX(A.기준년월)   AS 기준년월
							--, A.LineCode   AS 라인코드
							, MAX(A.공정코드) AS 공정코드
							, (SELECT SR.RouteName FROM STB_RouteInfo  SR WHERE SR.RouteCode = MAX(A.공정코드)) AS 공정명							
							, MAX(A.CompanyCode) AS CompanyCode
							, SUM(월간계획) AS  총합			
							, SUM(Day26) AS Day26, SUM(Day27) AS Day27 , SUM(Day28) AS Day28, SUM(Day29) AS Day29, SUM(Day30) AS Day30
							, SUM(Day31) AS Day31
               ...
               GROUP BY  A.사이즈
				...
				FROM MEDIUM_PLAN A
				 INNER JOIN  MEDIUM_PROD B ON A.사이즈 = B.사이즈 
													  ...
													  AND A.CompanyCode = B.CompanyCode
													  AND A.LineCode = B.LineCode
				 LEFT OUTER JOIN STB_RouteInfo C  ON A.공정코드 = C.RouteCode 
				WHERE 1=1						
				  AND A.기준년월 = @Month	                                                                                                 -- 원본삭제금지			
...
WHERE 1=1
  AND (사이즈 like @SizeCode OR 사이즈 = '')
  AND 기준년월 = REPLACE(SUBSTRING(CONVERT(VARCHAR(10),  @pMonth , 121), 0, 8), '-', '') 	
  AND (공정코드 IN ('E-28', 'V-28'))
ORDER BY 사이즈
        ,CASE WHEN 사업장 = '계획합계' THEN '0'
		      WHEN 사업장 = '실적합계' THEN '00'
			  WHEN 사업장 = '소계' THEN '힣' ELSE 사업장 END
		,공정코드
		,용도
 ...
 --  SELECT * FROM MEDIUM_PLAN WHERE 기준년월 = '201911' AND  사이즈 = '0820'  AND 공정코드 IN ('E-28','V-28')