...
-- 프로시저 실행 :                    usp_VPCBeforeDailyTotalProd_SMS_Curing  '',''
-- ==============================================================================

CREATE PROCEDURE [dbo].[usp_VPCBeforeDailyTotalProd_SMS_Curing]
   ...
   FROM  (  
                ...
                --  [1] 권취누계  									
					SELECT 'VPC권취합계 ' As LineCode
							, Format(Convert(BIGINT, 152000),  'N0')AS PlanQty                                         -- 고정값 세팅
							, Convert(Varchar(10), DateAdd(Day, -1, GetDate()), 121)  As ProdDateTime
							, '권취' As RouteName
							, '[권취합계]' As LineName
							, Format(CONVERT(BIGINT, Sum(A2.ProdQty)),  'N0') AS ProdQty 
							, Round(Convert(NUMERIC(20,2), (Sum(A2.ProdQty) / 152000.00 * 100)), 1) AS ProdRate                  -- 고정값 세팅													
					 FROM (
										SELECT  B.LineCode
   	             ...
   	             --  [2] 커링누계 Start  									
				        UNION ALL
							               ...
							               SELECT 'VPC조립합계 ' As LineCode
													, Format(Convert(BIGINT, 148000),  'N0')AS PlanQty                                         -- 고정값 세팅!!!
													, Convert(Varchar(10), DateAdd(Day, -1, GetDate()), 121)  As ProdDateTime
													, '조립(커링)' As RouteName
													, '[조립합계]' As LineName
													, Format(CONVERT(BIGINT, Sum(A2.ProdQty)),  'N0') AS ProdQty 
													, Round(Convert(NUMERIC(20,2), (Sum(A2.ProdQty) / 148000.00 * 100)), 1) AS ProdRate          -- 고정값 세팅!!!													
													FROM (
																SELECT  B.LineCode
																		, IsNull(B.PlanQty, 0) As PlanQty
																		, A.RouteName						
