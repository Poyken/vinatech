...
-- Modified : 2020.02.19 ????? ?? (kilee)

			--2021.07.16 Mr.Tung , adding MaterialName, ProdQty, DefectQty



-- ??? : usp_RawMaterialInputFullHist_get '','','','','','','2022-02-10 00:00:00','2022-02-11 23:30:30','VVT'
...
BEGIN
	...
	Declare @ProductGroupCode VARCHAR(20) = CASE WHEN ISNULL(@pProductGroupCode, '') = '' THEN  '*' ELSE @pProductGroupCode END
		   ...
		   ,@CompanyCode      VARCHAR(20) = CASE WHEN ISNULL(@pCompanyCode,'') = '' THEN '*' ELSE @pCompanyCode END                          --2020.02.19 ????        
	...
	SELECT UPPER(RMIH.Barcode) AS Barcode
		  ...
		  ,LI.CompanyCode        -- 2020.02.19 ??  (kilee)
	 ...
	 WHERE (@ProductGroupCode ='*' OR RMIH.ProductGroupCode = @ProductGroupCode)
	  ...
	  AND ((@CompanyCode = '*') OR (LI.CompanyCode = @CompanyCode))                                           -- 2020.02.19 ??  (kilee)
