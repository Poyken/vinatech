...
BEGIN
   ...
   -- Mr.Tri?u ch?n n?u mà hàng Module thì không c?n nh?p

   -- Còn l?i ? nhà máy B?c Ninh và B?c Giang ngo?i tr? công do?n ngo?i quan dóng gói ra t?t c? d?u ph?i ch?n máy

   -- N?u v?i v?i con hàng Cutting thì k c?n công do?n V-33_BG
  ...
  IF(@WorkCenterCode in ('VVT_F1','VVT_F2') and @MachineCode='' and @PoType not in('MODULE') and @RouteCode not in('V-28','V-27', 'V-33', 'V-27_BG','V-28_BG','V-33_BG'))
		...
		BEGIN
			...
			EXEC usp_RaiseLocalizedError @pProcessLanguage, N'B?n ph?i ch?n thi?t b? th?c hi?n !...' 
	...
	-- Thêm báo l?i công do?n ngay khi tìm ki?m cho B?c giang 2 
   ...
   --    IF (ISNULL(@Count,0) = 0 AND ISNULL(@IsInputRoute,0) != 1 AND @WorkCenterCode = 'VVT_F4')
	  ...
	  -- BEGIN
			...
			--SET @ErrorMsg = N'Không dúng công do?n hi?n t?i'
   ...
   -- IF ((ISNULL(@ProdQty,0) <= 0 and @WorkCenterCode = 'VVT_F4') OR (@CompleteRoute = 1 AND @WorkCenterCode = 'VVT_F4'))
	...
	BEGIN
			...
			EXEC usp_RaiseLocalizedError @pProcessLanguage, '????? Lot???'
...
-- ?? Select?
	...
	SELECT
			...
			--CONVERT(BIT, MAX(CASE WHEN SI.MaterialCode = 'ECVT30-370' THEN CASE WHEN RMIH.RawBarcodeCount = 1 THEN 1 ELSE 0 END -- Mr.Trieu Added condition for worker 'ECVT30-370' (JangHai)
			                      --WHEN RMIH.RawBarcodeCount = RMIH.TotCount TH
EN 1 ELSE 0 END)) AS IsRawMaterialInputFinish,
