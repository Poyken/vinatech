...
if ($reader.Read() -and $reader['def'] -ne [DBNull]::Value) {
    [System.IO.File]::WriteAllText('c:\Users\User Vinatech.DESKTOP-RJJSEQU\Desktop\database\usp_Vietnam_RawMaterialInputHist_uid.sql', $reader['def'].ToString())
    Write-Host "Fetched usp_Vietnam_RawMaterialInputHist_uid.sql"
} else {
