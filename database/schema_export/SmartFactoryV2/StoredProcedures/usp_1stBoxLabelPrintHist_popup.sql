-- Procedure: usp_1stBoxLabelPrintHist_popup
-- =============================================
-- Author: Jackaroe(yjyu@vina.co.kr)
-- Create date: 2024-06-21
-- Browsable : true
-- Group :
-- Description:	소지함 출력내역 popup 조회용
-- =============================================
CREATE PROCEDURE [usp_1stBoxLabelPrintHist_popup] 
	@pProcessLanguage VARCHAR(20)
   ,@pProcessUserID VARCHAR(20)
AS
BEGIN
	--Declare @Barcode VARCHAR(50) = @pBarcode
	--       ,@LabelClassCode VARCHAR(50)

	--SELECT @LabelClassCode = LabelClassCode 
	--  FROM STB_VINAEnesolBoxLabelPrintHist
	-- WHERE Barcode = @Barcode

	--IF @LabelClassCode IS NULL BEGIN
	--	EXEC usp_RaiseLocalizedError @pProcessLanguage, '바코드가 존재하지 않습니다.'
	--	RETURN
	--END

	--IF @LabelClassCode <> '1' BEGIN
	--	EXEC usp_RaiseLocalizedError @pProcessLanguage, '소지함 라벨 정보를 선택해주시기 바랍니다.'
	--	RETURN
	--END
	...
	SELECT BLPH.VINAEnesolBoxLabelPrintHistNo
	  ...
	  LEFT OUTER JOIN VW_ModelBasicInfo MBI
	    ON MBI.ModelCode = BLPH.MaterialCode
	 WHERE BLPH.LabelClassCode = 1
	 ORDER BY BLPH.Barcode
END
GO
