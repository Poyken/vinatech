-- Procedure: usp_CheckedSlittingHN_get
-- =============================================
-- Author:		DinhManh
-- Create date: 2025-02-07
-- Description:	Get Material slitting after QC checked for transfer Warehouse
-- =============================================

-- usp_CheckedSlittingHN_get '', '', '2024-01-01', '2025-02-06', 'ML20241223000039'

CREATE PROCEDURE [dbo].[usp_CheckedSlittingHN_get]
...
BEGIN
	...
	-- Khi điền LotID thì tìm kiếm theo LotID

	IF @pLotID IS NOT NULL 
		BEGIN
		IF @pLotID LIKE 'SL%'		-- tìm kiếm theo mã con sẽ ra tất cả mã con của Lot đó
			BEGIN
				SELECT
						...
						(
						CASE 
							WHEN MM.MaterialName IS NULL THEN (Select		MM2.MaterialUnit
								from STB_MaterialMaster MM2
								left join STB_MaterialLotInfo MLI2 on MM2.MaterialCode = MLI2.MaterialCode
								where MLI2.LotID = MLI.PackingID )
						
							ELSE MM.MaterialUnit
						END
			...
			WHERE 1=1			
					...
					AND (MLI.MaterialWarehouseCode = 'SLITTING_HN_WH' OR MLI.MaterialWarehouseCode = 'NG_RAW_VN_WH')
					AND MLI.IsCheck IS NOT NULL

					AND MLI.PackingIdParent = (SELECT MLI2.PackingIdParent FROM STB_MaterialLotInfo MLI2 WHERE LotId = @pLotID)
			END

		ELSE		-- tìm theo mã cha sẽ ra tất cả mã con đã cắt
			BEGIN	
				SELECT
			...
			WHERE 1=1				
					...
					AND MLI.PackingIdParent = @pLotID
				
		END

	END	
		-- khi không điền LotID thì tìm kiếm theo ngày
	ELSE 
		BEGIN
			SELECT
...
--select * from STB_MaterialLotInfo where LotID='SL20250228000327'
GO
