...
--	---------------------------------------------------------------------------------------------------------------------------------
	 EXEC sp_xml_preparedocument @iDoc OUTPUT, @pXml
		...
		if(@IsSlitting =1)
		begin
			RAISERROR( N'Lot này đã chốt rồi không thể chốt nữa' ,16, 1)
			return
		end
		if(@SlittingQty>@InitialQtyPar or @SlittingQty < @InitialQtyPar)
		begin
			RAISERROR( N'Tổng số lượng lot slitting lớn hơn hoặc nhỏ hơn số lượng lot cha' ,16, 1)
			return
		end
