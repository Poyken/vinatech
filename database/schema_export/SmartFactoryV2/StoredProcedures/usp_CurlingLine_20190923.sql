-- Procedure: usp_CurlingLine_20190923
-- ==================================================================
-- Author      : kilee
...
--   EXEC [usp_CurlingLine] '','', '2019-09-03 08:30:00'                                             ---> 소스에서는 BETWEEN '2019-04-01 08:30:00' and '2019-04-02 08:30:00'  

Create PROC [dbo].[usp_CurlingLine_20190923] 
																	...
																	FROM 
																			 ...
																			 LEFT OUTER JOIN (
																											...
																											--AND 기준년월 = '201908'
																											 --AND 기준년월 = REPLACE(SUBSTRING(CONVERT(VARCHAR(10),  GETDATE() , 121), 0, 8), '-', '') 			 
																					                           AND 기준년월 = 	REPLACE(SUBSTRING(CONVERT(VARCHAR(10),  @pToDt , 121), 0, 8), '-', '') 	
																									) BB		ON   AA.라인코드 = BB.라인코드    --AND AA.라인명 = BB.라인명 
														...
														FROM (
															...
															-- 주간실적부분 Start
																				  SELECT   MAX(라인명)     AS 라인명	
																					...
																					(
			                                                                                -- ERP기준 주간수량 S
																								SELECT  CASE WHEN ISNULL(A.조립라인명, '')  = '셀1' THEN '셀 1호기'
																												  WHEN ISNULL(A.조립라인명, '')  = '셀2' THEN '셀 2호기'
																												  WHEN ISNULL(A.조립라인명, '')  = '셀3' THEN '셀 3호기' 
																												  WHEN ISNULL(A.조립라인명, '')  = '셀4' THEN '셀 4호기'
																												  WHEN ISNULL(A.조립라인명, '')  = '셀5' THEN '셀 5호기'
																												  WHEN ISNULL(A.조립라인명, '')  = '셀6' THEN '셀 6호기'
																												  WHEN ISNULL(A.조립라인명, '')  = '셀7' THEN '셀 7호기'
																												  WHEN ISNULL(A.조립라인명, '')  = '셀8' THEN '셀 8호기'
																												  WHEN ISNULL(A.조립라인명, '')  = '셀9' THEN '셀 9호기'
																												  WHEN ISNULL(A.조립라인명, '')  = '셀10' OR ISNULL(A.조립라인명, '')  = '셀99'  THEN '셀10호기' 
																								...
																								FROM (
																											...
																											WHERE 1=1																																																																		
																											   AND D.작업일자 > @FromDt AND D.작업일자 <= @ChangeTime                              -- 원본으로 절대삭제금지!!
																											  --AND D.작업일자 > '2019-09-01 08:30:00' AND D.작업일자 <= '2019-09-03 20:30:00'            -- TEST용 : FromDt는 오전8시반, ChangTime는 당일 오후20시반																												
																												--AND B.공정코드 = 'E-24'
																												AND B.공정코드 LIKE @RouteCode 
																					...
																					(
																								...
																								FROM (
																											...
																											WHERE 1=1																		
																												...
																												--AND D.작업일자 > '2019-09-01 20:30:00'  AND D.작업일자 <=  '2019-09-03 08:30:00'               -- TEST용 : FromDt는 오전8시반, ChangTime는 당일 오후20시반
								  ...
								  -- GG START  (현재 19라인)
							                 ...
							                 LEFT OUTER JOIN (
																			SELECT 라인코드
																				...
																				-- AND 기준년월 = REPLACE(SUBSTRING(CONVERT(VARCHAR(10),  GETDATE() , 121), 0, 8), '-', '') 			 
																				AND 기준년월 = 	REPLACE(SUBSTRING(CONVERT(VARCHAR(10),  @pToDt , 121), 0, 8), '-', '') 	
																	) BB		  ON AA.라인코드 = BB.라인코드  --ADN AA.라인명 = BB.라인명 
														...
														SELECT  CASE WHEN ISNULL(A.조립라인명, '')  = '셀1' THEN '셀 1호기'
																...
																FROM (
																			...
																			WHERE 1=1																		
																				...
																				--AND D.작업일자 > '2019-09-01 08:30:00' AND D.작업일자 <= '2019-09-03 20:30:00'																		  -- Test		
												  ...
												  SELECT   MAX(라인명)    AS 라인명	
													...
													(
																...
																FROM (
																			...
																			WHERE 1=1																	
																			   AND D.작업일자 > @ChangeTime AND D.작업일자 <= @ToDt	
																				--AND D.작업일자 > '2019-09-01 20:30:00' AND D.작업일자 <= '2019-09-03 08:30:00' 
