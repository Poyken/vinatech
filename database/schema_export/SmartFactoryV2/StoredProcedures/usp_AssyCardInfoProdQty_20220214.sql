-- Procedure: usp_AssyCardInfoProdQty_20220214
-- =============================================
...
-- [프로시저실행문]    	usp_AssyCardInfoProdQty_20220214  '','','VJMK053R850616'

-- =============================================
CREATE PROCEDURE [dbo].[usp_AssyCardInfoProdQty_20220214]
...
--   ECVT30-229
	...
	-- 실적등록정보
	 SELECT PRH.MaterialCode
		  ...
		  --, Case When PRH.RouteCode = 'E-29' Then PRH.CreateDateTime  Else PRH.ProdDateTime End As ProdDate          

		  ,  Case when PRH.RouteCode = 'E-28' Then 99 Else ROW_NUMBER() OVER (ORDER BY PRH.RouteCode) End AS Sort_no


		  ,  ROW_NUMBER () OVER ( ORDER BY PRH.RouteCode) AS ProdQtyFinishYn
      ...
      Order by  Case when PRH.RouteCode = 'E-28' Then 99 Else ROW_NUMBER() OVER (ORDER BY PRH.RouteCode) End ASC   --2022.02.14 정렬추가 (히스토리 순서로)
END


/*

select * from stb_setinfo where barcode = 'VJMK053R850616'

SELECT RouteCode
       ,  ProdDateTime
	   , CreateDateTime
	   , Case When RouteCode = 'E-29' Then CreateDateTime  Else ProdDateTime End As ProdDate22     
FROM STB_ProdRouteHist
WHERE 1=1
AND ControlNo = '20220205000022' and RouteCode in ('E-29', 'E-33')
