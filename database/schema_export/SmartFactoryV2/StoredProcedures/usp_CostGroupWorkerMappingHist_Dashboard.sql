-- Procedure: usp_CostGroupWorkerMappingHist_Dashboard
-- =============================================
...
-- =============================================
CREATE PROCEDURE usp_CostGroupWorkerMappingHist_Dashboard
	...
	SELECT FIN.OldJobDate
	  ...
	  FROM (
			SELECT A.JobDate AS OldJobDate
				  ...
				  ,CASE WHEN ISNULL(A.TotApplyTime, 0) = 0 
				        THEN (CASE WHEN A.SpecialWork > 0 THEN A.SpecialWork + A.SpecialWorkExtend 
						...
						ELSE 
							  (CASE WHEN A.SpecialWork > 0 THEN A.SpecialWork + A.SpecialWorkExtend 
									ELSE CASE WHEN A.BasicWorkTime IS NULL AND A.AbnormalContents = '정상'  THEN 8 - ISNULL(A.Late,0) - ISNULL(A.LeaveEarly,0)
											  WHEN A.BasicWorkTime IS NULL AND A.AbnormalContents <> '정상' THEN 0
										 ELSE A.BasicWorkTime END
									END + A.Extend) / A.TotApplyTime * A.ApplyTime 
				   END AS WorkTimeHour
				  ,A.CreateDateTime
				  ,A.CreateUserID
				  ,A.ChangeDateTime
			  ...
			  FROM (
				SELECT CGWMH.JobDate
					 ...
					 , BC3.Description AS WorkGroupName
					  ...
					  ,ISNULL(CGI2.LineCode, CGI.LineCode) AS LineCode
					  ,ISNULL(LI.LineDesc, LIOri.LineDesc) AS LineName
				  ...
				  LEFT OUTER JOIN STB_LineInfo LI 		    ON CGI2.LineCode = LI.LineCode
				  LEFT OUTER JOIN STB_LineInfo LIOri 		    ON CGI.LineCode = LIOri.LineCode
