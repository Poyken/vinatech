-- Procedure: pop_Electrode_RollPressing_spec_iud
CREATE PROCEDURE [dbo].[pop_Electrode_RollPressing_spec_iud]
...
BEGIN
    ...
    BEGIN
        ...
        -----------------------------------------------------
        -- MERGE: 프레스 시각검사 등록/갱신
        -----------------------------------------------------
        MERGE INTO STB_ElectrodeRollPressingVisualInspectionInfo AS Target
        USING (
            SELECT
        ...
        WHEN NOT MATCHED THEN
            INSERT (
                ElectrodeLotNumber, MeasureTimeCode, Seq,
                LeftValue, MiddleValue, RightValue,
                CreateDateTime, CreateUserID
            )
            VALUES (
                Source.ElectrodeLotNumber, Source.MeasureTimeCode, Source.Seq,
