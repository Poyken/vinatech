-- Procedure: ExportWarehouseFinshGood_RD_HN_uid
-- =============================================
-- Author:
-- Create date:
-- Description:	
...
CREATE PROCEDURE [dbo].[ExportWarehouseFinshGood_RD_HN_uid]
	...
	BEGIN TRY
        ...
        SELECT 
           ...
           WHERE MergeParentId = @pPackingID;
            ...
            BEGIN
                -- Tạo mã xuất kho
                EXEC SmartFramework.dbo.usp_DoCreateSerial 'STB_VN_RND_FINISHGOODS_HN_Export', @CodeExport OUTPUT;
                SET @CodeExport = 'EXVNRDHN' + @CodeExport;
				...
				IF NOT EXISTS(SELECT 1 FROM STB_VN_RND_FINISHGOODS_HN WHERE PackingID=@LoopPackingID and StatusImport=1)
				BEGIN
                ...
                FROM STB_VN_RND_FINISHGOODS_HN 
                WHERE PackingID = @LoopPackingID AND LotNo = @LotNo;
                ...
                INSERT INTO STB_VN_RND_FINISHGOODS_HN_Export (
                ...
                -- Cập nhật tồn kho
                UPDATE STB_VN_RND_FINISHGOODS_HN
        ...
        BEGIN
            ...
            FROM STB_VN_RND_FINISHGOODS_HN_Export 
            WHERE PackingID = @pPackingID ;
			
			-- check điều kiện xem đã nhập kho với tem nhỏ
		   IF NOT EXISTS(SELECT 1 FROM STB_VN_RND_FINISHGOODS_HN WHERE PackingID=@pPackingID and StatusImport=1)
			BEGIN
				  SET @Err = N'Packing này của bạn chưa được nhập kho: ' + @LoopPackingID;
                  RAISERROR(@Err, 16, 1);
		  END
...
--select * from STB_VN_RND_FINISHGOODS_HN where PackingId in ('PKQK1300171','PKQK1300168','PKQK1300169','PKQK1300167','PKQK1300170')

--SELECT * FROM STB_DividePackaging WHERE PackingID = 'PKQJ2800143_02'


--select * from STB_VN_RND_FINISHGOODS_HN where PackingId='PKPU0900200_02'

--select * from STB_VN_RND_FINISHGOODS_HN_Export where PackingId='PKQJ2800143_02'
