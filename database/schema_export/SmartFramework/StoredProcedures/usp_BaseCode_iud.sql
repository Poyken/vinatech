...
-- Browsable : true
-- Group : 시스템 관리
-- Description:	기본코드 관리 IUD
-- Modified:
-- =============================================
CREATE PROCEDURE [dbo].[usp_BaseCode_iud]
...
BEGIN
    ...
    DECLARE @MaxKeyField VARCHAR(50)
        ...
        BEGIN TRY
            ...
            WHILE 1 = 1 BEGIN
                ...
                IF @IUD_FLAG = 'INSERT' BEGIN
                    ...
                    IF EXISTS (SELECT 1 FROM STB_BaseCode WHERE CodeGroup = @CodeGroup) BEGIN
						RAISERROR('Duplicate Data : KeyField = %s', 16, 1, @CodeGroup)
					END

                    IF @IsAutoKey = 1 BEGIN
						SELECT
								@MaxKeyField = MAX(CodeGroup)
						FROM
								STB_BaseCode 
						WHERE
								CodeGroup LIKE @PrefixString + '%'
													
						IF @MaxKeyField IS NULL BEGIN
						    SET @CodeGroup = @PrefixString + RIGHT(REPLICATE('0',@SerialLen) + '1', @SerialLen)
						END ELSE BEGIN
						    SET @CodeGroup = @PrefixString + RIGHT(REPLICATE('0',@SerialLen) + CONVERT(VARCHAR, CONVERT(BIGINT, RIGHT(@MaxKeyField, LEN(@MaxKeyField) - LEN(@PrefixString))) + 1), @SerialLen)
						END
                    END
