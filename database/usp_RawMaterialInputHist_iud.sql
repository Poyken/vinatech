...
--                 2020.12.26 ?? BOM?? Check, VET????? Check (???)
...
----------------[ Insert?? Check??]  ------------------------------------------------------------------ ???
			...
			-- 1.  Barcode? ???? ??? ??? VET???? Check
					...
					Select  @MotherCode =  SI.MaterialCode 
							...
							, @MaterialType = VM.MBIExtText03        -- ?? VET ??

							, @MotherVolt   = VM.MBIExtText04        -- ??? ?? (2020.02.09 ?? - ???? ??)
              ...
              -- 2. ?????? ???? ?????? VET?? ??
						...
						Where (LotID = @RawMaterialBarcode) Or (LotNo = @RawMaterialBarcode)                                                                            -- ??????? ???????? ??????? ??
               ...
               --  3. ?????? ?????? ??  (2020.02.09 ?? - ???? ??)
						 ...
						 Select @ChildVolt = MM.MMExtText01                          -- ??(V) [A230 ????] ??? : 2.7 Or 3.0
              ...
              -- 2. ?????? ???? ?????? VET?? ??
			   ...
			   -- 4. Check?? 
				...
				-- 5. ??? ?? ??? BOM ?? START
						...
						-- ??/?? ???? Barcode ??? PO??? BOM? ????? ?? 2025.03-18 by Jackaroe
						...
						IF @ProductGroupCode = 'ElectrodeP'
							...
							AND ESR.Barcode = @RawMaterialBarcode
								...
								IF @@ROWCOUNT = 0
									...
									RAISERROR('???? ??? ??(+)? ???? ????. %s ?? ??????.',16,1, @ProductGroupCode)
							...
							SELECT BOMCHECK.SlittingMaterialCode
							   ...
							   --IF @@ROWCOUNT = 0
								 ...
								 --  BEGIN
									...
									--RAISERROR('???? ?? BOM? ???? ?? ???????. %s ?? ??????.',16,1, @ProductGroupCode)
							   ...
							   (
								...
								SELECT TOP 1 MM.MaterialCode 
								 ...
								 WHERE MM.DelegateMaterialCode IS NOT NULL
								   ...
								   --AND MLI.MaterialWarehouseCode = 'ROUTE_WH' --????? ???? ??? ?? ??
