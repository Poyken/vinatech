...
$sql = "SELECT * FROM stb_MergeBoxReality WHERE CreateDateTime >= '$startTime' ORDER BY CreateDateTime DESC"
...
try {
    ...
    if ($dt.Rows.Count -eq 0) {
        Write-Host "Không có dữ liệu MergeBoxReality nào trong 1 giờ qua."
    } else {
