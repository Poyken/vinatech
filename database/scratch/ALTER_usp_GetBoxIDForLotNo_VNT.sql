...
/****** Object:  StoredProcedure [dbo].[usp_GetBoxIDForLotNo_VNT]    Script Date: 2026-05-06 ******/
SET ANSI_NULLS ON
GO
...
-- 2022.03.21 작업일자 자동
-- 2026.05.06 AI: Added -D, -T, -DT suffixes for Low ESR and Temp models.
-- ====================================================================================
ALTER PROCEDURE [dbo].[usp_GetBoxIDForLotNo_VNT]                                  
	...
	Begin 			
		 ...
		 @PartN0 = (RTRIM(LTRIM(SUBSTRING(MM.MaterialName, CHARINDEX(' ', MM.MaterialName), 12))) 
				   +  CASE WHEN CHARINDEX('-L', MM.MaterialName) > 0 THEN '-L' 
						   ...
						   WHEN CHARINDEX('-DT', MM.MaterialName) > 0 THEN '-DT' -- AI ADD: Low ESR / Temp
						   WHEN CHARINDEX('-D', MM.MaterialName) > 0 THEN '-D'   -- AI ADD: Low ESR
						   WHEN CHARINDEX('-T', MM.MaterialName) > 0 THEN '-T'   -- AI ADD: Temp
						   ELSE '' END ),
