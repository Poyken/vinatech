...
function Search($db) {
    $cs = "Server=$server;Initial Catalog=$db;User ID=$user;Password=$pass;TrustServerCertificate=True;Connect Timeout=30;"
    ...
    try {
        ...
        while ($reader.Read()) {
            ...
            Write-Host "--- Database: $db | Schema: $($reader['S']) ---"
        ...
        # Search for any other SP/Fn containing the string 'PBDM00-186'
        $cmd.CommandText = "
        ...
        "
        $reader = $cmd.ExecuteReader()
        Write-Host "--- Other objects containing 'PBDM00-186' in $db ---"
        while ($reader.Read()) {
        ...
        Write-Host "Error in $db: $($_.Exception.Message)"
    } finally {
        $conn.Close()
    }
}
