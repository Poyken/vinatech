...
$sql = "SELECT ProductionOrderNo, MaterialCode, MaterialName, OrderQty, OrderDate 
        FROM STB_ProductionOrderInfo 
        WHERE ProductionOrderNo = '$targetCode'"
...
try {
    ...
    if ($dt.Rows.Count -eq 0) {
        Write-Host "Không tìm thấy ProductionOrderNo: $targetCode."
    } else {
