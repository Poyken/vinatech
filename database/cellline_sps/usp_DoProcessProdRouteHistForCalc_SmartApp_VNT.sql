...
BEGIN
	...
	if(@Barcode like 'VE%' and @WorkCenterCode1 ='VVT_F3' and @RouteCode in ('VE01','VE04','VE08','VE03'))
	begin
		exec usp_CheckPQCInputForProductHistForBarcode @Barcode,@RouteCode
	end
	--end
...
-- 일괄등록 금지! 마지막 등록 시간 기준 20분 이후 실적등록 가능 2021. 10. 26 SJC
	...
	-- Mr.Triều kiểm tra xem có phải là công đoạn cuối hay không nếu không phải là tiếp tục(2026-04-04)
	  ...
	  */
		...
		BEGIN
				EXEC usp_RaiseLocalizedError @pProcessLanguage,N'Routing này không có trong PO (Phần Routing sản xuất)'
				RETURN
		END
