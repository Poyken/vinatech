...
CREATE PROCEDURE [dbo].[usp_DoChangeMaterialDocLotInfo]
	...
	--END
		...
		--Lấy nhà cung cấp
			...
			--End lấy nhà cung cấp
						...
						where  (replace(isnull(LotAttr10,''),' ','')='' or ISDATE(LotAttr10)=0) and isnull(LotNo,'')<>''
						...
						UPDATE STB_MaterialLotInfo  
						 SET   LotAttr10 =  [dbo].[fn_VVT_getdatebyVendorLot_MergeCode](materialcode,LotNo,isnull(@SourceCustomerCode,''))  
						where   (replace(isnull(LotAttr10,''),' ','')='' or ISDATE(LotAttr10)=0)
						and isnull(LotNo,'')<>''
						and MaterialLocationCode not like 'PROD%'
						and (MaterialLocationCode like '%VN_WH' or MaterialLocationCode like '%BG_WH')
						--- Mr.tung 2023-11-21 
				...
				if(@PackDate is not null  )
					begin try 
					 ...
					 if(replace(isnull(@LotNo,''),' ','')<>'')
						...
						WHERE LotID=@LotId
						and  (replace(isnull(LotAttr10,''),' ','')='' or ISDATE(LotAttr10)=0)
