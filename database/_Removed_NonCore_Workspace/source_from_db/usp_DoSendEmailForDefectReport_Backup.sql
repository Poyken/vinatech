...
-- Modified: 
-- 2020.10.15  베트남어 추가
-- 2020.10.20 베트남 작업자명 누락원인 해결

 -- 프로시저 실행 :  usp_DoSendEmailForDefectReport '','','VN200924-02'
-- =============================================
Create PROCEDURE [dbo].[usp_DoSendEmailForDefectReport_Backup]
...
BEGIN
	Declare @DefectReportNo VARCHAR(20) = @pDefectReportNo
		   ...
		   -- 생산부문, 베트남부문
		   ,@ToAddress VARCHAR(MAX) = 'v.manufacturing@vina.co.kr;v.vietnamcorp@vina.co.kr;'
		   -- 품질부문
		   ,@CcAddress VARCHAR(MAX) = 'v.qc@vina.co.kr;'
	...
	SELECT @DefectDivisionName = BC1.Description
		 ...
		 -- ,@WorkerName = IsNull(EI2.EmployeeName,  ( select CreateUserID from STB_ProdWorkerInfo	SPW where SPW.workerCode = QDR.PublishEmpID ))    
		  , @WorkerName = isnull(EI1.EmployeeName, isnull((select top 1 WorkerName from [SmartFactoryV2].[dbo].[STB_ProdWorkerInfo] where WorkerCode=QDR.PublishEmpID),QDR.PublishEmpID) )   --2020.10.20 수정 (엄제식요청)
		  ,@JobDate = QDR.JobDate
		  ...
		  ,@DefectRate = CONVERT(NUMERIC(20,2), CONVERT(NUMERIC(10,3), QDR.DefectErrorCnt) / CONVERT(NUMERIC(10,3), QDR.DefectLotSize) * 100)
	 ...
	 -- 수신처에 따라 메일제목 변경
	 SET @MailSubject = CASE WHEN @ReceiveDeptName IN ('생산부문(베트남법인)', '베트남법인') 
	...
	IF @ReceiveDeptName IN ('베트남법인', '생산부문(베트남법인)') BEGIN
		SET @ToAddress = @ToAddress + 'v.vietnamlocalstaff@vina.co.kr;'
	END
 ...
 --   SET @EmailBody = @EmailBody + '<body>'
    SET @EmailBody =                     N'    <table width="1000" cellspacing="1" cellpadding="1" border="1">'
    SET @EmailBody = @EmailBody + N'      <colgroup>'
    SET @EmailBody = @EmailBody + N'        <col width="16%"><col width="16%"><col width="16%"><col width="16%"><col width="16%"><col width="20%"> '
    SET @EmailBody = @EmailBody + N'      </colgroup>'
    ...
    SET @EmailBody = @EmailBody + N'          <td rowspan="2" colspan="3" valign="middle" align="center"><font size="+3"><b>부적합품 보고서<br/><font size="+1">(BÁO CÁO SẢN PHẨM KHÔNG PHÙ HỢP)</b></font></td>'
    ...
    SET @EmailBody = @EmailBody + N'          <td valign="top" align="center" bgcolor="#ccccff"><b>설비명<br/>(Tên thiết bị)</b></td>'
    SET @EmailBody = @EmailBody + N'          <td valign="top" align="center" bgcolor="#ccccff"><b>작업자<br/>(Công nhân thao tác)</b></td>'
    ...
    SET @EmailBody = @EmailBody + N'          <td rowspan="1" colspan="3" valign="middle" align="center" bgcolor="#ccccff"><b>부적합명 <br/>(Tên lỗi)</b></td>'
    SET @EmailBody = @EmailBody + N'        </tr>'
    SET @EmailBody = @EmailBody + N'        <tr height="20">'

 -- SET @EmailBody = @EmailBody + N'          <td valign="top" align="center">-</td>'        -- 원본백업
	SET @EmailBody = @EmailBody + N'          <td valign="top" align="center">' + ISNULL(@MaterialSpec, '') + '</td>'      --2020.10.20 추가
    ...
    SET @EmailBody = @EmailBody + N'          <td rowspan="1" colspan="3" valign="top" align="center">' + ISNULL(@DefectName, '') + '</td>'
    ...
    SET @EmailBody = @EmailBody + N'          <td rowspan="1" colspan="2" valign="top" align="center" bgcolor="#ccccff"><b>부적합 현상<br/>(Hiện trạng lỗi)</b></td>'
    SET @EmailBody = @EmailBody + N'        </tr>'
    SET @EmailBody = @EmailBody + N'        <tr>'
    ...
    SET @EmailBody = @EmailBody + N'          <td rowspan="4" colspan="2" valign="middle" align="center">' + CASE WHEN @TIMESTAMP IS NULL 
	                                                                                                             THEN 'No Image' 
