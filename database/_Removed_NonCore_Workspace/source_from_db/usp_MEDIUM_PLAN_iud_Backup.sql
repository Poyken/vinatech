...
-- Browsable : true
-- Group : 생산관리 > [B155] 중형생산목표 
-- Description:	
-- Modified: Grid 수정 업데이트
-- =============================================
Create PROCEDURE [dbo].[usp_MEDIUM_PLAN_iud_Backup]
...
BEGIN
	...
	--DECLARE @BaseYm  VARCHAR(10) =REPLACE(SUBSTRING(CONVERT(VARCHAR(10), @pBaseYm, 121), 1, 7), '-', '')                       -- kilee 추가
  ...
  --DECLARE @기준년월 VARCHAR(12) =   REPLACE(SUBSTRING(CONVERT(VARCHAR(10), @pBaseYm, 121), 1, 7), '-', '')   
  DECLARE @사이즈 VARCHAR(20)
  DECLARE @월간계획 INT
  DECLARE @공정코드 VARCHAR(10)                                                                                                                            --kilee 추가
  DECLARE @Day01 INT
  DECLARE @Day02 INT
  DECLARE @Day03 INT
  DECLARE @Day04 INT
	    ...
	    BEGIN TRY
			...
			USING
				(
					SELECT
							...
							--CASE WHEN Old기준년월 IS NULL THEN  @BaseYm  ELSE Old기준년월	END AS Old기준년월,
			...
			WHEN MATCHED THEN
				UPDATE SET
					...
					--기준년월 = '201904',
					--기준년월 = REPLACE(SUBSTRING(CONVERT(VARCHAR(10), @pBaseYm, 121), 1, 7), '-', ''),
			...
			ON
				(
					...
					TargetTable.사이즈 = SourceTable.Old사이즈
				)
			...
			ON
				(
					...
					TargetTable.사이즈 = SourceTable.사이즈
				)
        ...
        BEGIN TRY
            ...
            WHILE 1 = 1 BEGIN
                ...
                IF @IUD_FLAG = 'INSERT' BEGIN
                    ...
                    IF EXISTS (SELECT 1 FROM MEDIUM_PLAN WHERE 기준년월 = @기준년월 AND 사이즈 = @사이즈) 
