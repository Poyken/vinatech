...
-- Exec :  usp_MaterialWarehouseInOutHist_iud_GBKCCA002_Jackaroe 'kilee','Korean','VNT','VNT_F1','','','','','','202600120#2200039818#H219112362#32#DLC3702'
-- =============================================================================================
CREATE PROCEDURE [dbo].[usp_MaterialWarehouseInOutHist_iud_20200513]
	  ...
	  -- 2020.04.09 중복된 바코드 처리요청 (구보겸) + 2020.04.13 타겟창고에 따라 조건문추가 
		...
		BEGIN 
				SELECT  TOP 1 @MaterialLotNo = SM.MaterialLotNo 
								...
								, @MaterialDocDetailNo = SM.MaterialDocDetailNo								
				  FROM (
							SELECT  SML.MaterialLotNo                                                                        
									 ...
									 , SMD.MaterialDocDetailNo	                                                                   AS MaterialDocDetailNo
...
---------------------------------------------------------	
   ...
   --IF @@ROWCOUNT = 0                  -- 매핑이 안된 LotNo인 경우
	...
	--Declare @MaterialLotNo2                     VARCHAR(20)

   --2020.05.12 추가사항 (활성탄 두 제품의 경우에는 바코드를 찍으면 일괄처리되도록 - 구보겸요청)
	   IF @MaterialCode IN ('GAKCCA-002', 'GAKCCA-003','GATCCC-001') 	        BEGIN          --예외처리 제품인 경우
	...
	END
	   ...
	   IF @MaterialCode NOT IN ('GAKCCA-002', 'GAKCCA-003','GATCCC-001') 	BEGIN    -- 정상제품인 경우
	   ------------- 여기까지
