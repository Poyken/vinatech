...
-- EXEC usp_GetSetListForOqcLot_VNT_TEST @pProcessUserID='kilee',@pProcessLanguage='Korean',@pCompanyCode='VVT',@pWorkCenterCode='VVT_F1',@pLineCode='',@pBarCode = 'VVKK093R033504'      ---베트남 제품검사 LOT관리 (되는것)
-- EXEC usp_GetSetListForOqcLot_VNT_TEST @pProcessUserID='kilee',@pProcessLanguage='Korean',@pCompanyCode='VVT',@pWorkCenterCode='VVT_F1',@pLineCode='',@pBarCode = 'VVKK122R740611'      ---베트남 제품검사 LOT관리 (안되는것)
-- =============================================


CREATE PROCEDURE [dbo].[usp_GetSetListForOqcLot_VNT_TEST]
...
BEGIN
	...
	(
		...
		FROM
				...
				INNER JOIN STB_ProductionOrderRouting POR WITH(NOLOCK)		ON POR.PONo = SI.PONo               AND					POR.RouteCode IN ('V-22')
				...
				INNER JOIN STB_ProdRouteHist PRH WITH(NOLOCK)					   ON PRH.ControlNo = SI.ControlNo     AND					PRH.RouteCode IN ('V-22')
		WHERE 1=1
	...
	SELECT
			...
			WHEN SL.ComPanyCode = 'VVT' THEN '베트남' ELSE '기타' END   AS 사업장,
			SI.ControlNo,
			SI.PONo,
			SI.DayPlanNo,
