...
-- [프로시저 실행문] :  usp_GetCommInspectionHistoryForBarcode_20220210   'kilee', 'Korean' ,'ROUTE_TEST_VPC' ,'VJMK103R850612', '', 'E-24', '' ,'' ,'' ,''   ,''         
 --                         usp_GetCommInspectionHistoryForBarcode_20220210   'kilee', 'Korean' ,'ROUTE_TEST_VPC' ,'VJMK103R850612', '', '', '' ,'' ,'' ,''   ,''         
-- =========================================================================================================
CREATE PROCEDURE [dbo].[usp_GetCommInspectionHistoryForBarcode_20220210]
...
BEGIN
 ...
 -- DECLARE @ProcessSteps  VARCHAR(20) = @pProcessSteps   -- 2022.01.13 추가
	...
	SELECT @Size = CASE WHEN MBI.MBISizeW IN (22, 36) THEN 'L' ELSE 'SM' END
	 ...
	 WHERE ModelCode = (SELECT MaterialCode FROM STB_SetInfo WHERE Barcode = @Barcode OR Barcode = (SELECT NewBarcode  FROM STB_LotChangeMaterialHistory WHERE OldBarcode = @Barcode ))
	...
	-- VPC제품때문에 화면을 두개로 하면서 체크하는 부분임!!
		IF @CommInspTypeCode = 'ROUTE_TEST' AND @MaterialCode IN ('LIVT38-018', 'LIVT38-019')    --#211007 ROUTE_TEST_VPC != LIVT-018 이거나 ROUTE_TEST == LIVT-018 인 경우를 체크함.
	
		BEGIN
			EXEC SmartFramework.dbo.usp_GetAddonStringResource	@ProcessLanguage, '^VPC 0825제품입니다. [자주검사(VPC)] 화면에서 입력하세요.^', 	@ErrorMessage OUTPUT
					SET @ErrorMessage = @ErrorMessage + ' [%s]'
					RAISERROR(@ErrorMessage,16,1,@Barcode)
					RETURN
		END

		IF @CommInspTypeCode = 'ROUTE_TEST_VPC' AND @MaterialCode NOT IN ('LIVT38-018', 'LIVT38-019') 
	
		BEGIN
			EXEC SmartFramework.dbo.usp_GetAddonStringResource	@ProcessLanguage, '^VPC 0825제품이 아닙니다. 기존 자주검사 화면에서 입력하세요.^', @ErrorMessage OUTPUT
					SET @ErrorMessage = @ErrorMessage + ' [%s]'
					RAISERROR(@ErrorMessage,16,1,@Barcode)
					RETURN
		END
	...
	SELECT
		...
		--	, SCH.CIDHExtInt03   AS ProcessSteps   --2021.10.20 추가사항
			...
			, SCH.CreateDateTime AS InspDateTime
			, CII.RouteCode AS RouteCode
			, CII.CommInspItemCode  as aa
			, CIDI.CommInspItemCode as bb
	FROM					  	  STB_CommInspDocItem CIDI
	...
	WHERE 1=1
     ...
     AND (CII.RouteCode IS NULL OR CII.RouteCode LIKE  '%' + @RouteCode + '%')    -- 2022.02.10 추가사항
	 --AND (   CII.RouteCode LIKE  '%' + @RouteCode + '%')    -- 2022.02.10 추가사항
	 -- AND CII.RouteCode = *
...
Select * from STB_CommInspItem where 
