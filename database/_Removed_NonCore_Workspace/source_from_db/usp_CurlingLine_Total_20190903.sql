...
--   EXEC [usp_CurlingLine_Total] '','', '2019-07-16 08:30:00'                                             ---> 소스에서는 BETWEEN '2019-04-01 08:30:00' and '2019-04-02 08:30:00'  

Create PROC [dbo].[usp_CurlingLine_Total_20190903] 
...
SELECT  SUM(월간계획) / @DayCnt                                                        AS 일일목표
...
FROM  (
				...
				SELECT 						
					...
					, CASE WHEN ISNULL(GG.월간계획, 0) / 31 * 100 = 0 THEN 0 ELSE  (ISNULL(SS.전체수량, 0) + ISNULL(GG.월누적수량, 0))  / (ISNULL(GG.월간계획, 0) / @DayCnt * @ToDay)  *100      END  AS 누계달성율		-- 누계생산 / 누계목표										 										
						...
						(  
							-- GG START  (현재 19라인)
										...
										(
												...
												WHERE 1=1																	
													--AND 기준년월 = REPLACE(SUBSTRING(CONVERT(VARCHAR(10),  GETDATE() , 121), 0, 8), '-', '') 		   --201904																												
																...
																FROM
																      (			  
																		 SELECT  CASE WHEN ISNULL(A.조립라인명, '')  = '셀1' THEN '01호기'
																			...
																			WHERE 1=1																		
																				-- AND D.작업일자 BETWEEN '2019-04-12 08:30:00' AND '2019-04-13 08:30:00'
