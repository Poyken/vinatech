...
-- [usp_vvt_MaterialLotInfo_get] '','','VVT', 'VVT_F1', 'ROH_VN_WH', '',  '',  '',  '',  '',  '',  '',  '',  '',  '',  '',  ''

-- [usp_MaterialLotInfo_get] '','','VNT', 'VNT_F1', 'ROH_WH', '',  '',  '',  '',  '',  '',  '',  '',  '',  '',  '',  ''

-- [usp_vvt_MaterialLotInfo_get_TEST4] '','','VVT', '', 'SLITTING_HN_WH', '',  '',  '',  '',  '',  '',  '',  '',  '',  '',  ''
-- [usp_vvt_MaterialLotInfo_get_TEST4] '','','VVT', '', 'ROH_HN_WH', '',  '',  '',  '',  '',  '',  '',  '',  '',  '',  ''

-- =========================================================================
CREATE PROCEDURE [dbo].[usp_vvt_MaterialLotInfo_get_TEST4]
...
BEGIN
	...
	--end
					...
					--- cập nhật dữ liệu Ngay Thang SX của Vendor cho những Lót bị trống Ngay Thang SX
						...
						and (MaterialLocationCode like '%VN_WH' or MaterialLocationCode like '%BG_WH' or MaterialLocationCode like '%HN_WH')
...
;WITH LABELINFO AS
	(
	 ...
	 basedat as (
			...
			or MaterialWarehouseCode= case when @MaterialWarehouseCode not in ('ROH_VN_WH','ROUTE_VN_WH','ROH_BG_WH','ROUTE_BG_WH') then @MaterialWarehouseCode else '' end
),  holddate as(
...
,table1 as (
			...
			CASE 
				--WHEN MLI.MaterialWarehouseCode like 'SLITTING_HN_WH%' and MLI.MaterialLocationCode like 'SLITTING_HN_WH%' and MLI.IsSlitting =1  THEN isnull(MLI.CurrentQty,mdli.StockQty)- LDS.StockSltiting
			...
			case 
			when  @MaterialWarehouseCode <>'ROH_VN_WH' and  @MaterialWarehouseCode <>'ROH_BG_WH'  and  @MaterialWarehouseCode <>'ROH_HN_WH' then   convert(varchar(10),inout.LatestCreateDateTime,120)
	...
	FROM
		...
		where  
		--lotid='SL20241225000086'--and
		lotid like 'SP%' and MaterialWarehouseCode LIKE @MaterialWarehouseCode
		or MaterialWarehouseCode= case when @MaterialWarehouseCode not in ('ROH_VN_WH','ROUTE_VN_WH','ROH_BG_WH','ROUTE_BG_WH','ROH_HN_WH','ROUTE_HN_WH') then @MaterialWarehouseCode else '' end
		and (@MaterialCode = '*' OR MaterialCode LIKE @MaterialCode) --Mr.Duy thêm cho tìm kiếm theo mã NVL
		and  ((IsSlitting is null and PackingIdParent is null) or (PackingIdParent is not null and IsSlitting is not null ) )
		) mdli --with(nolock) 
		left outer join STB_MaterialLotInfo mli  with(nolock) on mdli.LotID = mli.LotID
		left outer join STB_MaterialDocLotInfo mdli11  with(nolock) on mdli11.LotID = mdli.LotID
	...
	where  
			...
			--and (MLI.LotID = 'SL20241225000088')					   
