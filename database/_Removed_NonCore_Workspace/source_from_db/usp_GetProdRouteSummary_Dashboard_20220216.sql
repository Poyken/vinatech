...
--usp_GetProdRouteSummary_DashBoard_20220216  '','','','','','','','2022-02-14','2022-02-15','',0
-- ==========================================================================================
CREATE PROCEDURE [dbo].[usp_GetProdRouteSummary_Dashboard_20220216]
...
BEGIN
	...
	FROM
			...
			LEFT OUTER JOIN NextProd NP
			  ON NP.PONo = PRS.PONo
			 AND NP.RouteCode = PRS.RouteCode
			LEFT OUTER JOIN (
			...
			LEFT OUTER JOIN (SELECT CompanyCode 
			  ...
			  ON PRHWorker.CompanyCode = PRS.CompanyCode
			 AND PRHWorker.WorkCenterCode = PRS.WorkCenterCode
			 AND PRHWorker.JobDate = PRS.JobDate
			 AND PRHWorker.ShiftCode = PRS.ShiftCode
			 AND PRHWorker.PONo = PRS.PONo
			 --AND PRHWorker.MachineCode = PRS.MachineCode
			 AND PRHWorker.LineCode = PRS.LineCode
			 AND PRHWorker.RouteCode = PRS.RouteCode
			 LEFT OUTER JOIN STB_ProdWorkerInfo PWI
			   ON PWI.WorkerCode = PRHWorker.WorkerCode
