...
-- =============================================
CREATE PROCEDURE [dbo].[usp_GetMaterialLotInfo_BG2_Test]
...
BEGIN
	...
	IF EXISTS (select * from STB_MaterialLotInfo_TEST where LotID = @LotID and MaterialWarehouseCode <> 'ROUTE_BG2_WH')
	BEGIN
	RAISERROR(N'Các lot gộp cần phải cùng kho!', 16, 1);
	END
	...
	(
		SELECT
				MLI.MaterialLotNo
		FROM
				STB_MaterialLotInfo_TEST MLI WITH(NOLOCK)
		WHERE  LotId = @LotId
	...
	FROM
			Lot L
			INNER JOIN STB_MaterialLotInfo_TEST MLI WITH(NOLOCK)				ON	MLI.MaterialLotNo = L.MaterialLotNo
