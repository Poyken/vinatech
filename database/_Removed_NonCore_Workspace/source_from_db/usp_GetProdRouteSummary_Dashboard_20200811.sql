...
--                    usp_GetProdRouteSummary_DashBoard  '','','','','','','','2020-07-26','2020-08-25',''
-- =============================================

Create PROCEDURE [dbo].[usp_GetProdRouteSummary_Dashboard_20200811]
...
BEGIN
	...
	SELECT
			...
			CASE WHEN Replace(PRS.RouteCode, 'V', 'E') = 'E-22' THEN '권취'
				   ...
				   --WHEN Replace(PRS.RouteCode, 'V', 'E') = 'E-40' THEN '내전압'  
			--RI.RouteName,
			POR.RouteIndex,
			PRS.MoldNumber,
			PRS.MachineCode,
	   ...
	   -- , ISNULL(dbo.fnGetWastePrice(LI.LineCode, PRS.RouteCode, PRS.MaterialCode, SUM(ISNULL(PRS.DefectQty, 0) - ISNULL(RepairInfo.RepairQty, 0))), 0)                 AS DefectPrice
		  --, dbo.fnGetWastePrice(LI.LineCode, PRS.RouteCode, PRS.MaterialCode, SUM(ISNULL(RepairInfo.DefectQty, 0) - ISNULL(RepairInfo.RepairQty, 0)))                              AS DefectPrice
		  ...
		  --,dbo.fnGetWastePriceByMaterial(PRS.CompanyCode, PRS.WorkCenterCode, LI.LineCode, PRS.MaterialCode, 'PC', PRS.RouteCode, 1) AS MaterialPcUnitPrice
		  --,dbo.fnGetWastePriceByMaterial(PRS.CompanyCode, PRS.WorkCenterCode, LI.LineCode, PRS.MaterialCode, 'PC', PRS.RouteCode, SUM(ISNULL(RepairInfo.DefectQty, 0) - ISNULL(RepairInfo.RepairQty, 0))) AS MaterialPcPrice
