...
-- Browsable : true
-- Group : 품질관리 > 공정검사(바코드) > 공정검사Grid  업데이트문
-- Description:	공용 검사이력을 추가 합니다.
-- Modified: 바코드 입력시
-- 2020.01.13  비고정보 업데이트 되도록 수정 (kilee)
-- 2020.01.13  

-- [프로시저 실행문]   EXEC usp_DoAddCommInspMeasureHistForBarcode          'kilee','Korean','20191203000049','20191203001073',  '', '7.78', '' , '0705201'  , ''                                    --> Barcode : VJJU032R750604 의 첫번째 행에 INSERT됨

-- [프로시저 실행문]   EXEC usp_DoAddCommInspMeasureHistForBarcode_TEST    'kilee','Korean','20191203000049','20191203001073','',     '1.10','2.20','3.30','4.40','5.50','6.66','7.77','8.88','9.99','10.01'           , '' , '0705201'  , ''  
...
-- 테스트용 정보 : 바코드 :  VJJU032R750605 /  검사문서번호 : 20200104000051 /  공용검사항목이력번호 :  20200104001441
...
-- =============================================
Create PROCEDURE [dbo].[usp_DoAddCommInspMeasureHistForBarcode_TEST_20200706]
...
--- [Table구조]
-- SELECT *                    FROM STB_SetInfo                      WHERE Barcode                  IN ( 'VJKJ263R033501')                                                                           -- ControlNo = '20200126000024'    * ControlNo 확인
-- SELECT ItemTargetQty, * FROM STB_CommInspDocItem        WHERE CommInspDocNo      = '20200126000024'                       -- CommInspDocItemNo     23개존재
-- SELECT * FROM STB_CommInspMeasureHist   WHERE CommInspDocItemNo IN ('20200126000024')
-- (한줄로표기)   SELECT * FROM STB_CommInspDocHistory     WHERE CommInspDocNo      IN ( '20200126000024'  ) 

-- SELECT CIDHExtText02, CIDHExtText03, * FROM STB_CommInspDocHistory     WHERE CommInspDocNo      IN ( '20200126000005'  )                --20200126000005