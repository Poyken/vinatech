...
CREATE PROCEDURE [dbo].[usp_ProductStockInfoLookup_get_20210416]
...
SELECT  AA.ProductStockNo
		  ...
		  -- ,  (Convert(INT, AA.Diff) / 365) AS YEARS                 --년수추가
...
FROM (
  	...
  	SELECT A.ProductStockNo
		...
		--	,A.StockPrice
			...
			, A.MBISizeW   -- 파이추가
			--, Case When  LEN(Case When Len(A.Day) = 1 then   CONCAT(A.Year, A.Month, '0',  A.Day)  
			--	        Else   CONCAT(A.Year, A.Month, A.Day) End)  =  8 Then   DATEDIFF(dd,  Convert(Datetime, Case When Len(A.Day) = 1 then     
			--	  CONCAT(A.Year, A.Month, 0,  A.Day)  else   CONCAT(A.Year, A.Month, A.Day) End) , Getdate() )   
	  --        else 0 end as Diff
	             , A.ManufacturingUnitPrice
					...
					SELECT PSI.ProductStockNo
					  ...
					  FROM STB_ProductStockInfoUpload PSI
						...
						LEFT OUTER JOIN (SELECT ApplyDate, CompanyCode, WorkCenterCode, CostTypeCode, RouteCode
												   ...
												   AND MCR.RouteCode IN ('E-28', 'V-28')
						  ...
						  LEFT OUTER JOIN (SELECT ApplyDate, CompanyCode, WorkCenterCode, CostTypeCode, RouteCode
												   ...
												   AND MCRM.RouteCode IN ('E-28', 'V-28')
...
select * from STB_ManufacturingCostByRoute where MaterialCode = 'ECVT30-270' and CostTypeCode = 'PC'
