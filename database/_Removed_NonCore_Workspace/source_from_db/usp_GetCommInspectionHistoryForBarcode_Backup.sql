...
-- Browsable : true
-- Group : 품질관리
-- Description:	공용검사이력조회 >  [C440] 공정검사(Lot No)화면 (Get)조회프로시저 
-- Modified: 
--               2020.01.13 비고정보 변경 (kilee)
--               2022.01.13 검사항목추가 (RQ_StrippingAll)

-- [프로시저 실행문]
-- usp_GetCommInspectionHistoryForBarcode_Backup @pProcessUserID='kilee',@pProcessLanguage='Korean',@pCommInspTypeCode='ROUTE_QUALITY',@pBarcode='VJJR023R036718',@pLineCode=default,@pRouteCode=default,@pMachineCode=default,@pMoldNumber=default,@pCategoryName=default
-- usp_GetCommInspectionHistoryForBarcode_Backup   'kilee', 'Korean' ,'ROUTE_QUALITY' ,'VJMJ132R710606', '', '', '' ,'' ,'' ,''   ,'E-22'    --29
-- ===================================================================================================================================================
CREATE PROCEDURE [dbo].[usp_GetCommInspectionHistoryForBarcode_Backup]
...
BEGIN
	...
	DECLARE @ProcessSteps VARCHAR(10) = CASE WHEN ISNULL(@pProcessSteps,'') = '' THEN '*' ELSE @pProcessSteps END
	...
	IF @CommInspTypeCode = 'ROUTE_TEST' AND @MaterialCode IN ('LIVT38-018', 'LIVT38-019') BEGIN
		EXEC SmartFramework.dbo.usp_GetAddonStringResource	@ProcessLanguage,
	...
	IF @CommInspTypeCode = 'ROUTE_TEST_VPC' AND @MaterialCode NOT IN ('LIVT38-018', 'LIVT38-019') BEGIN
		EXEC SmartFramework.dbo.usp_GetAddonStringResource	@ProcessLanguage,
	...
	FROM					
			...
			LEFT OUTER JOIN STB_MachineMaster MM ON MM.MachineCode = SCH.VPCMachineCode
			LEFT OUTER JOIN STB_ProdWorkerInfo PWI ON PWI.WorkerCode = SCH.InspWorkerCode
	...
	WHERE 1=1
	  ...
	  AND (@IsExceptDestInsp = CONVERT(BIT, 0) OR (CII.CommInspItemCode NOT IN ('STRIPPING', 'LENGTHP', 'LENGTHM', 'THICKP', 'THICKM', 'tP', 'tM', 'WDecapInsp', 'RQV_V')))
	    ...
	    AND (CII.RouteCode IS NULL OR CII.RouteCode LIKE  '%' + @ProcessSteps + '%')    -- 2022.01.13 추가사항
	   -- AND CII.RouteCode = 'E-22' 
	   --AND  ( @ProcessSteps = '*') OR ( CII.RouteCode = @ProcessSteps)
