...
ALTER PROCEDURE [dbo].[usp_LotTrackingInfo_VVT2_get] 
...
;with ViewBarcode as (
		 ...
		 where b.CompanyCode='VVT' AND b.WorkCenterCode = @WorkCenterCode  and b.ProdDateTime>=@FromDate  
		 and b.ProdDateTime<@ToDate AND (
        -- Trường hợp 1: Nếu ToDate trước 2025-05-01, không cần check CompleteRoute
        @ToDate < CONVERT(VARCHAR(10), DATEADD(DAY, 1, CONVERT(smalldatetime, '2025-05-01')), 120) + ' 10:00:00'  
        OR 
        -- Trường hợp 2: Từ 2025-05-01 trở đi, bắt buộc CompleteRoute = 1
        ( @ToDate >= CONVERT(VARCHAR(10), DATEADD(DAY, 1, CONVERT(smalldatetime, '2025-05-01')), 120) + ' 10:00:00'   AND b.CompleteRoute = 1 )
		-- Modified: Chỉ tính riêng trường hợp chạy thử nghiệm POP với 2 line 1,2
		-- Author:Mr.Triều(2026-09-14)
		-- Desc: Lấy mốc thời gian chạy thử là tháng 9 là, 2 line chạy thử nghiệm
		-- ✔ Key: Vì hệ thống POP sau khi hoàn thành công đoạn chỉ cho phép công đoạn đóng gói ghi nhận là CompleteRoute=1 trong bảng STB_ProdRouteHist
		OR
		( @ToDate >= CONVERT(VARCHAR(10), DATEADD(DAY, 1, CONVERT(smalldatetime, '2026-09-01')), 120) + ' 10:00:00' )  --AND c.InputLineCode in ('VVC-01','VVC-02'))
    )
...
RawView0 as (
		...
		SUM(CASE 
            ...
            ELSE 0
        END) AS DefectQty,
		case when DI.DirectlyUnder in (select TypeErrorCode from STB_TypeErrorGroupOfFactory where Y.TypeErrorCode='PQC') then sum(ISNULL(a.DefectQty, 0)) - sum(ISNULL(a.RepairQty, 0)) /*vanduc update 20260920*/ else 0 end as PQC,
		--case when DI.DirectlyUnder in (select TypeErrorCode from STB_TypeErrorGroupOfFactory where TypeErrorCode='Reliability') then sum(ISNULL(a.DefectQty, 0)) - sum(ISNULL(a.RepairQty, 0)) /*vanduc update 20260920*/ else 0  end as DoTinCay,
		--case when DI.DirectlyUnder in (select TypeErrorCode from STB_TypeErrorGroupOfFactory where TypeErrorCode='Production_Inspection') then sum(ISNULL(a.DefectQty, 0)) - sum(ISNULL(a.RepairQty, 0)) /*vanduc update 20260920*/  else 0 end as SX_Ktra,
		case when DI.DirectlyUnder in (select TypeErrorCode from STB_TypeErrorGroupOfFactory where Y.TypeErrorCode='Production_Defect') then sum(ISNULL(a.DefectQty, 0)) - sum(ISNULL(a.RepairQty, 0)) /*vanduc update 20260920*/  else 0 end as SX_NG,
		case when DI.DirectlyUnder in (select TypeErrorCode from STB_TypeErrorGroupOfFactory where Y.TypeErrorCode='Machine_Repair') then sum(ISNULL(a.DefectQty, 0)) - sum(ISNULL(a.RepairQty, 0)) /*vanduc update 20260920*/ else 0  end as SuaMay,
		case when DI.DirectlyUnder in (select TypeErrorCode from STB_TypeErrorGroupOfFactory where Y.TypeErrorCode='Regular_Production_Checks') then sum(ISNULL(a.DefectQty, 0)) - sum(ISNULL(a.RepairQty, 0)) /*vanduc update 20260920*/  else 0 end as Ktra_ThuongxuyenSX,
		case when DI.DirectlyUnder in (select TypeErrorCode from STB_TypeErrorGroupOfFactory where Y.TypeErrorCode='Fixed_Production_Inspection') then sum(ISNULL(a.DefectQty, 0)) - sum(ISNULL(a.RepairQty, 0)) /*vanduc update 20260920*/ else 0  end as Ktra_CoDinhSX,		
		case when DI.DirectlyUnder in  (select TypeErrorCode FROM STB_TypeErrorGroupOfFactory WHERE Y.TypeErrorCode='NG_Setup_Machine') then sum(ISNULL(a.DefectQty, 0)) - sum(ISNULL(a.RepairQty, 0)) /*vanduc update 20260920*/ else 0 end as SetupMay,
		...
		from  STB_SetInfo c with(nolock) 
		 ...
		 and  b.CreateUserID not in ('test_worker','assy_packing','assy_packing2','roh_worker','electrode_worker','dryroom_worker' )  and (
        -- Trường hợp 1: Nếu ToDate trước 2025-05-01, không cần check CompleteRoute
        @ToDate < CONVERT(VARCHAR(10), DATEADD(DAY, 1, CONVERT(smalldatetime, '2025-05-01')), 120) + ' 10:00:00'  
        OR 
        -- Trường hợp 2: Từ 2025-05-01 trở đi, bắt buộc CompleteRoute = 1
...
DRI as(
		...
		distinct RV.Barcode,RV.RouteCode, RV.RouteCode as FindRouteCode ,ControlNo,RV.MaterialCode,RV.InputLineCode,RV.MachineCode,RV.WorkerCode,RV.SIExtText07,RV.SIExtInt01,
		 ...
		 WHEN
				 RV.Barcode IN (
					...
					'VVQN192R710651',
					'VVQN192R710652'
				) AND RV.MaterialCode = 'ECVT27-344' THEN 'HY-CAP WEC2R7106QG (1030 Low)'
			else MM2.MaterialName end MaterialName,
	...
	select
		distinct  'VVT'   AS 사업장
		  ...
		  ,CASE WHEN DRI.MaterialCode = 'ECVT27-344' THEN 'HY-CAP WEC2R7106QG (1030 Low)'
			else MM2.MaterialName end MaterialName
		  ,DRI.InputLineCode
		  ,LI.LineName
		  ...
		  /*, DRI.ProdQty                                   AS InputProdQty    -- 투입수량 vanduc edit 20260920*/ , (DRI.ProdQty + ISNULL(DRI.DefectQty, 0)) AS InputProdQty
		  ...
		  /*, (DRI.ProdQty - ISNULL(DRI.DefectQty, 0) - ISNULL(PQC, 0)  -ISNULL(SX_NG, 0) - ISNULL(SuaMay, 0) - ISNULL(Ktra_ThuongxuyenSX, 0) - ISNULL(Ktra_CoDinhSX, 0)) -ISNULL(SetupMay,0) AS ProdQty           -- 생산수량 vanduc edit 20260920*/ , DRI.ProdQty AS ProdQty
		...
		)  as PriceINCREMENTAL vanduc edit 20260920*/ ,ISNULL(vwupINCREMENTAL.ProcessUnitPriceEA,vwupINCREMENTAL1.ProcessUnitPriceEA) * DRI.ProdQty as PriceINCREMENTAL
	...
	begin
		;with ViewBarcode as (
		 ...
		 STB_SetInfo c with(nolock) 
		 left outer join  STB_ProdRouteHist b	 with(nolock) on c.ControlNo=b.ControlNo	 
		 -- Thêm điều kiện cho nhà máy Hưng Yên
		 where b.CompanyCode='VVT' AND b.WorkCenterCode in ('VVT_F1','VVT_F2','VVT_F5')  and b.ProdDateTime>@FromDate  and b.ProdDateTime<@ToDate and  b.CompleteRoute=1
		 and  b.CreateUserID not in ('test_worker','assy_packing','assy_packing2','roh_worker','electrode_worker','dryroom_worker' )
	 ),
RawView0 as (
	 	select  b.CreateUserID,c.Barcode,b.RouteCode,b.RouteCode as FindRouteCode,c.ControlNo,c.MaterialCode,InputLineCode,b.MachineCode,b.WorkerCode,SIExtText07,SIExtInt01,
	...
	select
		distinct  'VVT'   AS 사업장
		  ...
		  WHEN DRI.Barcode IN (
					...
					'VVQN192R710651',
					'VVQN192R710652'
				) AND MM2.MaterialCode = 'ECVT27-344' THEN 'HY-CAP WEC2R7106QG (1030 Low)'
			else MM2.MaterialName end MaterialName
		  ,DRI.InputLineCode
		  ,LI.LineName
		  ,DRI.RouteCode as RouteCode
