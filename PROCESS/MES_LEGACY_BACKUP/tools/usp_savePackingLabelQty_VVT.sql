						...
						------------ END
						 ...
						 OPEN SourceData1
							WHILE 1=1 BEGIN
							  ...
							  -- Modified: Mr.Triều
							  -- Desc: Chặn không cho lưu nếu đó là CellLine 1,2
							  DECLARE @IsVavidCellLine VARCHAR(50)
							  SELECT @IsVavidCellLine=InputLineCode from STB_SetInfo where Barcode=@LotNo
							  IF(@IsVavidCellLine ='VVC-01' OR @IsVavidCellLine='VVC-02')
							  BEGIN
							       RAISERROR(N'CellLine 1,2 hiện tại đang thử nghiệm hệ thống POP nên chức năng này bị khoá',16,1)
								   RETURN;
							  END
								...
								*/
								...
								if(@isPrinted=0 and @EmpNo  in ('HaiTrieu','hopnguyen','VVTworker','huyen','32107013','hant-1998','hoaiyen') and @checklotno=0 and @LotNo like 'MV%')
