...
BEGIN
	...
	Declare @count int=0; 
	--DECLARE @RawMaterialBarcode NVARCHAR(200) =  @pRawMaterialBarcode vanduc edited by Mr.DuongPham 20260615 START
	DECLARE @RawMaterialBarcode NVARCHAR(1000) =  @pRawMaterialBarcode
	
	--DECLARE @LotMaterialBarcode NVARCHAR(200) =  @pRawMaterialBarcode	
	DECLARE @LotMaterialBarcode NVARCHAR(1000) =  @pRawMaterialBarcode -- lấy mã nguyên liệu nhập vào không được thay đổi
	
	--DECLARE @BarcodeInsert NVARCHAR(200) =  @pBarcode 
	DECLARE @BarcodeInsert NVARCHAR(1000) =  @pBarcode -- lấy code của sản xuất nhập vào không được thay đổi 
	--END
...
declare @cterminal1			nVARCHAR(300)='',
	...
	select  @cElectrode2= '  <<>>  '+materialcode+'  '+semiProductname  from stb_vvt_materialbo with(nolock)
		...
		if( UPPER(isnull(@pProductGroupCode,'')) in ( 'TERMINALP','TERMINALM','ATLTERMINAL' )  ) begin	
			...
			if( ltrim(@pRawMaterialBarcode) like 'SD-%') 
            set @pRawMaterialBarcode = substring(ltrim(@pRawMaterialBarcode), 4, len(@pRawMaterialBarcode))
        if( charindex('#', @pRawMaterialBarcode) > 0 ) 
            set @pRawMaterialBarcode = substring(@pRawMaterialBarcode, 1, charindex('#', @pRawMaterialBarcode) - 1)
		...
		-- select count(*) from STB_SetInfo where barcode = 'VVPR292R710617' and (MaterialCode like 'LIVT%' or MaterialCode LIKE 'ECVT%')
			...
			union all
			--hela 1840
			select 'GBHB00-004' as Terminal , 'VEC3R0606QG' as model,'1840' as size union all
             select 'GBHB00-004' as Terminal , 'VEC2R7506QG' as model,'1840' as size union all
             select 'GBHB00-004' as Terminal , 'WEC2R7506QG' as model,'1840' as size union all
			 select 'GBHB00-003' as Terminal , 'VEC3R0606QG' as model,'1840' as size union all
             select 'GBHB00-003' as Terminal , 'VEC2R7506QG' as model,'1840' as size union all
             select 'GGBHB00-003' as Terminal , 'WEC2R7506QG' as model,'1840' as size union all
			...
			select 'GBHB00-034' as Terminal , 'VET10252R7106G'		as model,'1025' as size union all
			select 'GBHB00-042' as Terminal , 'VEC3R0606QG' as model,'1840' as size 
			...
			;with eleclyte1 as ( 
			  ...
			  where part like '%electrolyte%' and size is not null and semiProductname not like '%Element%'
						...
						UNION ALL
						  ...
						  select 'GBCP00-005' AS electrolyte, 'VEL10403R8157G'  as model, '1040' size  UNION ALL -- 2026-09-07
						  select 'GBCP00-005' AS electrolyte, 'VEL13353R8257G'  as model, '1335' size  UNION ALL -- 2026-09-07
	   ...
	   return;
		...
		if( UPPER(isnull(@pProductGroupCode,'')) in ( 'ELECTRODEP','ELECTRODEM' )  ) begin
				...
				DECLARE @LotCount int
                    ...
                    if(@MaterialCodeNotLowESR in('ECVT30-367')) --'ECVT30-372'
					BEGIN
					...
					*/
					...
					END
				/*
				 IF (@MaterialCodeNotLowESR in('ECVT30-367'))
					BEGIn
						-- Lấy ra mã NVL đầu vào nhập mã điện cực
						--DECLARE @MaterialCodeBOM VARCHAR(30)
						SELECT TOP 1 @MaterialCodeBOM = MaterialCode 
		             ...
		             IF NOT EXISTS(SELECT 1 FROM STB_BomDetail WHERE MaterialCode=@MaterialCodeNotLowESR AND BomVersion='2001' AND ChildMaterialCode=@MaterialCodeBOM)
	                 BEGIN
		                SET @err = N'Lỗi BOM ' + ISNULL(@MaterialCodeBOM, N'Trống') + 
				          N' không được phép dùng cho Model ' + @MaterialCodeNotLowESR + 
				            N'. Vui lòng kiểm tra lại!';
		               RAISERROR (@err, 16, 1);
		               RETURN;
                ...
                */
				...
				--select * from STB_SetInfo where Barcode='VVQO273R010617'
		  ...
		  --      if (isnull((select  dateadd(day,(ISNULL(MMExtInt01,3) * 30)+ISNULL(MMExtInt01,3)/12*6,@ValidateTime)  from STB_MaterialMaster where MaterialCode= @MaterialUnit),getdate()-1) < getdate())
			...
			/*	if(@locationWarehouseCode='VVT_F2')
				...
				select @count=count(*) from STB_SetInfo  si			with(nolock) 
					...
					if(@IsNextModel in('ECVT30-309','ECVT30-380'))
					BEGIN
					    set @err='';
	                       set @count=1;
				...
				IF (
					...
					OR ( @IsNextModel IN( 'ECVT30-357','ECVT30-372','ECVT30-380') AND substring(@pRawMaterialBarcode,1,14) IN (SELECT DISTINCT ElectrodeLotNumber
			...
			if(@materialCodeCheck in ('ECVT27-400') AND @rawMaterialCheck in ('CRFYN85','CREBO85'))
				BEGIN
				    SET @count=1;
				END
			if(@materialCodeCheck in ('ECVT30-380') AND @rawMaterialCheck in ('CREBL85L','CRFYL85'))
				BEGIN
				    SET @count=1;
				END
				-- end update
...
if( UPPER(isnull(@pProductGroupCode,'')) in ( 'Separator' )  or lower(@pProductGroupCode) like '%separato%'  ) begin	
	...
	-- MR.Manh update 2026-04-25 following IQC request for 0820 Model
		...
		select 'GBNKSP-086' as separatocode,'WEC3R0105QG (0813)' as models union all
		...
		;with casetable1 as ( 				
			  ...
			  where part like '%case%' and size is not null and semiProductname not like '%Element%'
						    ...
						    select 'GBAKAC-048' as casecode , 'VEB2R8126HC' as models,'1030' as size  union all
							  ...
							  select 'GBDYAC-004' as casecode, 'VEP3R0367QG' as models, '3562' as size  union all
							  -- VET18402R7506G (1840)
							--VEL10303R8107G (1030)
...
-------------------------------------------------------------------------------------------------------------------------------------------------------
   ...
   -- Mr.Duy thiết lập điều kiện cho nhà máy hà nam
	...
	BEGIN
		...
		BEGIN
			IF ISNULL(@mmmaterialcode, '') = ''
			BEGIN
				SET @err = N'Mã NVL không hợp lệ của LotNo: ' + @pBarcode + N' với mã nhập: ' + ISNULL(@LotMaterialBarcode, '')
				RAISERROR(@err, 16, 1)
				RETURN
			END

			IF @pProductGroupCode = 'ModuleWire' AND @mmmaterialcode <> 'WRHI00-007'
			BEGIN
				SET @err = N'Mã Wire không đúng của LotNo: ' + @pBarcode + N' với mã nguyên vật liệu: ' + @mmmaterialcode + N'. Cần: WRHI00-007'
				RAISERROR(@err, 16, 1)
				RETURN
			END

			IF @pProductGroupCode = 'ModuleChip' AND @mmmaterialcode <> 'VRE-009'
			BEGIN
				SET @err = N'Mã Chip không đúng của LotNo: ' + @pBarcode + N' với mã nguyên vật liệu: ' + @mmmaterialcode + N'. Cần: VRE-009'
				RAISERROR(@err, 16, 1)
				RETURN
			END
...
END		-- END BG@
		...
		--DECLARE @existingRawBarcode NVARCHAR(200) = '' vanduc edited by Mr.DuongPham 20260615 START
