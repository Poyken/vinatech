...
-- exec [usp_Vietnam_GetBoxIDForLotNo_VVT] '','','VVQK113R825702','', ''
...
BEGIN
	...
	-- [FIX-20260618] Fallback: Nếu không tìm thấy record VV, thử tìm biến thể VJ (fix B353 VJ/VV prefix mismatch)
	-- Guard: CHỈ chạy khi chưa tìm thấy record VV VÀ @LotNo bắt đầu 'VV' (tránh collision với lot có cả VV+VJ record)
	...
	SELECT
			...
			(case 
				...
				*/
				...
				--ducnv edited by Mrs.Tran Lan 20260605 
				--start
				When @LotNo='VJPL103R025601' then 'VVQN103R025601'
				--end

			   when (@LotNoFirst = @oldLotid or @oldLotid =@Lotno or (@newLotid IS NOT NULL AND @oldLotid = STUFF(@LotNo,1,2,'VJ'))) then @newLotid --tùy chọn chuyển đổi lotno [FIX-20260618]
