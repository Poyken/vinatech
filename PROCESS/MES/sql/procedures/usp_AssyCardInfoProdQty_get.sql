...
-- [프로시저실행문]  	usp_AssyCardInfoProdQty_get  '','','VVOP113R015601', ''
-- [프로시저실행문]  	usp_AssyCardInfoProdQty_get  '','','K16418106262300427', ''
-- ==============================================================================
CREATE PROCEDURE [dbo].[usp_AssyCardInfoProdQty_get]
	...
	-- 실적등록정보
	 SELECT PRH.MaterialCode
	   ...
	   FROM STB_ProdRouteHist PRH
          ...
          LEFT OUTER JOIN STB_PassOrFailRouteStatus PFRS ON SI.Barcode = PFRS.Barcode 
                                              AND PRH.RouteCode = PFRS.RouteCode
     where SI.Barcode=@Barcode and PFRS.Status='Pass'
	 /*
...
END
	...
	--								     FROM STB_SetInfo SI
	--								    WHERE Barcode = 'K16418106262300427'
	--								   )
