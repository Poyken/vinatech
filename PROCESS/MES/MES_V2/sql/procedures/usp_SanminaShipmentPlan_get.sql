IF NOT EXISTS (SELECT 1 FROM sys.objects WHERE object_id = OBJECT_ID(N'[dbo].[usp_SanminaShipmentPlan_get]') AND type in (N'P', N'PC'))
    EXEC dbo.sp_executesql @statement = N'CREATE PROCEDURE [dbo].[usp_SanminaShipmentPlan_get] AS SELECT 1 AS Stub;'
GO

ALTER PROCEDURE [dbo].[usp_SanminaShipmentPlan_get]
