...
BEGIN
	...
	-- =========================================================================
	-- 1. BẢNG TẠM SẢN LƯỢNG KHO BẮC NINH (BN) - NGUỒN FG01 BẮC NINH
	-- =========================================================================
	SELECT 
		a.PublicCode, 
		a.PartNo, 
	...
	SELECT b.*, COALESCE(g.QtyOver1Y, 0) AS QtyOver1Y
	INTO #BN
	FROM #BN_Base b
	...
	-- =========================================================================
	-- 2. BẢNG TẠM SẢN LƯỢNG KHO HƯNG YÊN (HY) - NGUỒN FG01 HƯNG YÊN (THAY THẾ BG)
	-- =========================================================================
	SELECT 
		a.PublicCode, 
		a.PartNo, 
	...
	-- =========================================================================
	-- 3. KẾT HỢP DỮ LIỆU TỔNG HỢP (BẮC NINH + HƯNG YÊN) VÀ ĐƠN GIÁ B943
	-- =========================================================================
	SELECT 
		Master.PublicCode, 
		Master.PartNo, 
		...
		-- TỒN CUỐI KỲ TỔNG CỘNG (BN + HY)
		(COALESCE(bn.TonCuoiKy, 0) + COALESCE(hy.TonCuoiKy, 0)) AS TonCuoiKy,

		-- TỒN TRÊN 1 NĂM TỔNG CỘNG
		(COALESCE(bn.QtyOver1Y, 0) + COALESCE(hy.QtyOver1Y, 0)) AS TonTren1Nam,

		-- ĐƠN GIÁ B943
		COALESCE(r.UnitPrice, r_kr.UnitPrice, 0) AS DonGia,

		-- GIÁ TIỀN TỒN CUỐI KỲ (TỒN CUỐI KỲ * ĐƠN GIÁ)
		CAST((COALESCE(bn.TonCuoiKy, 0) + COALESCE(hy.TonCuoiKy, 0)) * COALESCE(r.UnitPrice, r_kr.UnitPrice, 0) AS DECIMAL(18, 2)) AS GiaTienTonCuoiKy,

		-- GIÁ TIỀN TỒN TRÊN 1 NĂM (TỒN > 1 NĂM * ĐƠN GIÁ)
		CAST((COALESCE(bn.QtyOver1Y, 0) + COALESCE(hy.QtyOver1Y, 0)) * COALESCE(r.UnitPrice, r_kr.UnitPrice, 0) AS DECIMAL(18, 2)) AS GiaTienTon1Nam
	...
	WHERE (
		...
		(COALESCE(bn.XuatSanXuat, 0) + COALESCE(hy.XuatSanXuat, 0)) > 0 OR 
