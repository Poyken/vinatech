...
SET NOCOUNT ON;
	...
	--select * from STB_VN_FINISHGOODS_HY
	  --STB_VN_FINISHGOODS_HY
	 
		 EXEC SmartFramework.dbo.usp_GetSerialRule 
			@pTableName = 'STB_VN_FINISHGOODS_HY_NEW',
  ...
  IF @IsAutoKey = 0 AND @IsLoopIUD = 0 BEGIN
   ...
   BEGIN TRY
   	-- Process Insert Table
			 MERGE STB_VN_FINISHGOODS_HY_NEW AS TargetTable
			USING
				(
					SELECT
							CASE
...
SELECT
	   ...
	   WHILE 1 = 1 BEGIN
		  ...
		  IF @IUD_FLAG = 'INSERT' BEGIN
                    ...
                    IF EXISTS (SELECT 1 FROM STB_VN_FINISHGOODS_HY_NEW WHERE ID = @OldCompanyCode) BEGIN
						RAISERROR('Duplicate Data : KeyField = %s', 16, 1, @OldCompanyCode)
					END
			 IF @IsAutoKey = 0 BEGIN
						EXEC SmartFramework.dbo.usp_DoCreateSerial 'STB_VN_FINISHGOODS_HY_NEW', @OldCompanyCode OUTPUT


						INSERT INTO STB_VN_FINISHGOODS_HY_NEW
						(
							SoPhieuNhapKho,
				            SoPhieuXuatKho,
							SoInVoice,
...
UPDATE STB_VN_FINISHGOODS_HY_NEW
						SET
			...
			Descrption =   CASE
					...
					WHERE
                        ...
                        DELETE FROM STB_VN_FINISHGOODS_HY_NEW
						WHERE
						    ID = ''
                    END
                END
