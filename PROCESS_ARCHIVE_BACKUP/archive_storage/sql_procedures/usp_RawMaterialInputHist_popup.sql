...
-- Modified :
-- 실행 :exec usp_RawMaterialInputHist_get '','','vvpr013r015601'
		...
		Begin
			SELECT RMIH.RawMaterialInputHistNo as RawMaterialInputHistNo
					 ...
					 , RMBI.ProductGroupName_VVT                  -- 2020.02.14 추가 (베트남법인)
					  ...
					  ,ISNULL(MDD.RevisionsVer, MR.Revision) AS Revision
					  ,MR.BomItemRev
					  ,MM.MaterialSpecL
					  ,RIGHT('00' + MM.MaterialName, 6) AS MaterialName
			  ...
			  FROM STB_RawMaterialInputHist RMIH with(nolock) 
						...
						LEFT OUTER JOIN 
						(
							SELECT MaterialCode, MAX(Revision) AS Revision, MAX(BomItemRev) AS BomItemRev
							FROM STB_MaterialRevision  WITH(NOLOCK)
							GROUP BY MaterialCode
						) AS MR
						  ON MR.MaterialCode = MDD.MaterialCode
						LEFT OUTER JOIN STB_MaterialMaster MM WITH(NOLOCK)
						  ON MM.MaterialCode = MDD.MaterialCode
						LEFT OUTER JOIN STB_DelegateMaterialInputLog DLOG WITH(NOLOCK) 
							ON DLOG.ProductionLotNo = RMIH.Barcode AND DLOG.DelegateMaterialLotNo = RMIH.RawMaterialBarcode
