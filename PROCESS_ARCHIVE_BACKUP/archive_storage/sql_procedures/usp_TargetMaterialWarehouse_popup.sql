...
-- ============================================= 
CREATE PROCEDURE [dbo].[usp_TargetMaterialWarehouse_popup] -- exec usp_TargetMaterialWarehouse_popup 'VVT', 'VVT_F5'
	    ...
	    SELECT
			...
			UNION ALL
				...
				FROM
			        STB_MaterialWarehouse MW WITH(NOLOCK)
				WHERE
					MW.WorkCenterCode = 'VVT_F5' AND MW.MaterialWarehouseCode='ROH_HY_WH'
					AND ISNULL(MW.IsUsed, 1) = 1
		...
		BEGIN
           ...
           UNION ALL
				select 
					...
					'ROH_VN_WH' as MaterialWarehouseCode,
					'VVT',
					@WorkCenterCode,
					--Case 
					--	when @WorkCenterCode='VVT_F1'
					...
					--	else 'NO_CONFIG'
					--end
					'ROH_모듈(베트남)' as MaterialWarehouseName,
					'',
					'',
					'',
					'',
