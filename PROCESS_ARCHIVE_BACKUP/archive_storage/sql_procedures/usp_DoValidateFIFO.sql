...
begin try
	...
	WHERE
			...
			(
				...
				or ( FORMAT(convert(date,MLI.Lotattr10,120), 'yyyy-MM')  <  ( FORMAT(convert(date,@Lotattr10,120), 'yyyy-MM')) and  @CompanyCode='VVT'  --2025-11-20 Mr Mạnh chuyển FIFO theo ngày
...
begin catch
		...
		set @CreateDateTime =  FORMAT(convert(datetime,@pCreateDateTime,103), 'yyyy-MM')
	...
	--Lấy thông tin để hiển thị ra ngoài biết lot nào đang cần xuất trước
		...
		IF EXISTS	(
							...
							WHERE 1=1
									...
									or ( FORMAT(convert(date,MLI.Lotattr10,120), 'yyyy-MM')  <  ( FORMAT(convert(date,@Lotattr10,120), 'yyyy-MM'))  --2025-11-20 Mr Mạnh chuyển FIFO theo ngày
									and Year(convert(date,MLI.Lotattr10,120)) <= Year(convert(date,@Lotattr10,120))) --2025-11-20 Mr Mạnh chuyển FIFO theo ngày
										 )
