...
-- Description:	
-- =============================================
CREATE PROCEDURE [dbo].[usp_Vietnam_ElectrodeDefectHist_get]
...
lasttable as (
	...
	ab.ElectrodeLotNumber as Barcode,
	'V-01' as RouteCode,
	N'믹싱 (Trộn)' as RouteName,
	null as DefectCode ,
	NGFilter as "Defect(KG)",
	null UnitMet2Kg,
    wmeter * NGFilter as "Defect(Meter)",
	si.MaterialCode,
	mm2.MaterialName,
	-- cast ((isnull(infoWeight.upricekg,0) * ab.DefectWeight ) as numeric(10,5)) as "WastePrice(USD)",
	--Công thức mới chị Mrs.Sao 2024-01-04
	emw.UnitPriceLossUSD as "WastePrice(USD)",
	emw.UnitPriceKgUSD as PriceMeter,
	emw.UnitPriceKgUSD as PriceKg,
	isnull(ab.CreateUserID,isnull(ab.ChangeUserID,'')) as CreateUserID
	, ab.MachineCode
	, MM.MachineName,
	ebb.Factory as Factory --update here
	--ab.*
	from STB_ElectrodeMixInfo ab  with(nolock) 
	  LEFT OUTER JOIN STB_ElectrodeCoatingInfo ECI   with(nolock) on ab.ElectrodeLotNumber = ECI.ElectrodeLotNumber
	  	  LEFT OUTER JOIN STB_MachineMaster MM  with(nolock) ON MM.MachineCode = ab.MachineCode
		LEFT OUTER JOIN STB_ElectrodeRollPressingInfo ERPI   with(nolock) on ab.ElectrodeLotNumber = ERPI.ElectrodeLotNumber
		  --LEFT OUTER JOIN STB_ElectrodeSlittingResult ERPI1 on ab.Barcode = ERPI1.ElectrodeLotNumber
		  outer apply( select top 1 * from STB_ElectrodeSlittingResult   with(nolock) where ElectrodeLotNumber = ERPI.ElectrodeLotNumber  ) b --on ab.Barcode = b.ElectrodeLotNumber
	--left outer join STB_DefectInfo b on replace(a.DefectCode,'E-','V-') = b.DefectCode
		   left outer join stb_setinfo si  with(nolock) on ab.ElectrodeLotNumber = si.Barcode
	   left outer join Electrode_BN_BG ebb with(nolock) on si.MaterialCode = ebb.MaterialCode  --update here
	   left outer join STB_MaterialMaster mm2 with(nolock) on si.MaterialCode = mm2.MaterialCode
	   LEFT OUTER JOIN STB_VVT_ElectrodeMixingWaste emw with(nolock) on emw.MaterialCode = si.MaterialCode
	  outer apply(
    select top 1 (meter/weight1*1.0) as wmeter from [dbo].[fn_VVT_ElecMixingKg2Met]() emeterMixing
    where 
    (mm2.MaterialName like ('%'+emeterMixing.type1 + '%'+emeterMixing.construct + '%'+ emeterMixing.thick+ '%'+emeterMixing.batch+ '%'+emeterMixing.anodecathode+'%')
    or mm2.MaterialName like ('%'+emeterMixing.type1 + '%'+ emeterMixing.thick+'%'+emeterMixing.construct +  '%'+emeterMixing.batch+ '%'+emeterMixing.anodecathode+'%')
    or mm2.MaterialName like ('%'+emeterMixing.type1 + '%'+emeterMixing.construct + '%'+ emeterMixing.thick+ '%'+emeterMixing.anodecathode+'%') and (	   
        select count(*) from [dbo].[fn_VVT_ElecMixingKg2Met]() emeterMixing
        where mm2.MaterialName like ('%'+emeterMixing.type1 + '%'+emeterMixing.construct + '%'+ emeterMixing.thick+ '%'+emeterMixing.batch+ '%'+emeterMixing.anodecathode+'%')
           or mm2.MaterialName like ('%'+emeterMixing.type1 + '%'+ emeterMixing.thick+'%'+emeterMixing.construct +  '%'+emeterMixing.batch+ '%'+emeterMixing.anodecathode+'%')
        )=0
    )
    ORDER BY LEN(emeterMixing.thick) DESC
) meterMixing
	 where  ab.CreateDateTime between @FromDate and  @ToDate and ab.ProductionQty > 0 and si.MaterialCode IN (Select MaterialCode from STB_VVT_ElectrodeMixingWaste)
		--union 
...
select lasttable.*, 
		 ...
		 when lasttable.RouteCode = 'V-01' THEN N'Phế slurry công đoạn lọc'
