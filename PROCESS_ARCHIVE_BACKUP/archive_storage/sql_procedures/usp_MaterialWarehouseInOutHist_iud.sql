...
BEGIN
	...
	if(@WarehouseInOutCode='O' and @SourceMaterialWarehouseCode in ('ROUTE_BG_WH','ROUTE_VN_WH','ROUTE_HN_WH', 'ROUTE_BG2_WH','ROUTE_HY_WH'))
	begin
	...
	-- 원본 백업
	  ...
	  -- Modified: Mr.Triều (2027-07-28) Đây là danh sách các kho có thể xuất NVl nếu bên sản xuất yêu cầu thêm kho xuất là HOLDING thì thêm vào đây là xong
		IF @SourceMaterialWarehouseCode IN ('ROH_WH', 'ROH_VN_WH','ROH_BG_WH','ROH_HN_WH','HOLDING_VN_WH', 'MODULE_BG2_WH','ROH_HY_WH','HOLDING_BG_WH')                     -- 원자재 출고처리의 경우	
		BEGIN 			 
		 --- Mr.Triều add condition materialcode turn on FIFO with factory Bắc Ninh
			if (
			 
			@pTargetMaterialWarehouseCode NOT IN ('ROH_BG_WH')  and @WorkCenterCode not in ( 'VVT_F3','VVT_F1','VVT_F2')
   ...
   -- 2. 해당 Lot의 유효일이 가장 빠르지 않으면 에러
		...
		begin
			...
			BEGIN
				 ...
				 --RAISERROR(' ^유효일자가 지난 자재입니다. 확인 바랍니다.^ ' ,16, 1)  유효기간이 지난 자재입니다.
   ...
   --2020.04.27 추가사항 (활성탄 두제품의 경우에는 바코드를 찍으면 일괄처리되도록 - 구보겸요청) ---------------------------------------------------------------------------------------------	  
		...
		IF (@MaterialCode = 'GATCCC-001' Or  @ProductGroupCode  ='A.C') AND @CompanyCode = 'VNT'   BEGIN		-- Add @CompanyCode
			...
			BEGIN
				...
				IF(@TargetMaterialWarehouseCode <> 'ROH_BG_WH' AND @TargetMaterialWarehouseCode <> 'ROH_VN_WH'AND @TargetMaterialWarehouseCode <> 'ROH_HN_WH' AND @TargetMaterialWarehouseCode <> 'MODULE_BG2_WH' and @TargetMaterialWarehouseCode <> 'ROH_HY_WH')--Duy check thuộc kho BG thì sẽ chưa cho update ngay
					BEGIN
	     ...
	     IF   @SourceMaterialWarehouseCode IN ('ROH_WH', 'ROH_VN_WH', 'ROH_BG_WH', 'ROH_HN_WH', 'MODULE_BG2_WH', 'ROH_HY_WH')       -- 원자재창고이면
	   ...
	   ------------- 여기까지
			...
			BEGIN
				IF(@TargetMaterialWarehouseCode <> 'ROH_BG_WH' AND @TargetMaterialWarehouseCode <> 'ROH_VN_WH' AND @TargetMaterialWarehouseCode <> 'ROH_HN_WH' AND @TargetMaterialWarehouseCode <> 'MODULE_BG2_WH' AND @TargetMaterialWarehouseCode <> 'ROH_HY_WH') --Duy check thuộc kho BG thì sẽ update cả nahf máy mới hiển thị
					BEGIN
	...
	-- ProcessedLotID 업데이트 이 부분이 정상출고로 업데이트 되는 부분인데, 수정  (2020.04.16)
	   IF   @SourceMaterialWarehouseCode IN ('ROH_WH', 'ROH_VN_WH','ROH_BG_WH','ROH_HN_WH', 'MODULE_BG2_WH','ROH_HY_WH')       
