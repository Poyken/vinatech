...
-- Modified :
-- 실행 :exec usp_RawMaterialInputHist_get '','','K10009911262500105'
-- 2021-06-14 : Mr.Tung  adding LotID columns
-- =============================================
CREATE PROCEDURE [dbo].[usp_RawMaterialInputHist_get]
...
BEGIN
	...
	BEGIN
		IF @WorkCenterCode = 'VVT_F4' AND @MaterialCode IN ('EDVTSY-001', 'EDVTMD-246-002', 'EDVTMD-246', 'EDVTMD-248', 'BEPCBA-001') BEGIN -- SL-7의 경우 BOM을 한단계 더 조회해야하므로 분리하여 처리
			exec usp_DoMakeRawMaterialInputHistBE @Barcode, @MaterialCode
		END ELSE BEGIN
		...
		Begin
			SELECT RMIH.RawMaterialInputHistNo as RawMaterialInputHistNo
					 ...
					 , RMBI.ProductGroupName_VVT                  -- 2020.02.14 추가 (베트남법인)
					  ...
					  MDII.SourceCustomerCode AS DistributorCode,
					  CI2.CustomerName AS DistributorName,
					  MDD.Weekcode,	
					  MDD.RevisionsVer
					  ,tmp.RawBacodeList
					  ,'' AS MaterialName
			  FROM STB_RawMaterialInputHist RMIH with(nolock) 
						...
						LEFT OUTER JOIN STB_MaterialDocInfo MDII ON MDII.MaterialDocNo = MDD.MaterialDocNo
						LEFT OUTER JOIN STB_CustomerInfo CI2 WITH(NOLOCK) ON CI.CustomerCode = MDII.SourceCustomerCode
	...
	END ELSE IF @WorkCenterCode = 'VVT_F4' BEGIN
		SELECT  
			RMIH.RawMaterialInputHistNo as RawMaterialInputHistNo
					 , RMBI.SizeCode
					 , @Barcode AS Barcode
					 , CASE WHEN RMIH.ProductGroupCode = 'PWB164180' THEN 'PWB173670' ELSE RMIH.ProductGroupCode END AS ProductGroupCode
					...
					-- ,RMBI.ProductGroupName
					 ,RMIH.MaterialCode,
					  MDI.LevelJIANGHAI, -- Mr.Trieu add Level JIANGHAI
					  CASE WHEN MDD.ManufacturerCode IS NOT NULL 
					       THEN (SELECT TOP 1 CustomerName FROM STB_CustomerInfo WHERE CustomerCode = MDD.ManufacturerCode)
						   ELSE RMEI.ManufacturerName 
						   END AS ManufacturerName,
					  CASE 
					       --WHEN RMIH.WeekCode IS NOT NULL
					       --THEN RMIH.WeekCode
						   WHEN RMIH.RawMaterialBarcode LIKE 'P1641%'
									OR RMIH.RawMaterialBarcode LIKE 'K164181%'
									OR RMIH.RawMaterialBarcode LIKE 'K100099%'
									OR RMIH.RawMaterialBarcode LIKE '%100098%'
									OR RMIH.RawMaterialBarcode LIKE '%100089%'
									OR RMIH.RawMaterialBarcode LIKE '%164869%'
									OR RMIH.RawMaterialBarcode LIKE '%030187%'
									OR RMIH.RawMaterialBarcode LIKE '%164700%'
									OR RMIH.RawMaterialBarcode LIKE '%164800%'
									OR RMIH.RawMaterialBarcode LIKE '%157475%'
									OR RMIH.RawMaterialBarcode LIKE 'P173670%'
						   THEN RIGHT(LEFT(RMIH.RawMaterialBarcode, 13), 4)
						   WHEN RMIH.RawMaterialBarcode LIKE '%711720%'
						   THEN RIGHT(LEFT(RMIH.RawMaterialBarcode, 20), 2) + RIGHT(LEFT(RMIH.RawMaterialBarcode, 18), 2)
						   ELSE 
							  RIGHT(LEFT(MDI.LotAttr10, 4), 2) 
								 + RIGHT('0' + CONVERT(VARCHAR(10), DATEPART(ISO_WEEK, CONVERT(DATE, MDI.LotAttr10, 121))), 2) 
						   END
						AS WeekCode,
					  CASE WHEN RMIH.RawMaterialBarcode LIKE 'P1641%' 
									OR RMIH.RawMaterialBarcode LIKE 'K164181%'
									OR RMIH.RawMaterialBarcode LIKE 'K100099%'
									OR RMIH.RawMaterialBarcode LIKE '%100098%'
									OR RMIH.RawMaterialBarcode LIKE '%100089%'
									...
									OR RMIH.RawMaterialBarcode LIKE '%157475%'
									OR RMIH.RawMaterialBarcode LIKE 'P173670%'
					       THEN RIGHT(LEFT(RMIH.RawMaterialBarcode, 9), 2) 
						   WHEN RMIH.RawMaterialBarcode LIKE '%711720%'
						   THEN RIGHT(RMIH.RawMaterialBarcode, 2) 
						   ELSE MDD.RevisionsVer END AS RevisionsVer
					  ,tmp.RawBacodeList
					  ...
					  MDD.ManufacturerCode, -- for MDD
					  RMIH.Grade,
					  ISNULL(MDD.ManufacturerPartNumber, RMEI.ManufacturerPartNumber) AS ManufacturerPartNumber,
					  RMEI.RouteCode,
					  MDII.SourceCustomerCode AS DistributorCode,
					  CASE WHEN MDII.SourceCustomerCode IS NULL 
					       THEN RMEI.DistributorName 
						   ELSE (SELECT CustomerName FROM STB_CustomerInfo WHERE CustomerCode = MDII.SourceCustomerCode) 
						   END AS DistributorName,
					 RMBI.DisplayIndex
			  ...
			  FROM		
						...
						LEFT OUTER JOIN STB_RawMaterialBaiscInfo  RMBI	 with(nolock)     ON RMIH.ProductGroupCode = RMBI.ProductGroupCode	   AND RMBI.SizeCode = @SizeCode
						left outer join STB_ElectrodeSlittingResult esr  with(nolock) on (esr.Barcode=RMIH.RawMaterialBarcode or esr.ElectrodeLotNumber=RMIH.RawMaterialBarcode)
							left outer join STB_MaterialDocLotInfo MDI with(nolock) on MDI.LotID=dbo.fnGetOriginalLotID(RMIH.RawMaterialBarcode)
					    left outer join STB_MaterialDocDetail MDD WITH(NOLOCK) ON MDD.MaterialDocDetailNo=MDI.MaterialDocDetailNo
						--left outer join STB_MaterialQcInfo MQI WITH(NOLOCK) ON MQI.MaterialQcNo=MDD.MaterialIqcNo
						LEFT outer join STB_CustomerInfo CI WITH(NOLOCK) ON CI.CustomerCode = MDD.ManufacturerCode
						...
						LEFT OUTER JOIN (
							SELECT BloomEnergyPartNumber
							  ...
							  FROM STB_RawMaterialExtendInfoBE 
							 GROUP BY BloomEnergyPartNumber
                                  ,ManufacturerName
                                  ,DistributorName
                                  ,yyww
                                  ,Revision
						  ...
						  ON RMEI.MaterialCode = RMIH.MaterialCode
						 AND RMEI.RouteCode = RMIH.RouteCode
			 WHERE RMIH.Barcode =@Barcode 
