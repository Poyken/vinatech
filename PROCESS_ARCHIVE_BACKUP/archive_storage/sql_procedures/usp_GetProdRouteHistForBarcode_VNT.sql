...
BEGIN
	...
	declare @BlockUserWorkCenterCode varchar(10)=''
	declare @userid varchar(20)=''
	
	select @BlockUserWorkCenterCode=WorkCenterCode,@userid=UserID from STB_UserInfo
	where UserID=@pProcessUserID
	IF(@BlockUserWorkCenterCode IN ('VVT_F5','VVT_F1','VVT_F2') and @userid not in ('HaiTrieu','nguyentha','DinhManh','vvtworker1','hoaiyen'))
    BEGIN
	   	EXEC usp_RaiseLocalizedError @pProcessLanguage, N'Vui lòng sử dụng hệ thống POP để nhập sản lượng'
		RETURN
	END
   ...
   -- Modified: Mr.Triều thêm điều kiện chặn cho nhà máy Hưng Yên, bắt buộc phải chọn tên máy ngoại trừ công đoạn ngoại quan và đóng gói
   -- Date Modified: 2026-08-24
  IF(@WorkCenterCode in ('VVT_F1','VVT_F2','VVT_F5') and @MachineCode='' and @PoType not in('MODULE') and @RouteCode not in('V-28','V-27', 'V-33', 'V-27_BG','V-28_BG','V-33_BG','V-27_HY', 'V-28_HY'))
		BEGIN
	...
	IF @WorkCenterCode='VVT_F4' BEGIN
	   EXEC usp_BlockOPCheckPreviousRouteStatus @pProcessLanguage, @pProcessUserID,@pBarcode=@Barcode,@pRouteCode=@RouteCode
	END
	IF @WorkCenterCode='VVT_F4' BEGIN
	  EXEC usp_BlockOPSelectRouteNotProductionWithMaterial @pProcessLanguage, @pProcessUserID,@pBarcode=@Barcode,@pRouteCode=@RouteCode
	END

	
	IF @WorkCenterCode='VVT_F4' BEGIN
		exec usp_DoCheckPreviousLotProcessed @pProcessLanguage, @pProcessUserID, @DayPlanNo, @Barcode

		-- Adding Dring time check logic
		IF @RouteCode = 'VP04' BEGIN
			SELECT @PrevProdDateTime = ProdDateTime
				FROM STB_ProdRouteHist
				WHERE ControlNo = @ControlNo
				AND RouteCode = 'VP03'

			IF DATEADD(minute, 120, @PrevProdDateTime) > GETDATE() BEGIN
				EXEC usp_RaiseLocalizedError @pProcessLanguage, 'RTV의 경화 시간이 충분하지 않습니다.'
				RETURN
			END
		END ELSE IF @RouteCode = 'VP06' BEGIN
			SELECT @PrevProdDateTime = ProdDateTime
				FROM STB_ProdRouteHist
				WHERE ControlNo = @ControlNo
				AND RouteCode = 'VP05'

			IF DATEADD(minute, 180, @PrevProdDateTime) > GETDATE() BEGIN
				EXEC usp_RaiseLocalizedError @pProcessLanguage, 'Conformal Coating의 경화 시간이 충분하지 않습니다.'
				RETURN
			END
		END
	END
