...
-- Modified: 제품불량정보
-- 프로시저 실행 :    Exec [usp_GetProdRouteBarcodeForDefect_VNT] '','','VP06','','','VP06',''
-- =============================================
...
BEGIN
	...
	FROM
			...
			LEFT JOIN STB_PassOrFailRouteStatus RS WITH(NOLOCK) 
				ON RS.Barcode = SI.Barcode 
				AND RS.RouteCode = @RouteCode
	WHERE
	...
	UNION ALL
	 SELECT
			NULL AS DefectSummaryNo,
			SI.ControlNo,
			SI.PONo,
			SI.DayPlanNo,
			SI.Barcode,
			SI.MaterialCode,
			NULL AS DefectGroupCode,
			NULL AS BasicDefectGroupName,
			NULL AS DefectCode,
			NULL AS BasicDefectName,
			NULL AS DefectQty,
			@RouteCode AS RouteCode,
			@LineCode AS LineCode,
			NULL AS DRIExtText02,
			NULL AS MarKingCode,
			NULL AS DefectCauseID,
			NULL AS DefectCause,
			RS.Status AS PassOrFailStatus 
	FROM
			STB_SetInfo SI WITH(NOLOCK)
			LEFT JOIN STB_PassOrFailRouteStatus RS WITH(NOLOCK) ON RS.Barcode = SI.Barcode AND RS.RouteCode = @RouteCode
	WHERE
			(SI.Barcode = @Barcode OR SI.Barcode = @NewBarcode)
			AND NOT EXISTS (
				SELECT 1 
				FROM STB_DefectRepairInfo DRI WITH(NOLOCK)
				WHERE DRI.ControlNo = SI.ControlNo 
