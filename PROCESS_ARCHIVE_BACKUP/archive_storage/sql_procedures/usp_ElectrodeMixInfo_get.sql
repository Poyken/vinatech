...
BEGIN
	...
	Declare @CompanyCodeDayPlan VARCHAR(10)

	SELECT @CompanyCodeDayPlan = CompanyCode
	  FROM STB_DayProdPlan
	 WHERE DayPlanNo = (
			SELECT TOP 1 DayPlanNo
			  FROM STB_SetInfo
			 WHERE Barcode = @ElectrodeLotNumber
	)

	IF @company <> @CompanyCodeDayPlan BEGIN
		EXEC usp_RaiseLocalizedError @pProcessLanguage, '전극 Lot번호가 발행된 사업장과 작업자의 사업장이 다릅니다.'
		RETURN
	END
