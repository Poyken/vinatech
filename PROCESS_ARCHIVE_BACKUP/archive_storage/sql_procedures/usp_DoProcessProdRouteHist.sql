...
BEGIN
	...
	IF ISNULL(@RouteIndex,-1) >= 0 BEGIN	-- 실적처리 공정
			...
			IF @IsCheckBefRouteProdQty = 1 AND @IsInputRoute <> 1 BEGIN
					...
					IF @RouteCode NOT IN ('E-28', 'V-28', 'V-28_BG','VE10', 'E-33', 'E-34', 'E-29', 'EM-03', 'M-06','V-28_HY') 
			...
			IF ISNULL(@ProdRouteHistNo,'') = '' BEGIN		-- 재처리가 아니면 Insert
					...
					IF @IsInputRoute = 1 BEGIN		-- 투입공정이면
							...
							END
					END
					-- Khi gộp box xong tức là chốt số lương hoàn thành thì số lượng này sẽ tự động cộng vào số lượng hoàn thành của PO gắn với kế hoạch ngày sản xuất
