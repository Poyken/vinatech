...
LEFT JOIN STB_ItemCode T10 
    ...
    tung22 AS (
        ...
        AND c.RouteCode = CASE 
            ...
            WHEN @pWorkCenterCode = 'VVT_F2' THEN 'V-28_BG'
            WHEN @pWorkCenterCode = 'VVT_F3' THEN 'VE10'
			-- Modified: Mr.Triều thêm đièu kiện cho nhà máy Hưng Yên khi xem lại sản lượng hoàn thành trong ngày với con hàng Cell
			WHEN @pWorkCenterCode='VVT_F5' THEN 'V-28_HY'
            ELSE '' END
	...
	*/
	 -- Modified: Chạy bản thử nghiệm lấy trực tiệp két quả của CellLine từ hệ thống POP (2026-09-11) 
	 -- Author: Mr.Triều
	 UPDATE [SmartFactoryV2].[dbo].[STB_SavePackingTime_VVT]
SET PackQty = -1 * PackQty, EmpNo = EmpNo + '.'
WHERE PackQty > 0 
...
tung22 AS (
    ...
    WHERE (@LineCode = '*' OR b.InputLineCode = @LineCode)
      AND (@LotNo = '*' OR a.LotNo = @LotNo OR a.newLotno = @LotNo)
      AND a.RowPackQty = 1
	  --AND b.InputLineCode NOT IN ('VVC-01','VVC-02')
),
RawMaterial AS (
    SELECT 
        vvt.LotNo,
    ...
    WHERE vvt.PrintTime >= @FromDate AND vvt.PrintTime <= @ToDate  
      AND (@LotNo = '*' OR vvt.LotNo = @LotNo)
      AND RW.ProductGroupCode IN ('ElectrodeM', 'ElectrodeP') AND vvt.PackQty > 0
    GROUP BY vvt.LotNo
),
MappedMaterials AS (
    SELECT 
 ...
 -- Ở đây này: Hiện tại đang là lấy theo dựa theo bảng STB_MaterialLotInfo
 -- Hiện tại chạy POP chạy CellLine 1,2 nên chỉ lấy kết quả náy thôi
tung_v2 AS (
    SELECT 
        ...
        MLI.InitialQty AS PackQty,                 
        CAST(NULL AS INT) AS isPrinted,               
          LTRIM(RTRIM(
            LEFT(
                REPLACE(MM.MaterialName, 'HY-CAP ', ''),
                CASE WHEN CHARINDEX('(', REPLACE(MM.MaterialName, 'HY-CAP ', '')) > 0 
                     THEN CHARINDEX('(', REPLACE(MM.MaterialName, 'HY-CAP ', '')) - 1
                     ELSE LEN(REPLACE(MM.MaterialName, 'HY-CAP ', ''))
                END
            )
        )) AS PartNo,           -- MLI không có PartNo
        (RANK() OVER (PARTITION BY MLI.LotNo ORDER BY MLI.GRDate DESC)) AS RankPackQty,
    ...
    WHERE MLI.PackingID IS NOT NULL and MLI.MaterialWarehouseCode IN ('PROD_VN_WH','PROD_HY_WH')  AND MLI.CreateUserID NOT IN ('vvtworker','nhat2k','vvtworker_hy')
	AND MLI.LotNo not in ('VVQQ283R010521',
                          'VVQQ273R010521',
                          'VVQQ273R010519',
                          'VVQQ223R010526',
                          'VVQQ213R010534',
                          'VVQQ213R010533')
    AND MLI.CreateDateTime BETWEEN (CASE WHEN @LotNo <> '*' THEN '2020-08-15 10:00:00' ELSE @FromDate END) 
    AND (CASE WHEN @LotNo <> '*' THEN CONVERT(VARCHAR(19), GETDATE(), 120) ELSE @ToDate END)
),
tung22_v2 AS (
    SELECT a.*, b.InputLineCode, c.ProdDateTime, b.ControlNo,
    ...
    WHERE (@LineCode = '*' OR b.InputLineCode = @LineCode)
      ...
      --AND a.RowPackQty = 1
	  --and b.InputLineCode  in ('VVC-01','VVC-02')
	


SELECT 
    CAST(DRI.id AS VARCHAR(50)) AS id,
    DRI.newLotno, DRI.LotNo, DRI.PackingID, DRI.MaterialCode,
    CASE WHEN DRI.MaterialCode = 'ECVT30-370' THEN 'HY-CAP VEC3R0387QG (3562-JIANGHAI)' ELSE DRI.MaterialName END AS MaterialName,
    DRI.EmpNo, DRI.PrintTime, DRI.PackQty,
    CASE WHEN isPrinted IS NULL OR isPrinted = 0 THEN 'Hien(show)' ELSE 'An(hide)' END AS isPrinted,
    DRI.PartNo, DRI.InputLineCode, DRI.ProdDateTime, DRI.ControlNo,
