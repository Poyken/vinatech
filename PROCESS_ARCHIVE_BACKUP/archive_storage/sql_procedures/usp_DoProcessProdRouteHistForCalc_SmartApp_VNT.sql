...
BEGIN
	...
	-- 블룸 원자재 투입 완료여부 체크 2026.05.10 By Jackaroe
	Declare @RawMaterialInputCnt INT
	Declare @RawMaterialListCnt INT
	DECLARE @Status INT

	-- 블룸 웨이브솔더링 이후 50분 초과 여부 체크
	Declare @PreProdDateTimeForWaveSoldering DATETIME
	...
	WHERE RouteCode = @RouteCode
	 ...
	 --Modified: Mr.Triều thêm điều kiện cho nhà máy Hưng yên với công đoạn cuốn bắt buộc phải nhập dữ liệu của điện cực âm và điện cực dương
	 IF(@WorkCenterCode1 in ('VVT_F1','VVT_F2','VVT_F5') and @RouteCode in ('V-22','V-22_BG','V-22_HY'))
	 begin
	     EXEC usp_CheckInputElectrodeInputForCodeProduct @Barcode,@RouteCode
	 end
	 

	 --Mr.Triều chăn không cho hoàn thành công đoạn nếu công đoạn đó chưa đươc PASS OR FAIL
	 if(@WorkCenterCode1 = 'VVT_F4')
	 BEGIN
	      EXEC usp_CancelCompleteRouteForBG2WithConditionPassOrFail  @Barcode, @RouteCode
	 END
	 
	 -- Mr.Triều chặn nếu mà là nó được là  Fail thì không được phép ấn hoàn thành công đoạn
	 if(@WorkCenterCode1 = 'VVT_F4')
	 BEGIN
	     --EXEC usp_HandleCancelOPInputProcess  @Barcode, @RouteCode
		 BEGIN TRY
             EXEC usp_HandleCancelOPInputProcess @Barcode, @RouteCode
        END TRY
      BEGIN CATCH
	...
	END
	 ...
	 -- Chặn BG2 khi chưa sửa chữa xong thì không cho hoàn thành công đoạn
	IF(@WorkCenterCode1 = 'VVT_F4')
	BEGIN

	  -- Chặn BG2 Không chốt được công đoạn khi chưa bắn đủ NVL
	  EXEC @Status =  usp_ChecRawMaterialWhenFinishProd @Barcode, @RouteCode
	  IF(@Status <> 1)
	   BEGIN
	    RAISERROR(N'Lot này chưa bắn đủ NVL!', 16, 1);
	    RETURN ;
	   END
    -- end

	IF EXISTS (SELECT 1 FROM STB_DefectRepairInfo where controlno = (select controlNo from STB_setinfo where barcode = @Barcode) and FindRouteCode = @RouteCode)
	BEGIN

	  IF EXISTS (SELECT 1 FROM STB_RepairInfor WHERE Barcode = @Barcode and StatusRepair != 1)
        BEGIN
        RAISERROR(N'Lot này chưa hoàn thành sửa chữa!', 16, 1);
        RETURN; 
      END

	  IF NOT EXISTS(SELECT 1 FROM STB_RepairInfor where Barcode = @Barcode and RouteCode = @RouteCode)
	    BEGIN
	    RAISERROR(N'Lot này chưa tạo công đoạn repair!', 16, 1);
	    END
	END
	END
	--end
...
-- 일괄등록 금지! 마지막 등록 시간 기준 20분 이후 실적등록 가능 2021. 10. 26 SJC
  ...
  --        IF @MBISizeW <= 10
	...
	END
	*/
	-- BE 원자재 입력 완료 체크 및 웨이브솔더링 경과시간 체크
	IF @WorkCenterCode = 'VVT_F4' BEGIN
		SELECT @RawMaterialListCnt = COUNT(*)
		  FROM STB_RawMaterialInputHist
		 WHERE Barcode = @Barcode

		SELECT @RawMaterialInputCnt = COUNT(*)
		  FROM STB_RawMaterialInputHist
		 WHERE Barcode = @Barcode
		   AND ISNULL(RawMaterialBarcode, '') <> ''
		...
		-- Wavesoldering 타임 체크
		IF @RouteCode = 'VP02' BEGIN 
			SELECT @PreProdDateTimeForWaveSoldering = ProdDateTime
			  FROM STB_ProdRouteHist
			 WHERE ControlNo = @ControlNo
			   AND RouteCode = 'VP01'

			 IF DATEDIFF(MINUTE, @PreProdDateTimeForWaveSoldering, GETDATE()) > 50 BEGIN
				EXEC usp_RaiseLocalizedError @pProcessLanguage,'웨이브솔더링 이후 50분이 경과하였습니다.'
				RETURN
			 END
		END
	END
