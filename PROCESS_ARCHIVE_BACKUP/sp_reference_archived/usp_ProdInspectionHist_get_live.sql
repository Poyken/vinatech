...
-- =================================================================================================================================
--usp_ProdInspectionHist_get 'dinhmanh','VI','2025-03-01','2025-03-31','','','','VVT','',''

CREATE PROCEDURE [dbo].[usp_ProdInspectionHist_get]
	...
	IF @CompanyCode = 'VVT' BEGIN
		...
		SELECT  dateadd(hour,2 , convert(datetime,MQI.BasicDate)) BasicDate   --Mr.Tung add date, avoid decrease 1 day 
		 ...
		 WHERE 1=1
		  ...
		  AND (
		  --vanduc allow view for HY factory 20260905 START
			(@MaterialQcNo <> '*' OR @Barcode IS NOT NULL)--vanduc add

			OR	(@workcentercode = 'VVT_F3' AND MQI.WorkCenterCode = 'VVT_F3')

			OR	(@workcentercode = 'VVT_F5' AND MQI.WorkCenterCode = 'VVT_F5')--vanduc add

			--OR (@workcentercode <> 'VVT_F3' AND MQI.WorkCenterCode IN ('VVT_F1', 'VVT_F2'))-- code cũ
			OR (@workcentercode NOT IN ('VVT_F3', 'VVT_F5') AND MQI.WorkCenterCode IN ('VVT_F1', 'VVT_F2', 'VVT_F5'))
)--vanduc allow view for HY factory 20260905 END
