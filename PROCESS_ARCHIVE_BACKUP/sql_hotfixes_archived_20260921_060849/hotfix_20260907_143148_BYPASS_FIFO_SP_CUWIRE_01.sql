-- ==============================================================================
-- HOTFIX SCRIPT: BYPASS_FIFO_SP_CUWIRE_01
-- Date: 2026-09-07
-- Target Database: SmartFactoryV2
-- Single Source of Truth: Follows Rules in .agents
-- Mục đích: Thêm mã CUWIRE-01 vào danh sách bỏ qua kiểm tra FIFO trong SP usp_VVTMaterialWarehouse_validFIFO
-- ==============================================================================

USE SmartFactoryV2;
GO

ALTER PROCEDURE [dbo].[usp_VVTMaterialWarehouse_validFIFO]
...
BEGIN
	...
	Declare @MaterialLotNo         VARCHAR(20) 
		...
		if(@pKindCheck='FIFO') begin		
				...
				-- 선입선출 체크
	-- Modified: Mr.Triều 
	-- Desc: Nếu là có các mã Code cần bỏ quả việc FIFO thì có thể vào đây để thêm mã vào để bỏ quả việc check FIFO
	if(@TargetMaterialWarehouseCode not like 'HOLDING%WH' and @TargetMaterialWarehouseCode not like 'NG_RAW%WH') 		
		IF  /* @IsFIFO = 1  and*/  
			...
			'GBCP00-004',
			'GCMDPT-507',
			'CUWIRE-01' -- Bỏ qua FIFO cho mã CUWIRE-01
