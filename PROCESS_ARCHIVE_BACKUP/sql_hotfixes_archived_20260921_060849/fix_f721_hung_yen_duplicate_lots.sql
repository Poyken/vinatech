...
-- Description: Hotfix F721 (VVT_MaterialStockList / HY_MaterialStockList)
--              Khử triệt để lỗi hiển thị lặp 2 lần mã Lot tại kho Hưng Yên (ROH_HY_WH),
--              đồng thời đảm bảo hiển thị đầy đủ cả Lot nhập trực tiếp và Lot chuyển kho
--              mà không ảnh hưởng tới bất kỳ kho nào khác (VN, BG, HN, BG2).
-- =========================================================================
-- =========================================================================
...
-- exec [usp_vvt_MaterialLotInfo_get] '','','VVT', 'VVT_F4', 'MODULE_BG2_WH', '',  '',  '',  '',  '',  '',  '',  '',  '',  '',  '',  'ML20260608000087',''
...
-- =========================================================================
ALTER PROCEDURE [dbo].[usp_vvt_MaterialLotInfo_get]
...
BEGIN
	...
	where UserID=@pProcessUserID
					...
					--- cập nhật dữ liệu Ngay Thang SX của Vendor cho những Lót bị trống Ngay Thang SX
						UPDATE MDLI  
						 SET   MDLI.LotAttr10 =  [dbo].[fn_VVT_getdatebyVendorLot_MergeCode](MDLI.materialcode,    case  when MDLI.LotNo not like MDLI.materialcode+'%' then MDLI.LotNo when MDLI.LotNo like MDLI.materialcode+'#%' then stuff(MDLI.LotNo,1,len(MDLI.materialcode)+1,'') else  stuff(MDLI.LotNo,1,len(MDLI.materialcode),'') end, isnull(MDI.SourceCustomerCode,''))  
						...
						LEFT OUTER JOIN STB_MaterialDocInfo MDI ON MDD.MaterialDocNo = MDI.MaterialDocNo
						where (replace(isnull(MDLI.LotAttr10,''),' ','')='' or MDLI.LotAttr10 = '1900-01-01' or ISDATE(MDLI.LotAttr10)=0) and isnull(MDLI.LotNo,'')<>''
						and MDLI.MaterialLocationCode not like 'PROD%'
						and (MDLI.MaterialLocationCode like '%VN_WH' or MDLI.MaterialLocationCode like '%BG_WH' or MDLI.MaterialLocationCode like '%HN_WH' or MDLI.MaterialLocationCode LIKE '%BG2_WH')
						

						UPDATE MLI  
						 SET   MLI.LotAttr10 =  [dbo].[fn_VVT_getdatebyVendorLot_MergeCode](MLI.materialcode,    case  when MLI.LotNo not like MLI.materialcode+'%' then MLI.LotNo when MLI.LotNo like MLI.materialcode+'#%' then stuff(MLI.LotNo,1,len(MLI.materialcode)+1,'') else  stuff(MLI.LotNo,1,len(MLI.materialcode),'') end, isnull(MDI.SourceCustomerCode,''))  
						FROM STB_MaterialLotInfo MLI
						LEFT OUTER JOIN STB_MaterialDocLotInfo MDLI ON MLI.LotID = MDLI.LotID
						LEFT OUTER JOIN STB_MaterialDocDetail MDD ON MDD.MaterialDocDetailNo = MDLI.MaterialDocDetailNo
						LEFT OUTER JOIN STB_MaterialDocInfo MDI ON MDD.MaterialDocNo = MDI.MaterialDocNo
						where (replace(isnull(MLI.LotAttr10,''),' ','')='' or MLI.LotAttr10 = '1900-01-01' or ISDATE(MLI.LotAttr10)=0) and isnull(MLI.LotNo,'')<>''
						and MLI.MaterialLocationCode not like 'PROD%'
						and (MLI.MaterialLocationCode like '%VN_WH' or MLI.MaterialLocationCode like '%BG_WH' or MLI.MaterialLocationCode like '%HN_WH' or MLI.MaterialLocationCode LIKE '%BG2_WH')
														
if(@MaterialWarehouseCode <> 'ROH_BG_WH')  --tiến hành audit cho bắc giang
begin
print('1')
;WITH LABELINFO AS
	(
	 ...
	 basedat as (
		...
		--where  TargetMaterialWarehouseCode ='HOLDING_VN_WH' or TargetMaterialWarehouseCode ='HOLDING_BG_WH'   order by CreateDateTime DESC
			   ...
			   SELECT
				...
				-- [START] vanduc 2026-08-29: Bổ sung kho HOLDING Hưng Yên
				OR TargetMaterialWarehouseCode = 'HOLDING_HY_WH'
				-- [END] vanduc 2026-08-29
				GROUP BY
					LotID
				--ORDER BY
 ...
 table1 as (
	...
	FROM
		...
		*/
		 ...
		 MergeParentId is  null
		 and MaterialWarehouseCode LIKE @MaterialWarehouseCode
		-- [START] vanduc 2026-08-29: Fix lỗi lặp 2 lần mã Lot kho Hưng Yên trên F721 (Khử trùng giữa STB_MaterialDocLotInfo và STB_MaterialLotInfo)
		-- Code cũ:
		...
		-- Code mới:
		or (
			MaterialWarehouseCode = case when @MaterialWarehouseCode not in ('ROH_VN_WH','ROUTE_VN_WH','ROH_BG_WH','ROUTE_BG_WH','ROH_HN_WH','ROUTE_HN_WH', 'MODULE_BG2_WH', 'ROUTE_BG2_WH') then @MaterialWarehouseCode else '' end
			and not exists (
				select 1 from STB_MaterialDocLotInfo d WITH(NOLOCK) 
				where d.LotID = STB_MaterialLotInfo.LotID and d.MaterialLocationCode LIKE @MaterialWarehouseCode+'%' and d.CreateUserID <> '23091804'
			)
		)
		-- [END] vanduc 2026-08-29
		and (@MaterialCode = '*' OR MaterialCode LIKE @MaterialCode) --Mr.Duy thêm cho tìm kiếm theo mã NVL
		...
		left outer join STB_MaterialLotInfo mli  with(nolock) on mdli.LotID = ISNULL(mli.PackingID, mli.LotID)
...
--Mr Tuân: THÊM MỚI ĐIỀU KIỆN CHẶN LOTID KHỎI KHO MODULE_BG2_WH 
AND NOT (t1.LotID = 'ML20251120000334' AND t1.MaterialWarehouseCode = 'MODULE_BG2_WH')
and t1.MaterialWarehouseCode not in ('MODULE_BG2_WH_01')
...
--- (Hải Triều) thêm để khi audit cho nhà máy Bắc Giang
 AND t1.MaterialWarehouseCode LIKE @MaterialWarehouseCode + '%' 
  ...
  AND (
        ...
        @MaterialWarehouseCode <> 'ROH_BG_WH' 
        OR 
...
begin
-- mặc định là các nhà máy khác chạy vào đây
	print('2')
	;WITH LABELINFO AS
	(
		SELECT
...
,
 ...
 table1 as (
	...
	FROM
		...
		--and   Isused is null
		union all 
		select CreateDateTime ,CAST('' AS NVARCHAR(50)) AS MaterialDocDetailNo,MaterialLotNo,lotno,LotID,MaterialLocationCode,MaterialCode,currentqty,LotAttr09,replace(replace(replace(ltrim(rtrim(isnull(LotAttr10,''))),'--','-' ),'--','-' ),' ','' )as LotAttr10
		from STB_MaterialLotInfo  WITH(NOLOCK) 
		where  
		--lotid='SM20250210000005'
		--MergeParentId is  null
