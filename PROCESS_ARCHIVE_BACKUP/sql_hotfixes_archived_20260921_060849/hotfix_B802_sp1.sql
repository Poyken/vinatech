...
-- Create date: 2021-04
-- Modified by: Nguyen Van Duc (Hotfix B802 Slitting Machine Display)
-- Modified date: 2026-08-29
-- Browsable : true
-- Group :  [B802] for Vietnam Electrode report Screen
-- Description: Lấy lịch sử sản xuất điện cực (Mixing, Coating, Rollpress, Slitting)
--              Fix 2026-08-29: Bổ sung ESI.MachineCode và MM.MachineName cho công đoạn Slitting (V-11)
-- =============================================         usp_Vietnam_ElectrodeProdRouteHist_get '','','2026-08-29','2026-08-29','','VVT',''
ALTER PROCEDURE [dbo].[usp_Vietnam_ElectrodeProdRouteHist_get]
...
-- select 'YP85200STDA',190,8268346.53281095,359.49332751352,1.892070144808  union all 
 ...
 select 'HCE 200NEO',0,0,0,3.3900  union all

 --Mr.Ha request 2026-06-25
 select 'BY120STD' , 0, 0, 0, 3.23 union all
 select 'YP120NEO' , 0, 0, 0, 3.23 union all

 -- update 2026-08-21
 select 'YP85200STD',0,0,0,3.3900 union all      -- SREYPK0 (YP85-200)
 select 'CEP85200STD',0,0,0,5.1900 union all      -- SRECEK0 (CEP85-200), ghi đè giá cũ 5.43
 select 'NCM 105STD',0,0,0,8.2000 union all       -- CRNCA5-002 (NCM-105), ghi đè giá cũ 8.3
 select 'PSCAM 120NEO',0,0,0,5.0000                -- CRPSC0-002 (PSCAM-120F), giữ nguyên giá cũ, không cần sửa
 ...
 from electrodeprice
	...
	SELECT distinct
	  ...
	  --Dateadd(hour,0, convert(datetime,A.PlanDate,120 ))   PlanDate 
		 ...
		 , case 
    when A.ElectrodeRouteCode = 'V-01' 
    then (
        select sum(isnull(InputQty1, 0)) + sum(isnull(InputQty2, 0)) 
        from STB_ElectrodeMixStepInfo
        where ElectrodeLotNumber = A.ElectrodeLotNumber
    )
    else null 
end as mixingkg
...
--where ElectrodeLotNumber= A.ElectrodeLotNumber)) else A.GoodQty end  as GoodQty
	      ...
	      ,SI_outer.GradeModelCode as HqModelCode --updated
		 ...
		 --)  as JobDate
		  ...
		  case 
				when   (DATEPART(HOUR, JobDate)>8)     or    (DATEPART(HOUR, JobDate)=8 and DATEPART(MINUTE, JobDate)>0)     
		  ...
		  , (	SELECT BaseMonth
			...
			--, IsSlit,
		--	A.GradeModelCode as HqModelCode, --update here
[V-01_W01],
[V-01_W02],
[V-01_W03],
[V-01_W04],
...
--[V-02_ZZ]
	  ...
	  FROM (
		...
		SELECT 	
				...
				, EM.WorkDate    AS JobDate
				,		  case 
				when   (DATEPART(HOUR, EM.WorkDate)>10)     or    (DATEPART(HOUR, EM.WorkDate)=10 and DATEPART(MINUTE, EM.WorkDate)>0)     
				then     convert(varchar(10),EM.WorkDate,120)    
				else    convert(varchar(10),DATEADD(DAY, -1,  EM.WorkDate),120)       
				end InputTime        
			...
			FROM STB_SetInfo SI
					  ...
					  LEFT OUTER JOIN STB_VVT_ElectrodeMixingWaste EMW with(nolock) on SI.MaterialCode = EMW.MaterialCode 
			 WHERE /*DPP.LineCode = 'ELECTRODE LINE'  and*/ 
				(  EM.WorkDate between @FromDate   and  @ToDate )
			...
			UNION ALL
			  ...
			  -- 1. 
				...
				,N'코팅 (Mạ điện cực)' AS ElectrodeRouteName
						  ,case when d.CreateDateTime between @FromDate and @ToDate then ECI.ProductionQty else 0 end as ProductionQty
						  ,case when d.CreateDateTime between @FromDate and @ToDate then ECI.GoodQty  else 0 end GoodQty 
						  ...
						  ,case when d.CreateDateTime between @FromDate and @ToDate then isnull(ECI.BadQty,0)  else 0 end  as BadQty   					 
						  , ((select sum(isnull(defectweight,0)) from STB_ElectrodeWasteInfoNew  WITH(NOLOCK) where upper(barcode) = eci.ElectrodeLotNumber  and RouteCode = 'V-02' and WorkDate between @FromDate and @ToDate group by barcode)) as wasteKg	 
						  ...
						  , CASE WHEN ECI.ProductionQty = 0 THEN 0 
								when d.CreateDateTime < @FromDate or d.CreateDateTime > @ToDate then 0
						  ...
						  , (DATEADD(HOUR, -2, d.CreateDateTime))      AS JobDate   ,     
						  		  case 
        when (DATEPART(HOUR, d.CreateDateTime)>10) or (DATEPART(HOUR, d.CreateDateTime)=10 and DATEPART(MINUTE, d.CreateDateTime)>0)
        then convert(varchar(10), d.CreateDateTime, 120)
        else convert(varchar(10), DATEADD(DAY,-1, d.CreateDateTime), 120)
    end as InputTime             
...
--[V-02_ZZ]
				  FROM STB_DayProdPlan DPP with(nolock) 
						 ...
						 outer apply ( (SELECT TOP 1 * FROM STB_ElectrodeCoatingVisualInspectionInfo WHERE ElectrodeLotNumber = ECI.ElectrodeLotNumber ORDER BY CreateDateTime desc)) d
						left outer join 
						(select*from (
...
--[V-02_ZZ]
				 ...
				 WHERE /*DPP.LineCode = 'ELECTRODE LINE'  and*/ 
			(d.CreateDateTime between @FromDate and @ToDate)
			...
			SELECT  	
				...
				,N'롤프레싱 (Ép cuộn)' AS ElectrodeRouteName
					  --,case when b.Barcode is  null then 'RollPress' else 'Slitting' end  AS ElectrodeRouteName
					  	  ...
					  	  ,case when ERPVII.CreateDateTime between @FromDate and @ToDate then isnull(ERPI.ProductionQty,ERPI.GoodQty+isnull(ERPI.BadQty,0)) else 0 end as ProductionQty
						  ,case when ERPVII.CreateDateTime between @FromDate and @ToDate then ERPI.GoodQty  else 0 end GoodQty 
						  --- isnull((select sum(isnull(defectweight,0)) from STB_ElectrodeWasteInfoNew  WITH(NOLOCK) where upper(barcode) = eci.ElectrodeLotNumber group by barcode),0) as GoodQty
						  ,case when ERPVII.CreateDateTime between @FromDate and @ToDate then isnull(ERPI.BadQty,0)  else 0 end  as BadQty   
					 ...
					 -- , isnull(ownDefect,0) as ownDefect  
					  ...
					  when ERPVII.CreateDateTime < @FromDate or ERPVII.CreateDateTime > @ToDate then 0
					  ...
					  , DPP.MaterialCode         AS MaterialCode                    
					 , (Select MaterialName From STB_MaterialMaster MM with(nolock)  WHERE MM.MaterialCode = DPP.MaterialCode) AS MaterialName      			  
					 , DATEADD(HOUR, -2, ERPVII.CreateDateTime)  AS JobDate  
					 ,		  case 
				when   (DATEPART(HOUR,   ERPVII.CreateDateTime)>10)     or    (DATEPART(HOUR,  ERPVII.CreateDateTime)=10
				and DATEPART(MINUTE,   ERPVII.CreateDateTime)>0)     
				then     convert(varchar(10),  ERPVII.CreateDateTime,120)    
				else    convert(varchar(10),DATEADD(DAY, -1,    ERPVII.CreateDateTime),120)       
				end InputTime           
				      , isnull(ewin.CompanyCode,DPP.CompanyCode) as CompanyCode
					  , ERPI.MachineCode
					  , MM.MachineName
...
-- , SI.GradeModelCode
 ...
 --[V-02_ZZ]
			  FROM STB_DayProdPlan DPP with(nolock) 
					  ...
					  OUTER APPLY (SELECT TOP 1 * FROM STB_ElectrodeRollPressingVisualInspectionInfo where ElectrodeLotNumber = ERPI.ElectrodeLotNumber order by CreateDateTime desc) ERPVII
					--LEFT OUTER JOIN STB_MaterialMaster MM2	 with(nolock) 		ON SI.MaterialCode = MM2.MaterialCode
...
--[V-02_ZZ]
			 ...
			 WHERE /*DPP.LineCode = 'ELECTRODE LINE'   and */ --( (ERPI.WorkDate) between @FromDate   and  @ToDate )
			 (
  --CASE 
  --  WHEN ERPI.WorkDate >= ERPI.CreateDateTime 
  --  THEN ERPI.WorkDate
  --  ELSE ERPI.CreateDateTime
  --END
  ERPVII.CreateDateTime
		 ...
		 --2. 
			SELECT  	
					...
					,DPP.PlanDate
					  ,DPP.PlanShiftCode
					  ,ESI.ElectrodeLotNumber
					  ,ESI.WorkerCode
					  ,PWI.WorkerName
					  ,isnull(ESI.CreateUserID,isnull(ESI.ChangeUserID,'')) as CreateUserID
				...
				,N'슬리팅 (Slitting)' AS ElectrodeRouteName
					 ...
					 -- , isnull(ownDefect,0) as ownDefect  
					  ...
					  , DPP.MaterialCode    AS MaterialCode       
					 , (Select MaterialName From STB_MaterialMaster MM with(nolock)  WHERE MM.MaterialCode = DPP.MaterialCode) AS MaterialName    
					 , DATEADD(HOUR, -2, b.CreateDateTime)      AS JobDate   
					 ,		  case 
				...
				then     convert(varchar(10),b.CreateDateTime ,120)    
					  ...
					  , isnull(ewin.CompanyCode,DPP.CompanyCode) as CompanyCode
					  -- Updated 2026-08-29 (DucNV): Fix hien thi Ma may & Ten may cho cong doan Slitting
					  , ESI.MachineCode as MachineCode
 ...
 --[V-02_ZZ]
			  FROM STB_DayProdPlan DPP with(nolock) 
					  ...
					  LEFT OUTER JOIN STB_ElectrodeSlittingInfo ESI with(nolock) ON SI.Barcode = ESI.ElectrodeLotNumber
					  outer apply( 
					  --fix 2026-02-24
					  --begin
					--  select top 1 * from STB_ElectrodeSlittingResult   with(nolock) where ElectrodeLotNumber=ERPI.ElectrodeLotNumber

					--bỏ comment đoạn này nếu đoạn dưới không đúng
			--		SELECT 
   ...
   --   AND SI.MaterialCode IN ('CRYPK0-017','CRYPK0-018','CRCEK0-268','CRYPK0-016','CREYO85-04','CREHCO85',
	  --'CRYPK0-022','CRYPK0-011','CRECO85-03') -- Mr hà request 2026-03-30
	  --end
   ...
   SELECT 
    ...
    MAX(Barcode) AS Barcode
FROM STB_ElectrodeSlittingResult WITH(NOLOCK) 
WHERE ElectrodeLotNumber = ESI.ElectrodeLotNumber
  AND (SI.MaterialCode IN (
      'CRYPK0-017','CRYPK0-018','CRCEK0-268','CRYPK0-016',
      'CREHCO85','CRYPK0-022','CRYPK0-011','CRECO85-04')) -- Updated 2026-08-18: Bỏ CREYO85-04 để tính đúng sản lượng TOP 1 cuộn mẹ, tránh x2
GROUP BY CAST(DATEADD(HOUR, -10, CreateDateTime) AS DATE)
...
--end
    ...
    FROM STB_ElectrodeSlittingResult WITH(NOLOCK) 
    WHERE ElectrodeLotNumber = ESI.ElectrodeLotNumber
      AND (SI.MaterialCode NOT IN ('CRYPK0-017','CRYPK0-018','CRCEK0-268','CRYPK0-016','CREHCO85',
	  'CRYPK0-022','CRYPK0-011','CRECO85-04') OR SI.MaterialCode IS NULL OR (SI.MaterialCode = 'CRECO85-03')) -- Updated 2026-08-18: Bỏ CREYO85-04
    ORDER BY CreateDateTime DESC
	                   --end
)
					  b
					  LEFT OUTER JOIN STB_MaterialMaster MM2	 with(nolock) 		ON SI.MaterialCode = MM2.MaterialCode
					  LEFT OUTER JOIN STB_ProdWorkerInfo PWI			 with(nolock)    ON PWI.WorkerCode = ESI.WorkerCode
					  LEFT OUTER JOIN STB_MachineMaster MM  with(nolock) ON MM.MachineCode = ESI.MachineCode
					    LEFT OUTER JOIN Electrode_BN_BG EBB with(nolock) ON DPP.MaterialCode = EBB.MaterialCode 


					--update 21/8/2026
					  outer apply (
							select top 1 
								isnull(ep.Price, e2p.unitUSD) as unitUSD
							from (select 1 as dummy) x
							outer apply (
								select top 1 Price 
								from STB_ElectrodePriceB802 WITH(NOLOCK)
								where MaterialCode = DPP.MaterialCode
							) ep
							outer apply (
								select top 1 unitUSD 
								from electrode2price 
								where mm2.MaterialName like cath 
								  and (mm2.MaterialName like kinda or mm2.MaterialSource+mm2.MaterialThickness like kinda)
							) e2p
						) tbprice
...
--[V-02_ZZ]
...
)              ewin            on  ESI.ElectrodeLotNumber=ewin.barcode
			 WHERE /*DPP.LineCode = 'ELECTRODE LINE' 			 and */ 
		...
		LEFT JOIN STB_SetInfo SI_outer WITH(NOLOCK) ON A.ElectrodeLotNumber = SI_outer.Barcode  --updated
		...
		outer apply(
	       ...
	       where 
			...
			ORDER BY 
             LEN(emeterMixing.thick) DESC --updated 2026-06-11
	   )  meterMixing
	 ...
	 WHERE 1=1		
		...
		or  (ElectrodeLotNumber IN( 'VWQN2820001E17','VWQN2020001E16','VWQN1920001E19') AND ElectrodeRouteCode = 'V-03') )--following Mr Ha's request
