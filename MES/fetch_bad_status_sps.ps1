...
function Export-SP($spName, $outputPath) {
    ...
    if ($defn) {
        ...
        Write-Host "Exported $spName to $outputPath successfully." -ForegroundColor Green
    } else {
        Write-Host "Failed to find $spName in database." -ForegroundColor Red
    }
}

Export-SP "usp_Get_VVT_Prod_Bad_Status" "c:\Users\User Vinatech.DESKTOP-RJJSEQU\Desktop\MES\sql\procedures\usp_Get_VVT_Prod_Bad_Status.sql"
Export-SP "usp_Get_VVT_Prod_Bad_Stat_tail" "c:\Users\User Vinatech.DESKTOP-RJJSEQU\Desktop\MES\sql\procedures\usp_Get_VVT_Prod_Bad_Stat_tail.sql"
