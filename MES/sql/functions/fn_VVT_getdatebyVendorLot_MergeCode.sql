...
declare @test varchar(200)=@materialcode +'--'+@vendorlot+'--'+@sourceCustomerCode
	...
	WHEN @materialcode = 'GBRLAC-005' THEN 
        CASE 
            -- Nếu là NCC VV040 (mã lot dạng 06218260523... ngày bắt đầu từ vị trí thứ 6)
            WHEN @sourceCustomerCode = 'VV040' THEN 
                '20' + SUBSTRING(@vendorlot, 6, 2) + '-' + 
                SUBSTRING(@vendorlot, 8, 2) + '-' + 
                SUBSTRING(@vendorlot, 10, 2)
            -- Fallback cho các NCC khác (mã lot cũ ngày bắt đầu từ vị trí thứ 5)
            ELSE 
