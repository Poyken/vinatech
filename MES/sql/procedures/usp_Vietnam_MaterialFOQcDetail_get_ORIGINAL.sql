...
-- 프로시저 실행문 :  EXEC usp_MaterialQcDetail_get '','','VVOO123R825705'
-- =======================================================================================================================
CREATE  PROCEDURE [dbo].[usp_Vietnam_MaterialFOQcDetail_get]
...
--	WHERE OldBarcode in (
--		select replace(MaterialQcNo,'FV','V')  
--		from	STB_MaterialQcInfo  WITH(NOLOCK) 
--		where DecisionResult='Pass' and InspectionDocType='FOQC'
--		and CreateDateTime>=DATEADD(DAY,-10,getdate())
--		)and CreateDateTime>=DATEADD(DAY,-10,getdate())
--	)
--	,
--newbar1 as (
--	select replace(MaterialQcNo,'FV','V') as MaterialQcNo
--	from	STB_MaterialQcInfo  WITH(NOLOCK) 
--	where   replace(MaterialQcNo,'FV','V') in (			
...
--select 
--		'F'+newbar2.NewBarcode as  MaterialQcNo, CompanyCode, WorkCenterCode, InspectionDocType, 
...
--		 join newbar2 on mqi.MaterialQcNo = 'F'+newbar2.OldBarcode
--		 where MaterialQcNo = @pMaterialQcNo
			...
			if(@cCount=0)
			 begin
			  ...
			  insert into STB_MaterialQcDetail 
				...
				--(SELECT USL  FROM [STB_MaterialQcInspectionItem]  where MaterialCode='VVLR153R050506' and QcInspectionItemCode='IQC_GPD_19') as USL, 
				(case when @FoqcMaterialQcNo like 'FM%' then 0 else LSL end) as LSL, 
				--(SELECT LSL  FROM [STB_MaterialQcInspectionItem]  where MaterialCode='VVLR153R050506' and QcInspectionItemCode='IQC_GPD_19') as LSL, 
				UCL, LCL, TextSpecValue, NULL as DecisionResult, getdate() as CreateDateTime, 
				@pProcessUserID as CreateUserID, ChangeDateTime, ChangeUserID--,null as Cpk
				from
...
--select*from STB_MaterialQcDetail
--where MaterialQcNo like '%VVMR133R850610%'