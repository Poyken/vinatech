...
-- vanduc 2026-06-05 : Restrict RouteCode to Cell Line only (exclude VE/VP) to fix B682 defect contamination
...
-- =============================================
ALTER PROCEDURE [dbo].[usp_Get_VVT_Prod_Bad_Stat_tail]
...
RawView as (
		...
		from  STB_SetInfo c with(nolock) 
		 ...
		 and b.RouteCode LIKE 'V%' and b.RouteCode NOT LIKE 'VE%' and b.RouteCode NOT LIKE 'VP%' -- vanduc 2026-06-05: Loc lay loi tu Cell Line va C530 OQC
...
X as(
	...
	SELECT
           dbo.fnGetLocalTime(PRS.JobDate, 420) as 작업일자,
			...
			CASE
			WHEN PRS.MaterialCode = 'ECVT27-344' THEN 'HY-CAP WEC2R7106QG (1030 Low)'
			else MM.MaterialName end 품목명,
