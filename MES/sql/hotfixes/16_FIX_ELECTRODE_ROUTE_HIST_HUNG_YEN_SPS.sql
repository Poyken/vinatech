...
PRINT 'Starting Hotfix: 16_FIX_ELECTRODE_ROUTE_HIST_HUNG_YEN_SPS...';
GO
-- =========================================================
-- 1. Stored Procedure: usp_Vietnam_ElectrodeDefectHist_HY_get
-- =========================================================
IF EXISTS (SELECT 1 FROM sys.procedures WHERE name = 'usp_Vietnam_ElectrodeDefectHist_HY_get')
    DROP PROCEDURE [dbo].[usp_Vietnam_ElectrodeDefectHist_HY_get];
GO
...
-- Description:	
-- =============================================
CREATE PROCEDURE [dbo].[usp_Vietnam_ElectrodeDefectHist_HY_get]
						...
						--usp_Vietnam_ElectrodeDefectHist_get '','','2026-05-12','2026-05-12','','VVT','','VVT_F1'
AS
...
select lasttable.*, 
	     ...
	     case  when 'Coating_Meter' = lasttable.DefectCode then N'Coating_Lỗi Mét chuyển đổi thành KG' 
		 ...
		 di.DefectEnglishName --thuctd add column
...
PRINT 'Procedure usp_Vietnam_ElectrodeDefectHist_HY_get created successfully.';
GO
-- =========================================================
-- 2. Stored Procedure: usp_Vietnam_ElectrodeProdRouteHist_HY_get
-- =========================================================
IF EXISTS (SELECT 1 FROM sys.procedures WHERE name = 'usp_Vietnam_ElectrodeProdRouteHist_HY_get')
    DROP PROCEDURE [dbo].[usp_Vietnam_ElectrodeProdRouteHist_HY_get];
GO
...
CREATE PROCEDURE [dbo].[usp_Vietnam_ElectrodeProdRouteHist_HY_get]
						@pProcessUserID VARCHAR(20), 
						@pProcessLanguage VARCHAR(20), 
						@pFromDate DATE, 
						@pToDate DATE,					
...
--[V-02_ZZ]
				 ...
				 WHERE /*DPP.LineCode = 'ELECTRODE LINE'  and*/ 
				(  ECI.WorkDate between @FromDate   and  @ToDate )
...
WHERE ElectrodeLotNumber = ERPI.ElectrodeLotNumber
  AND SI.MaterialCode IN (
      ...
      'CREYO85-04','CREHCO85','CRYPK0-022','CRYPK0-011','CRECO85-03'
  )
...
--end
    ...
    FROM STB_ElectrodeSlittingResult WITH(NOLOCK) 
    WHERE ElectrodeLotNumber = ERPI.ElectrodeLotNumber
      AND (SI.MaterialCode NOT IN ('CRYPK0-017','CRYPK0-018','CRCEK0-268','CRYPK0-016','CREYO85-04','CREHCO85',
	  'CRYPK0-022','CRYPK0-011','CRECO85-03') OR SI.MaterialCode IS NULL) -- Mr hà request 2026-03-30
...
PRINT 'Procedure usp_Vietnam_ElectrodeProdRouteHist_HY_get created successfully.';
GO
COMMIT TRAN;
