-- =============================================
-- BACKUP OF usp_SanminaShipmentPlan_iud
-- DATE: 2026-08-20
-- =============================================
CREATE PROCEDURE [dbo].[usp_SanminaShipmentPlan_iud]
...
BEGIN
    ...
    BEGIN TRY
        ...
        SET 
            ...
            TargetTable.PrintedBoxCount = ISNULL(SourceTable.PrintedBoxCount, TargetTable.PrintedBoxCount),
            TargetTable.StartSerial = SourceTable.StartSerial,
            TargetTable.IsActive = ISNULL(SourceTable.IsActive, TargetTable.IsActive),
            TargetTable.Status = CASE 
                WHEN ISNULL(SourceTable.PrintedBoxCount, TargetTable.PrintedBoxCount) >= ISNULL(SourceTable.TotalBox, TargetTable.TotalBox) THEN 'COMPLETED'
                WHEN ISNULL(SourceTable.IsActive, TargetTable.IsActive) = 1 THEN 'ACTIVE' 
                ELSE TargetTable.Status 
            END,
        ...
        FROM STB_SanminaShipmentPlan AS TargetTable
        INNER JOIN (
            SELECT PlanID FROM OPENXML(@DeleteTableName, 2)
            WITH (PlanID INT)
        ) AS SourceTable ON TargetTable.PlanID = SourceTable.PlanID;
