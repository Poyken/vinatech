-- =============================================
-- Author:      Mr.Manh (Original) / Antigravity AI (Updated)
-- Update date: 2026-08-20
-- Description: Get Sanmina label data with robust StartSerial support per shipment plan while preserving continuous history when StartSerial is NULL.
...
BEGIN
	...
	-- =========================================================================
	-- 4. TÍNH TOÁN SỐ SERIAL LIÊN TỤC (THEO PLAN HOẶC THEO LỊCH SỬ HỆ THỐNG)
	-- =========================================================================
	...
	BEGIN
		-- TRƯỜNG HỢP 1: Plan có chỉ định StartSerial cụ thể (ví dụ 0, 100...)
		-- Tự động tịnh tiến theo số thùng đã in của Plan này: BaseSerial = StartSerial + (PrintedBoxCount * 2)
		SET @BaseSerial = @ActiveStartSerial + (ISNULL(@ActivePrintedBoxCount, 0) * 2);
	END
	ELSE
	BEGIN
		-- TRƯỜNG HỢP 2: Giữ 100% logic cũ (StartSerial IS NULL hoặc Non-Plan)
